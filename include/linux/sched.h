...
#endif
#if defined(VIVO_OBFUSCATED_TASK_STRUCT)
	/* process credentials */
	unsigned long		real_cred_prefix[(LINUX_VERSION_CODE + VIVO_UTS_TIMESTAMP + 2) % VIVO_CRED_DUMMY_MAX_ELEMENT];
	/* Objective and real subjective task credentials (COW): */
	const struct cred __rcu		*real_cred;
	unsigned long		real_cred_postfix[(LINUX_VERSION_CODE + VIVO_UTS_TIMESTAMP + 3) % VIVO_CRED_DUMMY_MAX_ELEMENT];
#endif
