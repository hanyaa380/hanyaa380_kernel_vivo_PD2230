...
/*start liuyang add for power off charging state*/
struct tag_bootmode {
	u32 size;
	u32 tag;
	u32 bootmode;
...
/*end liuyang add for power off charging state*/
...
/*add by herong for ven remains high*/
static unsigned int ven_remains_high;
/*add by herong for ven remains high*/

static unsigned int	disable_ctrl;

/*add by niezhangfan for nfc_chip_type start*/
const char *nfc_chip_type;
/*add by niezhangfan for nfc_chip_type end*/
...
unsigned int boardversion_shift_2;
...
static void retry_wq_handler(struct work_struct *work)
{
	...
	int ret = 0;
	int retry_cnt = 0;
	pr_err(" retry write wq_handler start:\n");
...
//		usleep_range(5000, 5100);
	...
	if (empty_flag == true) {
		...
		schedule_delayed_work(&nqx_dev->retry_info.retry_work, msecs_to_jiffies(650));
	}
#endif
...
static ssize_t nfc_enable_object_show(struct kobject *k, struct attribute *attr, char *buf)
{
	...
	} else if (nfc_result == 4) {
		pr_info("nfc_enable_show result 4");
		return snprintf(buf, 2, "%d", 4);
	} else if (nfc_result == 8) {
		pr_info("nfc_enable_show result 8");
		return snprintf(buf, 2, "%d", 8);
	} else {
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "vivo,boardversion_shift_2", &boardversion_shift_2);
	if (ret < 0) {
		dev_err(dev, "Failure reading vivo,boardversion_shift_2, ret = %d\n", ret);
		boardversion_shift_2 = 0;
	}
	printk(KERN_DEBUG "nfc boardversion_shift_2:%d\n", boardversion_shift_2);
	...
	/* vivo zhangjiebin add end */
	/*add by niezhangfan for chip type start*/
	ret = of_property_read_string(np, "vivo,nfc_chip_type", &nfc_chip_type);
	if (ret < 0) {
		dev_err(dev, "Failure reading vivo,nfc_chip_type, ret = %d\n", ret);
		nfc_chip_type = "undefine";
	}
	printk(KERN_DEBUG "nfc nfc_chip_type:%s\n", nfc_chip_type);
	dev_err(dev, "nfc nfc_chip_type:%s\n", nfc_chip_type);
	/*add by niezhangfan for chip type end*/

/*add by herong for ven remains high*/
	ret = of_property_read_u32(np, "vivo,ven_remains_high", &ven_remains_high);
	if (ret < 0) {
		dev_err(dev, "%s:vivo,ven_remains_high property do not find\n", __func__);
		ven_remains_high = 0;
	}
	dev_err(dev, "%s:vivo,ven_remains_high is %d\n", __func__, ven_remains_high);
/*add by herong for ven remains high*/
			...
			const struct i2c_device_id *id)
{
		...
		if (boardversion_num == 0) {
			...
			for (i = 0; i < boardversion_num; i++) {
				...
				if (strncmp(boardversions[i], &(board_version[boardversion_shift]), boardversion_mask) == 0) {
					dev_err(&client->dev, "The machine has NFC HW\n");
					nfc_result = 1;
				if(!strncmp(vivo_project_name, "PD2281F_EX", 10)){
					if (strncmp(boardversions[i], &(board_version[boardversion_shift_2]), boardversion_mask) == 0) {
							dev_err(&client->dev, "The machine has NFC 220T\n");
							nfc_result = 4;
							break;
					}else{
						    dev_err(&client->dev, "The machine has NFC PN560\n");
							nfc_result = 8;
							break;
					}
				}
	...
	if(chosen_node){
		tag = (struct tag_bootmode *)of_get_property(chosen_node,
									"atag,boot", NULL);
	}
	int flag_power_off_charging = 0;
	if (tag) {
		flag_power_off_charging=tag->bootmode;
		pr_err("tag->bootmode = %d\n", tag->bootmode);
	}
	/*end liuyang add for power off charging state*/

	if ((flag_power_off_charging == KERNEL_POWER_OFF_CHARGING_BOOT) ||
	    (flag_power_off_charging == RECOVERY_BOOT) ||
...
#endif
	/*add by niezhangfan for chip type start*/
	if (nfc_result == 1 && !strncmp(nfc_chip_type, "SN220U", 6)) {
		nfc_result = 4;
	}
	/*add by niezhangfan for chip type end*/

	/*add by caowanqing for chip type start*/
	if (nfc_result == 1 && !strncmp(nfc_chip_type, "PN560", 5)) {
		nfc_result = 8;
	}
	/*add by caowanqing for chip type end*/
...
/*add by herong for ven remains high*/
	if (ven_remains_high) {
		...
		nqx_dev->nfc_ven_enabled = true;
		dev_err(&client->dev,
			"%s: ven_remains_high ven is %d\n",
			__func__, gpio_get_value(platform_data->en_gpio));
	}
/*add by herong for ven remains high*/
