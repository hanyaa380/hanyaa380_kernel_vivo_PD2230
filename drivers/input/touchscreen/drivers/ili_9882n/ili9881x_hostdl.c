...
{
	int i = 0, ret = UPDATE_PASS;
	u32 mode = 0, crc = 0, dma = 0, iram_crc = 0;
...
{
	int i = 0;
	u32 ex_addr = 0, data_crc = 0, file_crc = 0;
...
static int ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ILI_INFO("report_info_block : nReportByPixel = %d, nIsHostDownload = %d, nIsSPIICE = %d, nIsSPISLAVE = %d\n", ilits->rib.nReportByPixel,
		 ilits->rib.nIsHostDownload, ilits->rib.nIsSPIICE, ilits->rib.nIsSPISLAVE);
	ILI_INFO("report_info_block : nIsI2C = %d, nReserved00 = %d, nReportResolutionMode = %x, nCustomerType = %x, nReserved02 = %x,  nReserved03 = %x\n",
		 ilits->rib.nIsI2C, ilits->rib.nReserved00, ilits->rib.nReportResolutionMode, ilits->rib.nCustomerType, ilits->rib.nReserved02,
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	fw_get.data = NULL;
	ILI_INFO("Open file method = %d", op);
...
out:
	...
	if (ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE) {
		ILI_INFO("gesture mode not set data len\n");
		return ret;
	}
	return ret;
}
