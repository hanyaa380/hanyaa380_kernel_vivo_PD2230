...
static int ilitek_plat_gpio_register(void)
{
	...
	if (IS_ERR_OR_NULL(ilits->pinctrl)) {
		VTE("fail to get pinctrl !!!");
	} else {
		ilits->pinctrl_sleep = pinctrl_lookup_state(ilits->pinctrl, "cs_sleep");
		if (IS_ERR_OR_NULL(ilits->pinctrl_sleep)) {
			VTE("fail to get pinctrl_sleep !!!");
		}

		ilits->spi_cs_active = pinctrl_lookup_state(ilits->pinctrl, "spi_cs_set");
		if (IS_ERR_OR_NULL(ilits->spi_cs_active)) {
			VTE("select spi cs pinctrl failed");
		}

		ilits->spi_cs_sleep_pulllow = pinctrl_lookup_state(ilits->pinctrl, "spi_cs_pulllow");
		if (IS_ERR_OR_NULL(ilits->spi_cs_sleep_pulllow)) {
			VTE("select spi cs gpio pinctrl failed");
		}

		ilits->spi_miso_active = pinctrl_lookup_state(ilits->pinctrl, "spi_miso_set");
		if (IS_ERR_OR_NULL(ilits->spi_miso_active)) {
			VTE("select spi miso pinctrl failed");
		}

		ilits->spi_miso_sleep_pulllow = pinctrl_lookup_state(ilits->pinctrl, "spi_miso_pulllow");
		if (IS_ERR_OR_NULL(ilits->spi_miso_sleep_pulllow)) {
			VTE("select spi miso gpio pinctrl failed");
		}
	}
	...
	ret = gpio_request_one(ilits->tp_rst, GPIOF_OUT_INIT_HIGH, "TP_RESET");
	if (ret < 0) {
	...
	ret = gpio_request_one(ilits->tp_cs, GPIOF_OUT_INIT_HIGH, "TP_CS");
	if (ret < 0) {
...
#endif
#endif
	if (vts_get_boot_mode() == VTS_BOOT_MODE_AT) {
		VTI("set reset gpio down in at mode");
		if (gpio_is_valid(ilits->tp_rst)) {
			gpio_set_value(ilits->tp_rst, 0);
		}
	}
...
static const struct vts_operations ili9882_vts_ops = {
	...
	.set_input_method = SetKeyboardState_v2_9882n,
	.get_ic_mode = bbk_ili_read_ic_mode,
	.set_gesture = bbk_ili_set_gesture,
	.set_debug_print = bbk_set_debug_print,
	.set_virtual_prox = bbk_ili_set_proxmity_state,
};
