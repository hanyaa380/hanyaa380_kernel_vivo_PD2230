...
int ili_gesture_recovery(void)
{
	...
	if (!ilits->shutdown)
		vts_dsi_panel_reset_power_ctrl(7);
	mdelay(10);
...
{
	esd_wq = create_singlethread_workqueue("esd_check");
	bat_wq = create_singlethread_workqueue("bat_check");
...
int ili_sleep_handler(int mode)
{
	...
	case TP_RESUME:
		...
		if (ilits->prox_face_mode == PROXIMITY_SUSPEND_RESUME) {
			...
			if (ili_ic_func_ctrl("proximity", 0x01) < 0) { //亮屏拉lcm_rst，需要驱动重下FD状态
...
int ili_fw_upgrade_handler(void *data)
{
	...
	bbk_set_debug_print(ilits->vtsdev, true);
...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	u16 self_key = 2;
	int ret = 0, tp_mode = ilits->actual_tp_mode, len = 0, demo_mode_packet_len = 0;
	if (ilits->rib.nReportResolutionMode == FINGER_TYPE_CUST) {
		demo_mode_packet_len = P5_X_5B_LOW_RESOLUTION_LENGTH;
	} else {
		demo_mode_packet_len = P5_X_DEMO_MODE_PACKET_LEN;
	}
	
	if (ilits->rib.nReportResolutionMode != FINGER_TYPE_CUST && format == DATA_FORMAT_DEBUG_CUST ) {
		format = DATA_FORMAT_DEBUG;
	}
	...
	case DATA_FORMAT_DEBUG_CUST:
		...
		if (format == DATA_FORMAT_DEBUG_CUST) {
			ctrl = DATA_FORMAT_DEBUG_CUST_CMD;
		} else {
	...
	case DATA_FORMAT_DEMO_DEBUG_INFO:
		...
		len = demo_mode_packet_len + P5_X_DEMO_DEBUG_INFO_ID0_LENGTH + P5_X_INFO_HEADER_LENGTH;
	...
	if (ctrl == DATA_FORMAT_DEBUG_LITE_CMD) {
		...
		if (ret < 0 && !atomic_read(&ilits->fw_stat)) {
		...
		if (send) {
			if (ilits->prox_face_mode == PROXIMITY_SUSPEND_RESUME) { //只要prox使能，休眠就下0x20
				ret = ili_ic_func_ctrl("lpwg", 0x20); //手势与FD共存
				VTI("ic fun lpwg FD & gesture");
				if (ilits->gesture_load_code == false) { //当前IC进手势不加载code
					ret = ili_ic_func_ctrl("lpwg", 0x21); //扩展cmd留用，当前固件不响应
				}
			} else { //prox未使能，进纯手势
				ret = ili_ic_func_ctrl("lpwg", ctrl);
				VTI("ic fun lpwg ctrl");
			}
			if (ret < 0)
...
int ili_report_handler(ktime_t kt)
{
	...
	if (ilits->tr_buf[0] == P5_X_DEMO_PROXIMITY_ID) {
		ILI_INFO("packet id proximity  ilits->prox_face_mode = %d\n",ilits->prox_face_mode);
		if (ilits->prox_face_mode == PROXIMITY_SUSPEND_RESUME) { //FD模式上报事件
			ili_report_proximity_mode(ilits->tr_buf, 1);
		} else if (ilits->prox_face_mode == PROXIMITY_BACKLIGHT) { //仅关背光模式，无动作，目前不涉及
			if (ilits->tr_buf[1] == PROXIMITY_NEAR_STATE_1 || ilits->tr_buf[1] == PROXIMITY_NEAR_STATE_2 || ilits->tr_buf[1] == PROXIMITY_NEAR_STATE_3 || ilits->tr_buf[1] == PROXIMITY_NEAR_STATE_4) {
				VTD("proxmity face status : Near, cmd : %X\n", ilits->tr_buf[1]);
			} else if(ilits->tr_buf[1] == PROXIMITY_FAR_STATE_0 || ilits->tr_buf[1] == PROXIMITY_FAR_STATE_5) {
				VTD("proxmity face status : Far, cmd : %X\n", ilits->tr_buf[1]);
			} else {
				VTD("proxmity face status : Ignore, cmd : %X\n", ilits->tr_buf[1]);
			}
		}
		goto out;
	}
	...
	if (ret < 0) {
		...
		if (ret == DO_SPI_RECOVER) {
			...
			if (ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE && ilits->gesture && !ilits->prox_near) {
				...
				ILI_ERR("SPI ACK failed, doing spi recovery\n");
				vts_report_release(ilits->vtsdev);
				ili_spi_recovery();
			}
			msleep(100);
			ilits->irq_after_recovery = false;
	...
	ili_dump_data(ilits->tr_buf, 8, (rlen > 10) ? 10 : rlen, 0, "finger report");

	if (ilits->tr_buf[0] == P5_X_GESTURE_SUCCESS_PACKET_ID) {
		ILI_INFO("FW enter gesture successfully!\n");
		goto out;
	}
	...
	if (pid == P5_X_PALM_DOWN_PACKET_ID) {
		ILI_INFO("P5_X_PALM_DOWN_PACKET_ID key event");
	...
	if (pid == P5_X_PALM_UP_PACKET_ID) {
		ILI_INFO("P5_X_PALM_UP_PACKET_ID key event");
		vts_report_event_up(ilits->vtsdev, VTS_EVENT_GESTURE_LARGE_AREA_PRESS);
		goto out;
	}
	...
	case P5_X_DEMO_PACKET_ID:
		ilits->charge = (0x20 & trdata[rlen-2]) >> 5;
		ilits->frequency_index = 0x1f&trdata[rlen-2];
	...
	case P5_X_DEBUG_HIGH_RESOLUTION_PACKET_ID:
		if (ilits->rib.nReportResolutionMode == FINGER_TYPE_CUST)
			ili_report_debug_cust_mode(trdata, rlen, kt);
		else
			ili_report_debug_mode(trdata, rlen, kt);
		break;
