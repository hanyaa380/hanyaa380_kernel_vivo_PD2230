...
{
	int strlen = 0, result = 0, intermed = 0, intermedtop = 0;
	char *s = str;
...
static ssize_t ilitek_proc_rw_tp_reg_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	u32 type = 0, addr = 0, read_data = 0, write_data = 0, write_len = 0, stop_mcu = 0;
...
int ili_get_tp_recore_data(void)
{
	...
	u32 base_addr = 0x20000, addr = 0, len = 0, *ptr =  NULL, i = 0, fcnt = 0;
...
static ssize_t ilitek_proc_vivo_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if ((size - 1) > sizeof(cmd) || size <= 0) {
		ILI_INFO("ERROR! input length is invalid\n");
		return -1;
	}
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	} else if (strncmp(cmd, "switchdebugcustmode", strlen(cmd)) == 0) {
		ili_set_tp_data_len(DATA_FORMAT_DEBUG_CUST, false, NULL);
	...
	} else if (strncmp(cmd, "custlog", strlen(cmd)) == 0) {
		ILI_INFO("ioctl: cust log\n");
		if (data[1] == 0x0)
			cust_log_en = DISABLE;
		else
			cust_log_en = ENABLE;
		ILI_INFO("set cust log to %s \n", (cust_log_en ? "ON" : "OFF"));
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_IOCTL_TP_MODE_CTRL:
		...
		if(szBuf[0] == TEST_MODE) {
			ILI_INFO("ioctl: test start finger up\n");
		}else {
			if (ilits->mpstatus == true) {
				ILI_INFO("ioctl: test end finger up\n");
			}
		}
