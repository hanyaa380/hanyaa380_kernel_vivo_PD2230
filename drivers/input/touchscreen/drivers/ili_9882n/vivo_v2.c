...
int bbk_ili_set_proxmity_state(struct vts_device *vtsdev, int state)
{
	int ret = 0;
	ilits->proxmity_face = false;
	ilits->prox_face_mode = state & 0xFF;
	if (state) {
	...
	ILI_INFO("switch proximity face mode = %d, level = %d\n", ilits->prox_face_mode , state);
	return ret;
}

int SetKeyboardState_v2_9882n(struct vts_device *vtsdev, int state)
{
	int ret = 0;
	u8 cmd[3] = { 0x1, 0x20, 0x00 };

	cmd[2] = state & 0xff;
	ILI_INFO("set keyboard cmd = 0x%x , 0x%x , 0x%x \n", cmd[0], cmd[1], cmd[2]);
	ret = ilits->wrapper(cmd, 3, NULL, 0, OFF, OFF);
	if (ret < 0) {
		ILI_ERR("Write keyboard state failed\n");
	}
	return ret;
}
...
{
	int i = 0,ret = 0,rlen = 0;
	u8 cmd[2] = {0},row = 0,col = 0;
	u8 checksum = 0;
	s16 temp = 0;
	int retry = 5;
	u16 self_key = 2;
...
int bbk_ili_mode_change_v2_9882n(struct vts_device *vtsdev, int which)
{
	...
	case DEEP_SLEEP_MODE:
		...
		if (ilits->vddi_poweroff) {
			...
			if (!IS_ERR_OR_NULL(ilits->spi_miso_sleep_pulllow)) {
				pinctrl_select_state(ilits->pinctrl, ilits->spi_miso_sleep_pulllow);
				VTI("spi_miso_sleep_pulllow");
			}
		}
#endif
...
#if defined(MTK_FOR_VIVO_TS)
		if (ilits->vddi_poweroff) {
			if (!IS_ERR_OR_NULL(ilits->spi_cs_active)) {
				pinctrl_select_state(ilits->pinctrl, ilits->spi_cs_active);
				VTI("spi_cs_active");
			}
			if (!IS_ERR_OR_NULL(ilits->spi_miso_active)) {
				pinctrl_select_state(ilits->pinctrl, ilits->spi_miso_active);
				VTI("spi_miso_active");
			}
		}
#endif
...
int bbk_set_debug_print(struct vts_device *vtsdev, int enable)
{
	u32 cust_enable = 0;
	//debug信息一直打开，不依赖于外部参数
	enable = true;
	// vts_property_get(vtsdev, VTS_PROPERTY_DEBUG_CUST, &cust_enable);
	// VTI("dtsi property enable %s, debug trigger %s\n", cust_enable ? "ON" : "OFF", enable ? "ON" : "OFF");

	// if(ilits->rib.nReportResolutionMode != FINGER_TYPE_CUST) {
	// 	cust_enable = 0;
	// 	VTI("The firmware does not support cust debug mode\n");
	// }

	if (enable && cust_enable) {
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_CUST, false, NULL) < 0)
			VTE("Failed to set tp data length\n");
		return -EINVAL;
	} else {
