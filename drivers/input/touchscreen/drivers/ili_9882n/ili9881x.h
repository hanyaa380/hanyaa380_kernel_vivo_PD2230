...
// clang-format off
#define DRIVER_VERSION			"3.0.3.2.221014"
...
enum PROXIMITY_MODE {
	PROXIMITY_NULL = 0, //FD未使能模式
	PROXIMITY_SUSPEND_RESUME, //FD已使能模式
	PROXIMITY_BACKLIGHT //仅关背光模式，当前不涉及
};
...
enum PROXIMITY_REPORT_STATE {
	PROXIMITY_FAR_STATE_0 = 0, //亮屏远离
	PROXIMITY_NEAR_STATE_1 = 1, //亮屏靠近
	PROXIMITY_NEAR_STATE_2 = 2, //亮屏靠近
	PROXIMITY_NEAR_STATE_3 = 3, //亮屏靠近
	PROXIMITY_NEAR_STATE_4 = 4, //灭屏靠近
	PROXIMITY_FAR_STATE_5 = 5, //灭屏远离
};
...
struct report_info_block {
	...
	u8 nIsI2C		:1;
	u8 nReserved00		:3;
	u8 nReportResolutionMode:3; //与FINGER_TYPE_CUST对比，判断是否为新固件
	u8 nCustomerType	:5; //后续固件扩展预留
	u8 nReserved02		:8;
	u8 nReserved03		:8;
};
...
#define SPI_ESD_GESTURE_CORE146_RUN			0x5B92 //密码修改，用于升级手势固件的方式
...
#define DATA_FORMAT_DEBUG_CUST_CMD			0x08 //A8协议命令，下发固件打开rawdata打印
#define P5_X_DEMO_MODE_PACKET_INFO_LEN			3
#define P5_X_DEMO_MODE_PACKET_LEN			43 //5A协议的长度
#define P5_X_DEMO_MODE_PACKET_LEN_HIGH_RESOLUTION	72 //高分辨率普通报点协议长度
#define P5_X_DEMO_MODE_AXIS_LEN				50 //未用到，预留
#define P5_X_DEMO_MODE_STATE_INFO			16 //未用到，预留
#define P5_X_INFO_HEADER_LENGTH				3
#define P5_X_INFO_CHECKSUM_LENGTH			1
#define P5_X_DEMO_DEBUG_INFO_ID0_LENGTH			14
#define P5_X_DEBUG_MODE_PACKET_LENGTH			1280 //debug 模式封包长度
#define P5_X_TEST_MODE_PACKET_LENGTH			1180 //未用到
#define P5_X_GESTURE_NORMAL_LENGTH			8 //手势模式不带轨迹的封包长度
#define P5_X_GESTURE_INFO_LENGTH			170 //手势模式带轨迹的封包长度
#define P5_X_DEBUG_LITE_LENGTH				300
#define P5_X_CORE_VER_THREE_LENGTH			5
#define P5_X_CORE_VER_FOUR_LENGTH			6
#define P5_X_DEBUG_LOW_RESOLUTION_FINGER_DATA_LENGTH	35 //低分辨率存放坐标的数据长度，不代表整个协议长度，仅为坐标部分
#define P5_X_DEBUG_HIGH_RESOLUTION_FINGER_DATA_LENGTH	45 //高分辨率存放坐标的数据长度，不代表整个协议长度，仅为坐标部分
#define P5_X_5B_LOW_RESOLUTION_LENGTH			62 //5B协议的长度
#define P5_X_CUSTOMER_LENGTH				50 //预留数据长度，由固件控制ilits->rib.nCustomerType来选择是否开启
#define P5_X_CUST_DEBUG_SHIFT				45 //A8协议rawdata数据前存放坐标部分的长度
...
#define P5_X_DEMO_PACKET_ID				0x5A //普通报点模式的包头
#define P5_X_DEBUG_PACKET_ID				0xA7 //debug模式的包头
#define P5_X_TEST_PACKET_ID				0xF2
#define P5_X_GESTURE_PACKET_ID				0xAA //手势模式协议包头
...
#define P5_X_PALM_DOWN_PACKET_ID			0xBB //掌压大面积按压包头
#define P5_X_PALM_UP_PACKET_ID				0xBA //掌压大面积抬起包头
#define P5_X_GESTURE_SUCCESS_PACKET_ID			0xDD //固件拉中断通知驱动成功进入手势模式包头
#define SPI_WRITE					0x82
#define SPI_READ					0x83
#define SPI_ACK						0xA3
#define P5_X_DEBUG_HIGH_RESOLUTION_PACKET_ID		0xA8 //增加A8协议debug模式的包头
#define P5_X_DEMO_HIGH_RESOLUTION_PACKET_ID		0x5B //增加5B协议普通报点模式的包头
...
#define POSITION_LOW_RESOLUTION				0x00 //未用到，预留
#define POSITION_HIGH_RESOLUTION			0x01 //未用到，预留
#define POSITION_CUSTOMER_TYPE_ON			0x00 //增加协议长度，由固件设定，目前暂未用到，预留
#define POSITION_CUSTOMER_TYPE_OFF			0x1F //增加协议长度，由固件设定，目前暂未用到，预留
#define FINGER_TYPE_CUST				0x03 //增加新A8/5B协议的固件区分标记，驱动通过该标识位来识别新旧固件(作用同dtsi属性vts-debug-cust)
// clang-format on
...
/*  add_for_charger_end  */
	...
	bool reset_poweroff;
	int support_highrate;// Event for 240HZ highrate
	bool vddi_poweroff;
	...
	bool prox_near;
	/*
	 * 由于固件加入FD功能后，固件code size不够，固件全部内容没办法一次性下载装进ic内存中，需要分批下载固件进ic
	 * 唤醒时只下载亮屏要用的固件，休眠时再下载手势唤醒的固件
	 * ilits->gesture_load_code为1表示设定为使用升级手势固件，此变量为固件控制
	 */
	bool gesture_load_code; //当前带FD固件仍沿用旧模式，该变量为0
	bool trans_xy;
	bool ss_ctrl;
	bool node_update;
	bool int_pulse;
	...
	bool shutdown;
	/*
	 * 驱动需要记录FD状态，用于resume时lcm拉reset、打esd后恢复
	 */
	bool power_status; //亮灭屏状态，false为灭屏，true为亮屏
	bool proxmity_face; //靠近远离状态，false为远离，true为靠近
	bool prox_face_gesture_loadcode; //当前不涉及
	u8 prox_face_mode; //FD使能状态
...
extern void ili_report_debug_cust_mode(u8 *buf, int len, ktime_t kt);
...
extern int SetKeyboardState_v2_9882n(struct vts_device *vtsdev, int state);
...
extern int bbk_ili_set_gesture(struct vts_device *vtsdev, int gesture_state);
extern int bbk_set_debug_print(struct vts_device *vtsdev, int enable);
extern int bbk_ili_set_proxmity_state(struct vts_device *vtsdev, int state);
