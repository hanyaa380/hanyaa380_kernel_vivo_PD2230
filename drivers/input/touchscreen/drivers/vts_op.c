...
#define call_symbol_return(symbol, fmt, arg1...) \
			do { \
				return symbol(arg1); \
		 } while (0)
#else
...
{
	call_symbol_return(mdss_report_lcm_id, " ");
	return err_default;
}
...
u32 vts_msdd_report_lcm1_id(u32 err_default)
{
	...
	call_symbol_return(mdss_report_lcm1_id, " ");
#endif
#ifdef VTS_SECOND_LCMID
	call_symbol_return(mdss_report_lcm2_id, " ");
#endif
	return err_default;
}
int vts_channel_broken_collect(int type, unsigned long long rx_bit, unsigned long long tx_bit, u32 rx_sensor_num, u32 tx_sensor_num)
{
	#ifdef TOUCH_DATA_COLLECT
	call_symbol(report_channel_broken, "%d, %lld, %lld, %d, %d", type, rx_bit, tx_bit, rx_sensor_num, tx_sensor_num);
	#endif
	return 0;
}