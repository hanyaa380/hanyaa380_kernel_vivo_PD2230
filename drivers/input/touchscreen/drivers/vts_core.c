...
static atomic_t vts_tp_count = ATOMIC_INIT(0);
...
{
	return atomic_read(&vts_tp_count);
}
...
static unsigned long int vts_get_cmdline_board_version(enum vts_type type) {
	...
	pos = strnstr(cmdline_buf, "bbk_board_version", strlen(cmdline_buf));
	...
	if (pos) {
		...
		VTI("board version is %s", state);
		if (kstrtoul(state, 16, &val)) {
			VTE("invalid command format!\n");
			val = -EINVAL;
			goto err_free;
		}
		VTI("board version is %lx", val);
	}
...
/* get lcm_software_id in cmdline, /proc/cmdline */
static int vts_get_cmdline_softid(enum vts_type type){
	struct device_node *node;
	int val = 0;
	const char *cmdline = NULL;
	char *pos;
	...
	VTI("QCOM get cmdline softid");
	if (type == VTS_TYPE_MAIN) {
		pos = strnstr(cmdline_buf, "dsi_display0", strlen(cmdline_buf));  //�����ֶ�
		if (pos)
			pos = strnstr(pos, "lcm_software_id", strlen(pos));
	} else if (type == VTS_TYPE_SECOND) {
		pos = strnstr(cmdline_buf, "dsi_display1", strlen(cmdline_buf));  //�����ֶ�
		if (pos)
			pos = strnstr(pos, "lcm_software_id", strlen(pos));
	} else {
		VTE("error vts_type!");
...
#else
	VTI("except QCOM get cmdline softid");
	if (type == VTS_TYPE_MAIN) {
		pos = strnstr(cmdline_buf, "lcm_software_id", strlen(cmdline_buf));
	} else if (type == VTS_TYPE_SECOND) {
		pos = strnstr(cmdline_buf, "lcm_module_id", strlen(cmdline_buf));
	} else {
		VTE("error vts_type!");
		val = -EINVAL;
		goto err_free;
...
int vts_get_lcmid_compatible(struct vts_device *vtsdev, u32 *lcmid, size_t *size)
{
	...
	for (i = 0; i < vtsdev->module->lcmid_cnt; i++) {
		if (i >= *size)
			return -EOVERFLOW;
		if(vtsdev->module->lcmid[i] == 0xffff) {
			vtsdev->find_board_id = true;
			break;
		} else {
			vtsdev->find_board_id = false;
		}
	...
	if (vtsdev->find_board_id) {
		VTI("find board version id config");
		return 0;
	} else {
		for (i = 0; i < vtsdev->module->lcmid_cnt; i++) {
			if (i >= *size)
...
{
	struct vts_vendor_module_id *vendor_module_id = NULL;

	if (vtsdev->module->softid_cnt <= 0)
		return -EINVAL;

	list_for_each_entry(vendor_module_id, &vtsdev->module->vendor_module_list, entry) {
		if (vtsdev->module->softid_cmdline == vendor_module_id->softid) {
			*mod_id = vendor_module_id->module_id;
			break;
		}
	}
	return 0;
...
static int vts_parse_module_id(struct vts_panel_module *module, struct device_node *np)
{
	...
	u32 *arr_32 = NULL;
	const u32 *arr = NULL;
	struct vts_module_id *module_id = NULL;
...
static int vts_parse_softid_module_id(struct vts_panel_module *module, struct device_node *np)
{
	...
	parse_property_u32_with_default(np, "vts-softid-match-bit", &module->module_match_bit, 0xFF);
	VTI("get module_match_bit 0x%x", module->module_match_bit);
...
/* parse board version id in dts tp_module */
static int vts_parse_board_version_id(struct vts_panel_module *module, struct device_node *np)
{
	int i = 0;
	int ret = 0;
	u32 length = 0;
	...
	u32 *arr_32 = NULL;
	const u32 *arr = NULL;
	struct vts_board_version_id *board_version_id = NULL;

	INIT_LIST_HEAD(&module->board_version_id_list);
	arr = of_get_property(np, "vts-board-version-id", &length);
	if (!arr) {
		VTE("vts-board-version-id not found");
		return -EINVAL;
	}

	length = length / sizeof(u32);
	if (length <= 0 || length % 3 != 0) {
		VTE("syntax error for vts board version id");
		return -EINVAL;
	}
	...
	ret = of_property_read_u32_array(np, "vts-board-version-id", arr_32, length);
	if (ret) {
		VTE(" read vts-softid-module_id failed");
		kfree(arr_32);
		return -EINVAL;
	...
	for (i = 0; i < length; i += 3) {
		board_version_id = kzalloc(sizeof(*board_version_id), GFP_KERNEL);
		if (!board_version_id) {
			VTE("kmalloc vts board version id fail!");
			continue;
		}

		board_version_id->board_id_bit  = arr_32[i];
		board_version_id->board_id_val = arr_32[i + 1];
		board_version_id->module_id = arr_32[i + 2];
		VTI("vts board version id array value %d, %d, %d", board_version_id->board_id_bit, board_version_id->board_id_val,
			board_version_id->module_id);
		INIT_LIST_HEAD(&board_version_id->entry);
		list_add_tail(&board_version_id->entry, &module->board_version_id_list);
	}
	ret = length / 3;
	kfree(arr_32);
	return ret;
}
struct module_ic_number {
	struct list_head list;
	u32 ic_number;
};
static LIST_HEAD(module_ic_number_list);
static int vts_parse_module_property(struct vts_panel_module *module, struct device_node *np)
{
	int ret = 0;
	struct module_ic_number *new_module_ic_number = NULL;
	...
	new_module_ic_number = kzalloc(sizeof(struct module_ic_number), GFP_KERNEL);
	INIT_LIST_HEAD(&new_module_ic_number->list);
	/* need to configure in dtsi file start */
	parse_property_u32(np, "vts-ic-number", &module->properties[VTS_PROPERTY_IC_NUMBER]);
	new_module_ic_number->ic_number = module->properties[VTS_PROPERTY_IC_NUMBER];
	list_add_tail(&new_module_ic_number->list, &module_ic_number_list);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 5, 0)
	module->board_version_id_cnt = of_property_read_variable_u32_array(np, "vts-board-version-id", module->board_version_id, 1, ARRAY_SIZE(module->board_version_id));
	VTI("board_version_id_cnt:%d\n", module->board_version_id_cnt);
#endif

	if (module->board_version_id_cnt > 0) {
		vts_parse_board_version_id(module, np);
	}
...
static void vts_module_dump(struct vts_panel_module *module)
{
	...
	memset(softid_buf, 0, sizeof(softid_buf));
	for (i = 0; i < module->lcmid_cnt; i++)
...
{
	struct vts_vendor_module_id *vendor_module_id = {0};
...
{
	int dts_ic_number = 0;
	int mult_ic_number = 0;
	int ret = 0;
	struct module_ic_number *entry;
	if(node == NULL){
	...
	ret = of_property_read_u32(node, "mult-ic-number", &mult_ic_number);
	if (ret)
		VTI("mult-ic-number not config");

	VTI("ic number in node:%d; in module:%d; check number:%d\n", dts_ic_number, match_ic_number, mult_ic_number);

	if (mult_ic_number == 1) { //˫����Ŀ������ͬ����ic number��ͬ,����device node���ô�����
		if (!list_empty(&module_ic_number_list)) {
			list_for_each_entry(entry, &module_ic_number_list, list) {
				if(dts_ic_number == entry->ic_number) {
					VTI("device node match ic number %d", entry->ic_number);
					return true;
				}
			}
		}
...
{
	struct vts_panel_module *module = NULL;
	u32 softid = 0;

	vts_parse_module_in_dts();

	mutex_lock(&modules_mutex);
	vtsdev->cmdline_board_version = vts_get_cmdline_board_version(type);
	VTI("vtsdev->cmdline_board_version is %lx", vtsdev->cmdline_board_version);
	
	softid = vts_get_cmdline_softid(type);
	list_for_each_entry(module, &vts_panel_modules, list) {
		if (module->vts_lcm_module_active) {
			if (!vts_module_contain_vts_lcm_module(module, vts_current_module_softid()))
				continue;
		} else if (module->lcmid_cnt <= 0) {
			softid &= module->module_match_bit;
			VTI("use softid 0x%x, type %d", softid, type);
...
int vts_register_driver(struct vts_device *vtsdev)
{
	...
	atomic_inc(&vts_tp_count);
	vts_dev_info(vtsdev, "register vtsdev success, tp count %d", atomic_read(&vts_tp_count));
	return 0;
...
bool vts_is_load_driver(char *driver_name, const int *ic_numbers, size_t size)
{
	...
	list_for_each_entry(module, &vts_panel_modules, list) {
		if (module->vts_lcm_module_active) {
			/* 配置vts-lcm-module匹配softid，部分项目使用，升级项目会用，保留 */
			if (vts_module_contain_vts_lcm_module(module, vts_current_module_softid()) &&
			...
			/* 配置vts-softid-module_id匹配命令行softid，后续项目使用 */
			soft_id = vts_get_cmdline_softid(module->properties[VTS_PROPERTY_PANEL_TYPE]);
			soft_id &= module->module_match_bit;
	...
	module_match = true;
	VTI("boot mode:%d, load %s driver, ic_number:%d", boot_mode, driver_name, match_ic_number);
	return true;
}
