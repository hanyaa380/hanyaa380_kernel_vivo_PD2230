...
static int32_t nvt_write_firmware(const u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	char *name = NULL;
	uint32_t BIN_addr = 0, SRAM_addr = 0, size = 0;
	uint32_t i = 0;
	uint32_t len = 0;
	int32_t count = 0;
	int32_t ret = 0;
...
static int32_t nvt_check_gesture_checksum(uint32_t SRAM_addr, uint32_t size, uint32_t crc)
{
	...
	if (nvt_read_max >= NVT_READ_LEN) {
		NVT_ERR("read len %d is larger than buffer size %d\n",
				nvt_read_max, NVT_READ_LEN);
		return -EINVAL;
...
{
	uint32_t BIN_addr = 0, SRAM_addr = 0, size = 0, crc = 0;
	uint32_t i = 0;
	uint32_t retry = 0;
	uint8_t buf[8] = {0};
	int32_t ret = 0;
