...
#if defined CONFIG_SPI_MT65XX_MODULE
...
#elif defined CONFIG_SPI_MT65XX
static struct mtk_chip_config spi_ctrdata = {
	.rx_mlsb = 1,
    .tx_mlsb = 1,
    .cs_pol = 0,
...
info_retry:	
	...
	ntnf_spi->fw_eventbuf_prot = buf[13];
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		if(retry_count < 3) {
			...
			VTE("Set default fw_ver=%d, fw_eventbuf_prot=%d, x_num=%d, y_num=%d, abs_x_max=%d, abs_y_max=%d, max_button_num=%d",
				ntnf_spi->fw_ver, ntnf_spi->fw_eventbuf_prot, ntnf_spi->x_num, ntnf_spi->y_num,
	...
	NVT_LOG("fw_ver = 0x%02X, fw_type = 0x%02X, fw_eventbuf_prot=0x%02X\n", ntnf_spi->fw_ver, buf[14],
		ntnf_spi->fw_eventbuf_prot);
...
{
	struct nvt_flash_data *dev = NULL;
...
void nvt_read_fw_history(uint32_t fw_history_addr)
{
	...
	ntnf_set_page(fw_history_addr);
	buf[0] = (uint8_t) (fw_history_addr & 0x7F);
	NTNF_CTP_SPI_READ(ntnf_spi->client, buf, 64+1);	//read 64bytes history
...
static irqreturn_t nvt_ts_work_func(int irq, void *handle, ktime_t kt)
{
	...
	if (nvt_wdt_fw_recovery(point_data)) {
		...
		nvt_read_fw_history(ntnf_spi->mmap->MMAP_HISTORY_EVENT0);
		nvt_read_fw_history(ntnf_spi->mmap->MMAP_HISTORY_EVENT1);
	...
	for (i = 0; i < ntnf_spi->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			irq_timer = jiffies;
#endif /* #if NVT_TOUCH_ESD_PROTECT */
			if (ntnf_spi->fw_eventbuf_prot == NVT_EVENTBUF_PROT_HIGH_RESO) {
				...
				if ((input_x < 0) || (input_y < 0))
					continue;
				if ((input_x > ntnf_spi->abs_x_max * NVT_COORD_HIGH_RESOL_N) ||
				    (input_y > ntnf_spi->abs_y_max * NVT_COORD_HIGH_RESOL_N))
					continue;
...
static int32_t nvt_ts_probe(struct spi_device *client, struct device_node *np)
{
	...
	if (!ntnf_spi->xbuf) {
		NVT_ERR("vts_kvzalloc for xbuf failed!\n");
		ret = -ENOMEM;
		goto xbuf_error;
	}
	
	ntnf_spi->rbuf = (uint8_t * )kzalloc(NVT_READ_LEN, GFP_KERNEL | GFP_DMA);
	if (!ntnf_spi->rbuf) {
		NVT_ERR("vts_kvzalloc for rbuf failed!\n");
		ret = -ENOMEM;
		goto rbuf_error;
	}
	...
	if (!ntnf_spi->g_data) {
		NVT_ERR("vts_kvzalloc for g_data failed!\n");
		ret = -ENOMEM;
		goto g_data_error;
	}
