...
struct vts_incell {
	...
	// int last_lcm_state;	/* last_state避免重复lock和unlock死机 */
	atomic_t is_register;	/* 判断incell panel是否注册 */
	void *notifier_entry; /* 高通drm notify 注册返回值 */
};
...
#elif defined(CONFIG_ARCH_QCOM) && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0) && LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)//QCOM kernel version > =4.19 use DRM_PANEL
...
#elif defined(QCOM_FOR_VIVO_TS) && LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)  //QCOM kernel version > = 5.10 use drm event notify
#include <drm/drm_panel.h>
#include <linux/soc/qcom/panel_event_notifier.h>
#define VTS_BLANK_UNBLANK		DRM_PANEL_EVENT_UNBLANK
#define VTS_BLANK_POWERDOWN		DRM_PANEL_EVENT_BLANK
#define VTS_EVENT_BLANK			0
#define VTS_EARLY_EVENT_BLANK	1

#define vts_lcm_register_client panel_event_notifier_register 
#define vts_lcm_unregister_client panel_event_notifier_unregister
