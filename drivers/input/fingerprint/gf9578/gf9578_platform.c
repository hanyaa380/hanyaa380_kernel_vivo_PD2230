...
static void gf9578_set_spi_status(struct gf_dev *gf_dev, int enable)
{
	if (enable) {
...
int gf9578_request_resource(struct gf_dev *gf_dev)
{
	int rc = 0;
	pr_info("%s: enter\n", __func__);
...
void gf9578_release_resource(struct gf_dev *gf_dev)
{
	...
	if (gf_dev->resource_requested) {
		...
		if (gf_dev->vdd_use_pmic) {
			regulator_put(gf_dev->vdd);
			pr_info("regulator_put vdd success\n");
		}
		...
		if (gf_dev->vddio_use_pmic) {
			regulator_put(gf_dev->vddio);
			pr_info("regulator_put vddio success\n");
		}
