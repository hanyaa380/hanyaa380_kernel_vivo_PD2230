...
static void gfspi_ioctl_clk_enable(struct gf_dev *data, uint32_t flag)
{
	vfp_spi_clk_enable(1);
}
static void gfspi_ioctl_clk_disable(struct gf_dev *data)
{
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_REQUEST_SEOURCE:
		...
		if (1 == support_soft_fpid) {
			...
			retval = gf9578_request_resource(gf_dev);
		}
		break;
...
#endif
{
	...
	if (0 == support_soft_fpid) {
		...
		status = gf9578_request_resource(gf_dev);
		if (status) {
