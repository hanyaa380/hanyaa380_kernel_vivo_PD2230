...
struct of_device_id egistec_match_table[] = {
	{
		.compatible = "mediatek,egistec-sidefp",
	},
	{
...
static void send_navi_event(struct egisfp_dev_t *egis_dev, int nav_event)
{
	...
	case NAVI_EVENT_LONG_PRESS:
		...
		break;
	case EGIS_NAV_INPUT_FINGER_EVENT:
		DEBUG_PRINT(" %s : nav finger wake up \n", __func__);
		input_event = EGIS_NAV_INPUT_FINGER_DOWN;
		break;
	default:
...
{
	gpio_direction_output(egis_dev->rstPin, 0);
...
int set_egisfp_spi_pin_active(struct egisfp_dev_t *egis_dev, int en)
{
	...
	if (en)
	{
		ret = pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_miso_active);
		ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_mosi_active);
		ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_cs_active);
		ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_clk_active);
	}
	else
	{
		ret = pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_miso_default);
		ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_mosi_default);
		ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_cs_default);
		ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->spi_clk_default);
	}
	if (ret)
		ERROR_PRINT(" %s : failed ret = %d \n", __func__, ret);
	return ret;
...
int egisfp_platforminit(struct egisfp_dev_t *egis_dev)
{
	...
	if (egis_dev != NULL)
	{
		if (!egis_dev->platforminit_done)
		{
			...
			{
				egis_dev->vcc = devm_regulator_get(&egis_dev->dd->dev, "vfp");
			...
			if (egis_dev->dd)
			{
				...
				egis_dev->spi_clk_active = pinctrl_lookup_state(egis_dev->pinctrl, "clk_spi");
				if (IS_ERR(egis_dev->spi_clk_active))
				{
					status = PTR_ERR(egis_dev->spi_clk_active);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_clk_active \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_clk_default = pinctrl_lookup_state(egis_dev->pinctrl, "clk_pulllow");
				if (IS_ERR(egis_dev->spi_clk_default))
				{
					status = PTR_ERR(egis_dev->spi_clk_default);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_clk_default \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_cs_active = pinctrl_lookup_state(egis_dev->pinctrl, "cs_spi");
				if (IS_ERR(egis_dev->spi_cs_active))
				{
					status = PTR_ERR(egis_dev->spi_cs_active);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_cs_active \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_cs_default = pinctrl_lookup_state(egis_dev->pinctrl, "cs_pulllow");
				if (IS_ERR(egis_dev->spi_cs_default))
				{
					status = PTR_ERR(egis_dev->spi_cs_default);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_cs_default \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_mosi_active = pinctrl_lookup_state(egis_dev->pinctrl, "mosi_spi");
				if (IS_ERR(egis_dev->spi_mosi_active))
				{
					status = PTR_ERR(egis_dev->spi_mosi_active);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_mosi_active \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_mosi_default = pinctrl_lookup_state(egis_dev->pinctrl, "mosi_pulllow");
				if (IS_ERR(egis_dev->spi_mosi_default))
				{
					status = PTR_ERR(egis_dev->spi_mosi_default);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_mosi_default \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_miso_active = pinctrl_lookup_state(egis_dev->pinctrl, "miso_spi");
				if (IS_ERR(egis_dev->spi_miso_active))
				{
					status = PTR_ERR(egis_dev->spi_miso_active);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_miso_active \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_miso_default = pinctrl_lookup_state(egis_dev->pinctrl, "miso_pulllow");
				if (IS_ERR(egis_dev->spi_miso_default))
				{
					status = PTR_ERR(egis_dev->spi_miso_default);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_spi_miso_default \n", __func__);
					goto egisfp_pinctrl_fail;
				}

				if (set_egisfp_spi_pin_active(egis_dev, 0))
				{
					ERROR_PRINT(" %s : set_egisfp_spi_pin fail \n", __func__);
					goto egisfp_pinctrl_fail;
				}
...
int egisfp_parse_dt(struct egisfp_dev_t *egis_dev)
{
	...
	if (node)
	{
		...
		{
			egis_dev->regulator_voltage_max = 3000000;
			egis_dev->regulator_voltage_min = 3000000;
			egis_dev->regulator_current = 100000;
			INFO_PRINT(" %s : cfg vcc regulator voltage Max = %d, Min = %d, current = %d  \n", __func__, egis_dev->regulator_voltage_max, egis_dev->regulator_voltage_min, egis_dev->regulator_current);
		}
