...
static ssize_t fp_id_object_show(struct kobject *k, struct attribute *attr, char *buf)
{
    ...
    if(support_soft_fpid) {
        ...
        } else if(fp_id == EGIS_EC617) {
            fp_frame_id = "sidefp_egis_ec617";
        }else if(fp_id == SILEAD_GSL7001) {
            fp_frame_id = "udfp_silead_gsl7001";
        } else if(fp_id == FOCALTECH_FT9395) {
            fp_frame_id = "sidefp_focaltech_ft9395";
...
static int fp_id_probe(struct platform_device *pdev)
{
    ...
    if(!strncmp(fp_project_name, "VTD1901", 7) || !strncmp(fp_project_name, "VTD1902", 7)) {
        ...
        for(i = 0; i < 6; i++) {
            printk(KERN_INFO "%s board_info i:%d value:%c", __func__, i, *(board_version + i));
        }
