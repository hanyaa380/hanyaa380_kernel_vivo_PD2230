	...
	if (gf_dev->vdd_use_pmic) {
		...
		if (regulator_count_voltages(gf_dev->vreg) > 0) {
			rc = regulator_set_voltage(gf_dev->vreg, 3000000, 3000000);
				if (rc) {
					pr_info(KERN_ERR "gf3956:Unable to set voltage on vcc_spi");
					return rc;
				}
		}
...
int gf3956_power_off(struct gf_dev *gf_dev)
{
	...
	if (gf_dev->vdd_use_pmic) {
		if (gf_dev->vreg) {
			...
			pr_info(KERN_ERR "gf3956: disable  vcc_spi %d\n", rc);
		}
	}
	...
	if (gf_dev->vdd_use_gpio) {
		...
		if (rc) {
			pr_info("gf3956 set reset gpio output fail.\n");
			return -EIO;
		}

		rc = gpio_direction_output(gf_dev->vdd_en_gpio, 0);
		if (rc) {
			pr_info ("gf3956 power off fail.\n");
			return -EINVAL;
		}
	}
