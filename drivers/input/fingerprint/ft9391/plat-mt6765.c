...
int ff_ctl_init_pins(int *irq_num)
{
	...
	err = gpio_direction_output(g_context->gpio_rst_pin, 0); // init low.
	if (err) {
...
int ff_ctl_reset_device(int level)
{
	int err = 0;
	FF_LOGV("'%s' enter.", __func__);

	FF_LOGD("reset level: '%s'.", level ? "high" : "low");

	if (unlikely(!g_context)) {
		return (-ENOSYS);
	}

	err = gpio_direction_output(g_context->gpio_rst_pin, level);
