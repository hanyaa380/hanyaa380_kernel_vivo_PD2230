...
static int silfp_hw_poweron(struct silfp_data *fp_dev)
{
    ...
    if(fp_dev->avdd_ldo) {
        ...
        if (err != 0) {
            LOG(ERR_LOG, "%s, avdd_ldo voltage set failed!", __func__);
            return err;
        }
        err = regulator_enable(fp_dev->avdd_ldo);	/*enable regulator*/
        if (err != 0) {
            LOG(ERR_LOG, "%s, avdd_ldo regulator enalbe failed!", __func__);
            return err;
        }
    }
    /*if (fp_dev->vddio_ldo) {
    ...
    LOG(INFO_LOG, "%s, power supply ret:%d ", __func__, err);
    return err;
}
...
{
    LOG(INFO_LOG, "%s, enter, port=%d", __func__, fp_dev->rst_port);
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
    ...
    if(node) {
        ...
        LOG(INFO_LOG, "%s, irq = %d", __func__, fp_dev->int_port);
    } else {
        LOG(ERR_LOG, "%s %s compatible device node is null", __func__, FP_IRQ_OF);
    }
    ...
    if(node) {
        ...
        if(pdev) {
            ...
            if(IS_ERR(fp_dev->pin.pinctrl)) {
                ...
                LOG(ERR_LOG, "%s can't find silfp pinctrl", __func__);
                return ret;
            }
        } else {
            LOG(ERR_LOG, "%s platform device is null", __func__);
        }
    } else {
        LOG(ERR_LOG, "%s %s compatible device node is null", __func__, FP_PINS_OF);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_irq)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp irq-init", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_rst_h)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp rst-high", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_mosi_spi)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp mosi_spi", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_mosi_pullhigh)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp mosi_pullhigh", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_mosi_pulllow)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp mosi_pulllow", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_miso_spi)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp miso_spi", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_miso_pullhigh)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp miso_pullhigh", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_miso_pulllow)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp miso_pulllow", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_cs_spi)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp cs_spi", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_cs_pullhigh)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp cs_pullhigh", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_cs_pulllow)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp cs_pulllow", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_clk_spi)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp clk_spi", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_clk_pullhigh)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp clk_pullhigh", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_clk_pulllow)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp clk_pulllow", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.spi_default)) {
        ...
        LOG(INFO_LOG, "%s can't find silfp spi-default", __func__);
        return ret;
    }
    ...
    if(IS_ERR_OR_NULL(fp_dev->pin.pins_avdd_h)) {
        fp_dev->pin.pins_avdd_h = NULL;
        LOG(ERR_LOG, "%s can't find silfp avdd-enable", __func__);
        // Ignore error
    }
    ...
    if(IS_ERR_OR_NULL(fp_dev->pin.pins_vddio_h)) {
        fp_dev->pin.pins_vddio_h = NULL;
        LOG(ERR_LOG, "%s can't find silfp vddio-enable", __func__);
        // Ignore error
    }
    ...
    fp_dev->avdd_ldo = regulator_get(&fp_dev->spi->dev, "vdd");
    ...
    if(!gpio_is_valid(fp_dev->avdd_port)) {
        LOG(ERR_LOG, "avdd_port GPIO is invalid.");
        //return -ENODEV;
    } else {
        LOG(INFO_LOG, "000 avdd_port GPIO is %d.", fp_dev->avdd_port);
    }

    ret = gpio_request(fp_dev->avdd_port, "silead_fp_vdd");

    if(ret) {
        LOG(ERR_LOG, "Failed to request avdd_port GPIO. ret = %d, number=%d", ret, fp_dev->avdd_port);
        //return -ENODEV;
    }

    LOG(INFO_LOG, "111 avdd_port GPIO is %d.", fp_dev->avdd_port);
#endif /* BSP_SIL_POWER_SUPPLY_GPIO */
    ...
    if(fp_dev->spi->dev.of_node) {
        ...
        if(ret) {
            fp_dev->pin.spi_id = 0;
            LOG(INFO_LOG, "Error getting spi_id");
        }
        ...
        if(ret) {
            ...
            LOG(INFO_LOG, "Error getting spi_irq");
        }
        ...
        if(ret) {
            ...
            LOG(INFO_LOG, "Error getting spi_reg");
        }
    }
...
static int silfp_set_default_spi_status(struct silfp_data *fp_dev, int enable)
{
    if(fp_dev->pin.pinctrl == NULL) {
        LOG(ERR_LOG, "pinctrl is null, do nothing!.\n");
        return 0;
    }
...
static void silfp_pinctrl_deinit(struct silfp_data *fp_dev)
{
    ...
    if(fp_dev->pin.pinctrl) {
        LOG(INFO_LOG, "%s, devm_pinctrl_put ", __func__);
...
static int silfp_pull_down_cs(struct silfp_data *fp_dev, int pull_down)
{
    ...
    LOG(INFO_LOG, "%s, pull_down=%d", __func__, pull_down);
    mdelay(1);
    return 0;
}
...
static int silfp_set_spi(struct silfp_data *fp_dev, bool enable)
{
    ...
    if(/*!fp_dev->pin.spi_id || */ !ms) {
        LOG(ERR_LOG, "%s, not support", __func__);
        return ret;
    }
    ...
    LOG(DBG_LOG, "%s, done(%d).", __func__, ret);
#else
...
static int silfp_irq_to_reset_init(struct silfp_data *fp_dev)
{
    ...
    if(IS_ERR(fp_dev->pin.pins_irq_rst_h)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp irq_rst-high", __func__);
        return ret;
    }
    ...
    if(IS_ERR(fp_dev->pin.pins_irq_rst_l)) {
        ...
        LOG(ERR_LOG, "%s can't find silfp irq_rst-low", __func__);
        return ret;
    }
...
static int silfp_set_feature(struct silfp_data *fp_dev, u8 feature)
{
    ...
    case FEATURE_FLASH_CS:
        LOG(INFO_LOG, "%s set feature flash cs", __func__);
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    if(atomic_read(&fp_dev->init)) {
        ...
        LOG(DBG_LOG, "%s, dev already init(%d).", __func__, atomic_read(&fp_dev->init));
        return status;
    }
    ...
    if (status) {
        LOG(ERR_LOG, "%s, silfp_parse_dts failed!", __func__);
        return status;
    }
    //silfp_hw_poweron(fp_dev);
    ...
    LOG(INFO_LOG, "%s, int_port %d, rst_port %d.", __func__, fp_dev->int_port, fp_dev->rst_port);
    ...
    if(ret < 0) {
        LOG(ERR_LOG, "%s, Filed to request_irq(%d), ert=%d", __func__, fp_dev->irq, ret);
        status = -ENODEV;
        goto err_irq;
    } else {
        LOG(INFO_LOG, "%s, Enable_irq_wake.", __func__);
    ...
    if(fp_dev->rst_port > 0) {
        ...
        if(ret < 0) {
            LOG(ERR_LOG, "%s, Failed to request GPIO=%d, ret=%d", __func__, (s32)fp_dev->rst_port, ret);
            status = -ENODEV;
            goto err_rst;
        } else {
