...
static void silfp_netlink_send(struct silfp_data *fp_dev, const int cmd)
{
    ...
    LOG(INFO_LOG, "%s, send cmd %d", __func__, cmd);
    if (!fp_dev || !fp_dev->nl_sk) {
        LOG(ERR_LOG, "%s, invalid socket", __func__);
        return;
    }

    if(!pid) {
        LOG(ERR_LOG, "%s, invalid PID", __func__);
        return;
    }
    ...
    if(!nlh) {
        LOG(ERR_LOG, "%s, nlmsg_put() failed", __func__);
        kfree_skb(skb);
        return;
    }
    ...
    *(char *)NLMSG_DATA(nlh) = cmd;
    ...
    if(ret == 0) {
        LOG(ERR_LOG, "%s, send failed", __func__);
        kfree_skb(skb);
        return;
    }

    LOG(INFO_LOG, "%s, sent, len=%d", __func__, ret);
}
...
static void silfp_netlink_recv(struct sk_buff *__skb)
{
    ...
    if(!skb) {
        LOG(ERR_LOG, "%s, skb NULL", __func__);
        return;
    }
    ...
    if(skb->len >= NLMSG_SPACE(0)) {
        ...
        if((strlen(str) == strlen("FP")) && !strncmp(str, "FP", strlen(str))) {
            pid = nlh->nlmsg_pid;
            LOG(ERR_LOG, "%s, PID=%d, msg=%s", __func__, pid, str);
        } else {
            LOG(ERR_LOG, "%s, Unknown PID=%d, msg=%s", __func__, nlh->nlmsg_pid, str);
        }
    } else {
        LOG(ERR_LOG, "%s, data len incorrect", __func__);
    }
...
static int silfp_netlink_init(struct silfp_data *fp_dev)
{
    ...
    if(fp_dev->nl_sk == NULL) {
        LOG(ERR_LOG, "%s, netlink create failed", __func__);
        return -1;
    }

    LOG(INFO_LOG, "%s, netlink create success", __func__);
    return 0;
}
...
static int silfp_netlink_destroy(struct silfp_data *fp_dev)
{
    ...
    LOG(ERR_LOG, "%s, no netlink socket", __func__);
    return -1;
}
#else
static void silfp_netlink_send(struct silfp_data *fp_dev, const int cmd)
{
    ...
    LOG(INFO_LOG, "silfp_poll cmd %d", cmd);
...
static ssize_t silfp_read(struct file *fd, char __user *buf, size_t len, loff_t *ptr)
{
    ...
    if(copy_to_user(buf, &list->msg, msglen)) {
        LOG(ERR_LOG, "copy_to fail");
        msglen = -EFAULT;
    } else {
        LOG(INFO_LOG, "%s, %d", __func__, list->msg);
    }
                             ...
                             unsigned long event, void *data)
{
    ...
    LOG(INFO_LOG, "%s, enter, blank=0x%x", __func__, blank);

    switch(blank) {
    case SIL_EVENT_UNBLANK:
        LOG(INFO_LOG, "%s, LCD ON", __func__);
        fp_dev->scr_off = 0;
        silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_ON);
        break;

    case SIL_EVENT_POWERDOWN:
        LOG(INFO_LOG, "%s, LCD OFF", __func__);
        fp_dev->scr_off = 1;
        silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_OFF);
        break;

    default:
        LOG(INFO_LOG, "%s, Unknown notifier", __func__);
        break;
    }
...
static void silfp_irq_disable(struct silfp_data *fp_dev)
{
    ...
    if(!fp_dev->irq_is_disable) {
        ...
        LOG(INFO_LOG, "%s, irq disabled", __func__);
    }
...
static void silfp_irq_enable(struct silfp_data *fp_dev)
{
    ...
    if(fp_dev->irq_is_disable) {
        ...
        LOG(INFO_LOG, "%s, irq enabled", __func__);
    }
...
static irqreturn_t silfp_irq_handler(int irq, void *dev_id)
{
    ...
    if(true) {
        ...
        LOG(INFO_LOG, "%s, irq ignore", __func__);
    }
...
static void silfp_work_func(struct work_struct *work)
{
    ...
    LOG(INFO_LOG, "%s, running", __func__);
    silfp_netlink_send(fp_dev, SIFP_NETLINK_IRQ);
}
...
static int silfp_keyevent(struct silfp_data	*fp_dev, struct fp_dev_key_t *pkey)
{
    ...
    if(!fp_dev->input) {
        LOG(INFO_LOG, "%s, invalid input device", __func__);
        return -1;
    }
    ...
    if(IS_KEY_VALID(pkey->value)) {
        ...
        if(fp_dev->keymap_cust.k[pkey->value - NAV_KEY_START]) {
            LOG(INFO_LOG, "%s, custom-key %d", __func__, fp_dev->keymap_cust.k[pkey->value - NAV_KEY_START]);
    ...
    for(i = 0; ret && i < ARRAY_SIZE(keymap); i++) {
        if(keymap[i].key_orig == pkey->value) {
            LOG(INFO_LOG, "%s, key %d", __func__, keymap[i].key_new);
    ...
    if(ret) {
        LOG(INFO_LOG, "%s, unregister custom-key %d", __func__, pkey->value);
...
static int silfp_proc_create_node(struct silfp_data *fp_dev)
{
    if(fp_dev->proc_entry == NULL) {
        ...
        if(fp_dev->proc_root == NULL) {
            LOG(ERR_LOG, "Create dir %s under /proc error!", PROC_DIR);
            goto err_out;
        }
        ...
        if(fp_dev->proc_entry == NULL) {
            LOG(ERR_LOG, "Create entry %s under /proc/ error!", PROC_NODE);
            goto err_out1;
        }
    }
...
static int silfp_input_init(struct silfp_data *fp_dev)
{
    ...
    if(!fp_dev->input) {
        LOG(ERR_LOG, "%s, input_allocate_device() fail!", __func__);
        status = -ENOMEM;
        return status;
    }
    ...
    if(input_register_device(fp_dev->input)) {
        LOG(ERR_LOG, "%s, input_register_device() fail!", __func__);
...
static int silfp_init(struct silfp_data *fp_dev)
{
    ...
    LOG(INFO_LOG, "%s, register_early_suspend", __func__);
...
static int silfp_resource_deinit(struct silfp_data *fp_dev)
{
    ...
    if(atomic_read(&fp_dev->init)) {
        ...
        if(!atomic_read(&fp_dev->init)) {
            LOG(INFO_LOG, "%s, no more users, free GPIOs", __func__);
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
    case SIFP_IOC_INIT:
        retval = silfp_resource_init(fp_dev, &silfp_dev_init_d);
        if (retval) {
            LOG(ERR_LOG, "%s, silfp_resource_init failed!", __func__);
            break;
        }
    ...
    case SIFP_IOC_RESET: {
        ...
        LOG(INFO_LOG, "%s, chip reset", __func__);
        ...
        if(fp_dev->power_is_off) {
            retval = silfp_hw_poweron(fp_dev);
            if (retval) {
                LOG(ERR_LOG, "%s, silfp_hw_poweron failed!", __func__);
                break;
            }
            mdelay(5);
        }
    ...
    case SIFP_IOC_ENABLE_IRQ:
        LOG(INFO_LOG, "%s, enable irq", __func__);
        silfp_irq_enable(fp_dev);
        break;

    case SIFP_IOC_DISABLE_IRQ:
        LOG(INFO_LOG, "%s, disable irq", __func__);
        silfp_irq_disable(fp_dev);
        break;

    case SIFP_IOC_CLR_IRQ:
        LOG(INFO_LOG, "%s, clear irq", __func__);
        reinit_completion(&fp_dev->done);
        break;

    case SIFP_IOC_WAIT_IRQ:
        LOG(INFO_LOG, "%s, wait irq", __func__);
        retval = __put_user(silfp_wait_irq(fp_dev)?1:0, (__u8 __user *)arg);
        break;

    case SIFP_IOC_IRQ_STATUS:
        LOG(INFO_LOG, "%s, irq status", __func__);
    ...
    case SIFP_IOC_KEY_EVENT:
        if(copy_from_user(&key, (struct fp_dev_key_t *)arg, sizeof(struct fp_dev_key_t))) {
            LOG(ERR_LOG, "%s, copy key fail!", __func__);
            retval = -EFAULT;
        } else {
    ...
    case SIFP_IOC_SCR_STATUS:
        if(arg) {
            LOG(INFO_LOG, "[IOC_SCR_STATUS] put v = %d", (u8)(!fp_dev->scr_off));
            retval = __put_user((u8)(!fp_dev->scr_off), (__u8 __user *)arg);
        } else {
            LOG(INFO_LOG, "[IOC_SCR_STATUS] send v = %d", (u8)(!fp_dev->scr_off));
    ...
    case SIFP_IOC_GET_VER:
        if(copy_to_user((void __user *)arg, (void *)FP_DEV_VERSION, sizeof(char)*7)) {
            LOG(ERR_LOG, "[IOC_GET_VER] copy_to fail");
            retval = -EFAULT;
        }
    ...
    case SIFP_IOC_SET_KMAP:
        ...
        if(copy_from_user(&fp_dev->keymap_cust.k, (void __user *)arg, sizeof(struct fp_dev_kmap_t))) {
            LOG(ERR_LOG, "[IOC_SET_KMAP] copy_from fail");
            retval = -EFAULT;
            break;
        }
    ...
    case SIFP_IOC_PKG_SIZE:
        if(arg) {
            LOG(DBG_LOG, "[IOC_PKG_SIZE] %d", (u8)(PKG_SIZE));
    ...
    case SIFP_IOC_DBG_LEVEL:
        if(arg) {
            ...
            LOG(ERR_LOG, "debug level %d-->%d", sil_debug_level, level);
    ...
    case SIFP_IOC_WAKELOCK:
        if(arg) {
            ...
            if(copy_from_user(&lock, (void __user *)arg, sizeof(char))) {
                LOG(ERR_LOG, "[SIFP_IOC_WAKELOCK] copy_from fail");
                retval = -EFAULT;
                break;
            }
    ...
    case SIFP_IOC_PWDN:
        if(arg) {
            ...
            if(copy_from_user(&flag_avdd, (void __user *)arg, sizeof(char))) {
                LOG(ERR_LOG, "[SIFP_IOC_PWDN] copy_from fail");
                retval = -EFAULT;
                break;
            }
    ...
    case SIFP_IOC_PROC_NODE:
        if(arg) {
            if(copy_from_user(vendor_name, (void __user *)arg, PROC_VND_ID_LEN)) {
                LOG(ERR_LOG, "[SIFP_IOC_PROC_NODE] copy_from fail");
                retval = -EFAULT;
                break;
            }
    ...
    case SIFP_IOC_SET_FEATURE:
        if(arg) {
            ...
            if(copy_from_user(&feature, (void __user *)arg, sizeof(char))) {
                LOG(ERR_LOG, "[SIFP_IOC_SET_FEATURE] copy_from fail");
                retval = -EFAULT;
                break;
            }
...
int  silfp_touch_event_handler(struct fp_dev_touch_info *tp_info)
{
    ...
    LOG(INFO_LOG, "tp_info %d, %d, %d, %d ", tp_info->touch_state, tp_info->area_rate, tp_info->x, tp_info->y);
...
static int silfp_open(struct inode *inode, struct file *filp)
{
    ...
    if(status == 0) {
        ...
        LOG(ERR_LOG, "silfp: nothing for minor %d", iminor(inode));
    }
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    if(status < 0) {
        LOG(ERR_LOG, "%s, request devt fail, ret=%d.", __func__, status);
        goto err_devt;
    } else {
        LOG(INFO_LOG, "%s,, major=%d, minor=%d", __func__, MAJOR(fp_dev->devt), MINOR(fp_dev->devt));
    }

    fp_dev->dev = device_create(silfp_class, &fp_dev->spi->dev, fp_dev->devt, fp_dev, FP_DEV_NAME);

    if(IS_ERR(fp_dev->dev)) {
        LOG(ERR_LOG, "%s, device_create() fail.", __func__);
        status = -ENODEV;
        goto err_dev;
    } else {
        mutex_lock(&device_list_lock);
        ...
        mutex_unlock(&device_list_lock);
        LOG(INFO_LOG, "%s, device create success.", __func__);
    }
    ...
    if(status) {
        LOG(ERR_LOG, "%s, cdev_add() fail, ret=%d.", __func__, status);
        goto err_cdev;
    }
    ...
    if(status) {
        LOG(ERR_LOG, "%s, silfp_init fail ret=%d.", __func__, status);
        goto err_cdev;
    }
...
static const struct of_device_id sildev_dt_ids[] = {
    ...
    { .compatible = "mediatek,silead-udfp" },
    {},
};
...
int silfp_dev_init(void)
{
    ...
    if(is_atboot == 1) {
        ...
        LOG(INFO_LOG, "%s, in AT mode, not load gsl7001 driver!", __func__);
        return 0;
    }
    ...
    if(get_fp_id() != SILEAD_GSL7001) {
        LOG(INFO_LOG, "%s, wrong gsl7001 id, exit", __func__);
        return 0;
    }
...
#endif
    LOG(INFO_LOG, "SILEAD_FP Driver, Version: %s.", FP_DEV_VERSION);
    ...
    if(status < 0) {
        ...
        unregister_chrdev(FP_DEV_MAJOR, silfp_driver.driver.name);
        LOG(ERR_LOG, "%s, spi_register_driver fail ret=%d.", __func__, status);
        return status;
    }
