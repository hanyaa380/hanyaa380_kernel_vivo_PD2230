...
#define PA1_RG_U2_INTR_MASK (0x1F)
#define PA1_RG_U2_INTR_OFST (19)
...
static int proc_phy_intr_show(struct seq_file *s, void *unused)
{
	struct mtk_phy_instance *instance = s->private;
	struct u2phy_banks *u2_banks = &instance->u2_banks;
	void __iomem *com = u2_banks->com;
	...
	tmp = readl(com + U3P_USBPHYACR1);
	tmp >>= PA1_RG_U2_INTR_OFST;
	tmp &= PA1_RG_U2_INTR_MASK;
...
static int proc_phy_intr_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_phy_intr_show, PDE_DATA(inode));
}

static ssize_t proc_phy_intr_write(struct file *file,
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct mtk_phy_instance *instance = s->private;
	...
	writel(tmp, com + U3P_USBPHYACR1);
	instance->eye_intr = val;
	instance->eye_intr_host = val;

	return count;
}

static const struct file_operations proc_phy_intr_fops = {
	.open = proc_phy_intr_open,
	.write = proc_phy_intr_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
			...
			struct mtk_phy_instance *instance)
{
	...
	file = proc_create_data(PROC_FILE_INTR, 0644,
			phy_root, &proc_phy_intr_fops, instance);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", PROC_FILE_INTR);
		ret = -ENOMEM;
		goto err1;
...
/* phy_rev6 should be set by the dts, not the code */
#if 0
	tmp = readl(com + U3P_USBPHYACR6);
	tmp &= ~PA6_RG_U2_PHY_REV6;
...
#define BOARD_VERSION_0   0  //default board version
#define BOARD_VERSION_1   1
#define ID_GPIO_MAX_LEN   20
static u32 phy_get_board_version(struct mtk_phy_instance *instance)
{
	struct device *dev = &instance->phy->dev;
	u32 board_version_bit = 0;
	char *version_state = NULL;

	device_property_read_u32(dev, "board-version-bit",
				 &board_version_bit);
	if ((board_version_bit > ID_GPIO_MAX_LEN) ||
		(board_version_bit < 1)) {
		dev_dbg(dev, "%s board_version_bit:%d\n", __func__, board_version_bit);
		return BOARD_VERSION_0;
	}

	dev_dbg(dev, "version_state=%s, board_version_bit=%d\n", version_state, board_version_bit);

	/*
	 * mediatek,eye-xxx-1 is the other board eye tune.
	 * if use this board eye tune, board-version-bit should
	 * be set in dts, and the version_state[board_version_bit-1] == '1'.
	 */
	if (version_state[board_version_bit - 1] == '1') {
		return BOARD_VERSION_1;
	}
				...
				struct mtk_phy_instance *instance)
{
	...
	if (instance->eye_inited)
		return;

	instance->bc12_en = device_property_read_bool(dev, "mediatek,bc12");

	board_version = phy_get_board_version(instance);
	if (board_version == BOARD_VERSION_1) {
		device_property_read_u32(dev, "mediatek,eye-src-1",
					 &instance->eye_src);
		device_property_read_u32(dev, "mediatek,eye-vrt-1",
					 &instance->eye_vrt);
		device_property_read_u32(dev, "mediatek,eye-term-1",
					 &instance->eye_term);
		device_property_read_u32(dev, "mediatek,eye-rev6-1",
					 &instance->eye_rev6);
		device_property_read_u32(dev, "mediatek,eye-intr-1",
					 &instance->eye_intr);
		device_property_read_u32(dev, "mediatek,eye-disc-1",
					 &instance->eye_disc);
		device_property_read_u32(dev, "mediatek,host-eye-src-1",
					 &instance->eye_src_host);
		device_property_read_u32(dev, "mediatek,host-eye-vrt-1",
					 &instance->eye_vrt_host);
		device_property_read_u32(dev, "mediatek,host-eye-term-1",
					 &instance->eye_term_host);
		device_property_read_u32(dev, "mediatek,host-eye-rev6-1",
					 &instance->eye_rev6_host);
		device_property_read_u32(dev, "mediatek,host-eye-intr-1",
					 &instance->eye_intr_host);
		device_property_read_u32(dev, "mediatek,host-eye-disc-1",
					 &instance->eye_disc_host);
	} else {
		...
		device_property_read_u32(dev, "mediatek,eye-rev6",
					 &instance->eye_rev6);
		device_property_read_u32(dev, "mediatek,eye-intr",
					 &instance->eye_intr);
		...
		device_property_read_u32(dev, "mediatek,host-eye-rev6",
					 &instance->eye_rev6_host);
		device_property_read_u32(dev, "mediatek,host-eye-intr",
					 &instance->eye_intr_host);
			     ...
			     struct mtk_phy_instance *instance)
{
	...
	if (instance->eye_intr) {
		tmp = readl(com + U3P_USBPHYACR1);
		tmp &= ~PA1_RG_U2_INTR_CAL;
		tmp |= PA1_RG_U2_INTR_CAL_VAL(instance->eye_intr);
		writel(tmp, com + U3P_USBPHYACR1);
	}
			     ...
			     struct mtk_phy_instance *instance)
{
	...
	dev_info(dev, "%s,host src:%d, vrt:%d, term:%d, rev6:%d, dis:%d",
		__func__, instance->eye_src_host, instance->eye_vrt_host,
		instance->eye_term_host, instance->eye_rev6_host,
		instance->eye_disc_host);
	...
	if (instance->eye_rev6_host) {
		tmp = readl(com + U3P_USBPHYACR6);
		tmp &= ~PA6_RG_U2_PHY_REV6;
		tmp |= PA6_RG_U2_PHY_REV6_VAL(instance->eye_rev6_host);
		writel(tmp, com + U3P_USBPHYACR6);
	}

	if (instance->eye_intr_host) {
		tmp = readl(com + U3P_USBPHYACR1);
		tmp &= ~PA1_RG_U2_INTR_CAL;
		tmp |= PA1_RG_U2_INTR_CAL_VAL(instance->eye_intr_host);
		writel(tmp, com + U3P_USBPHYACR1);
	}

	if (instance->eye_disc_host) {
		tmp = readl(com + U3P_USBPHYACR6);
		tmp &= ~PA6_RG_U2_DISCTH;
		tmp |= PA6_RG_U2_DISCTH_VAL(instance->eye_disc_host);
		writel(tmp, com + U3P_USBPHYACR6);
	}
