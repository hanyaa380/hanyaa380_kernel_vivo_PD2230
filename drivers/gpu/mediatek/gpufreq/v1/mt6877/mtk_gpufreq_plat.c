...
static bool g_force_downgrade_mt6877t;
...
static unsigned int __mt_gpufreq_get_segment_id(void)
{
	...
	if (g_force_downgrade_mt6877t) {
		segment_id = MT6877_SEGMENT;    /* 5G-5(cust) */
		gpufreq_pr_info("@%s: Force cust segment 5G-5\n",
			__func__);
	}
...
static int __mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	struct device_node *force_downgrade_mt6877t_node;
	struct device_node *node;
	int ret;
	int gpio_num = 0;
	int dg_gpio_value = -1;
	...
	/* Get FORCE_DOWNGRADE_MT6877T config */
	force_downgrade_mt6877t_node = of_find_node_by_name(
			NULL, "FORCE_DOWNGRADE_MT6877T_ON");
	if (!force_downgrade_mt6877t_node) {
		g_force_downgrade_mt6877t = false;
	}
	else {
		g_force_downgrade_mt6877t = true;
		gpio_num = of_get_named_gpio(force_downgrade_mt6877t_node, "vivo,diff-6190m-6190w", 0x00);
		if (!gpio_is_valid(gpio_num)) {
			g_force_downgrade_mt6877t = true;
		} else {
			dg_gpio_value = gpio_get_value(gpio_num);
			gpufreq_pr_info("@%s gpu if downgrade_mt6877t gpio num: %d and gpio val: %d\n",
			__func__, gpio_num, dg_gpio_value);
			if (dg_gpio_value == 0) {
				g_force_downgrade_mt6877t = false;
			} else if (dg_gpio_value == 1) {
				g_force_downgrade_mt6877t = true;
			} else {
				g_force_downgrade_mt6877t = true;
			}
	...
	gpufreq_pr_info("@%s g_force_downgrade_mt6877t %d\n",
			__func__, g_force_downgrade_mt6877t);
