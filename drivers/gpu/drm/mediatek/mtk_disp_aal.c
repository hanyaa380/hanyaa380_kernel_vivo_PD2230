...
void disp_aal_notify_backlight_changed(int trans_backlight, bool cust_flag)
{
	unsigned long flags;
	unsigned int service_flags;
	int prev_backlight;

	g_aal_cust_flag = cust_flag;
	AALAPI_LOG("%d/%d,cust_flag=%d\n", trans_backlight, g_max_backlight, cust_flag);
...
#if defined(CONFIG_DRM_PANEL_PD2204_RM692E5_FHDP_CMD_BOE) ||  defined(CONFIG_DRM_PANEL_PD2204_EA8079P_FHDP_CMD_SAMSUNG)
static int led_set_trans_level(int level, void *v)
{
	int trans_level;
	struct led_conf_info *led_conf;

	led_conf = (struct led_conf_info *)v;
	if (lcm_board_id == 0) {	//EA8079P
		trans_level = (
			(((1 << led_conf->trans_bits) - 1)
			* level + ((led_conf->cdev.max_brightness) / 2))
	...
	pr_info("leds trans level %d , max_level %d /n", trans_level, led_conf->max_level);
	return trans_level;
}
#endif
	...
	void *v)
{
	int trans_level;
	struct led_conf_info *led_conf;
	bool cust_flag = false;

	led_conf = (struct led_conf_info *)v;

	cust_flag = true;

	AALAPI_LOG("cust_flag=%d\n", cust_flag);

	#if defined(CONFIG_DRM_PANEL_PD2204_RM692E5_FHDP_CMD_BOE) ||  defined(CONFIG_DRM_PANEL_PD2204_EA8079P_FHDP_CMD_SAMSUNG)
		if (lcm_board_id == 0) {	//EA8079P
			g_max_backlight = (1 << led_conf->trans_bits) - 1;
		} else {				//RM692E5
			g_max_backlight = 3515;
		}
	#else
		g_max_backlight = (1 << led_conf->trans_bits) - 1;
	#endif
	...
	case 1:
		#if defined(CONFIG_DRM_PANEL_PD2204_RM692E5_FHDP_CMD_BOE) ||  defined(CONFIG_DRM_PANEL_PD2204_EA8079P_FHDP_CMD_SAMSUNG)
			trans_level = led_set_trans_level(led_conf->cdev.brightness, v);
		#else
			trans_level = (
				(((1 << led_conf->trans_bits) - 1)
		...
		disp_aal_notify_backlight_changed(trans_level, cust_flag);
		AALAPI_LOG("brightness changed: %d(%d)\n",
			trans_level, led_conf->cdev.brightness);
		break;
	case 2:
		disp_aal_notify_backlight_changed(0, cust_flag);
		break;
	case 3:
		#if defined(CONFIG_DRM_PANEL_PD2204_RM692E5_FHDP_CMD_BOE) ||  defined(CONFIG_DRM_PANEL_PD2204_EA8079P_FHDP_CMD_SAMSUNG)
			trans_level = led_set_trans_level(led_conf->max_level, v);
		#else
			trans_level = (
				(((1 << led_conf->trans_bits) - 1)
				* led_conf->max_level
	...
	if (spin_trylock_irqsave(&g_aal_hist_lock, flags)) {
		...
		if (result) {
			...
			if (atomic_read(&g_aal_first_frame) == 1 &&
				atomic_read(&g_aal_interrupt_enabled) == 1) {
	...
	struct cmdq_pkt *handle)
{
	...
	if (atomic_read(&g_aal_force_relay) != bypass) {
		...
		if (comp->mtk_crtc->is_dual_pipe) {
			...
			if (bypass == 0)
				cmdq_pkt_write(handle, comp_aal1->cmdq_base,
					comp_aal1->regs_pa + DISP_AAL_CFG, 0x3 << 1, (0x3 << 1));
		}
...
void disp_aal_on_start_of_frame(void)
{
	...
	if (isDualPQ) {
		...
		if (atomic_read(&g_aal_first_frame) == 1 &&
				atomic_read(&g_aal1_first_frame) == 1 &&
				atomic_read(&g_aal_interrupt_enabled) == 1) {
			AALIRQ_LOG("dual pipe first_flip queue_work");
			queue_work(aal_flip_wq, &g_aal_data->aal_flip_task);
			AALIRQ_LOG("end first_flip queue_work");
		}
		AALIRQ_LOG("g_aal_interrupt_enabled[%d]", atomic_read(&g_aal_interrupt_enabled));
	}
#endif
}
...
static int mtk_disp_aal_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(81)) {      //gpio 52 parallelism gpio number 81
		lcm_board_id = gpio_get_value(81);
		if (lcm_board_id < 0)
			pr_err("lcm get  board_id valuel fail, ret = %d\n", lcm_board_id);
		else
			pr_info("lcm get  board_id valuel %d\n", lcm_board_id);
	}
#endif
