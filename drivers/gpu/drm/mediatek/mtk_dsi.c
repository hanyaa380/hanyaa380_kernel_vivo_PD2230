...
static int mtk_dsi_set_data_rate(struct mtk_dsi *dsi)
{
	...
	DDPMSG("%s, data_rate: %d MHz, mipi_tx_rate: %lu Hz\n",
...
#else
#if defined(CONFIG_DRM_PANEL_PD2257_SAMSUNG_S6E8FC3_FHDP_VIDEO) || defined(CONFIG_DRM_PANEL_PD2282_BOE_NT37705_FHDP_CMD)
	if (handle && (handle->cl == (void *)client)) {
		if(handle -> sec_data) {
			DDPINFO("%s sec cmdq handle \n", __func__);
			cmdq_pkt_poll(handle, NULL, val, reg, mask, CMDQ_GPR_R14);
		} else {
		...
		if(handle -> sec_data) {
			DDPINFO("%s sec cmdq handle \n", __func__);
			cmdq_pkt_poll(handle, NULL, val, reg, mask, CMDQ_GPR_R07);
		} else {
			cmdq_pkt_poll_timeout(handle, val, SUBSYS_NO_SUPPORT, reg, mask, 0xFFFF, CMDQ_GPR_R07);
		}
	}
#else
	if (handle && (handle->cl == (void *)client)) {
...
static int mtk_notify_fingerprint_status(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (mtk_crtc->fingershooter_status == SHOOTER_SHOW_BIT) {
		if (hbm_on_te_count >= 3 && shooter_count >= 1)
			need_notify = true;
	} else {
		if (shooter_count >= 1)
			need_notify = true;
...
static irqreturn_t mtk_dsi_irq_status(int irq, void *dev_id)
{
	...
	|| defined(CONFIG_MACH_MT6781)
			...
			err_num_count++;
			/*err cout more than 5 ,and report error code 1001*/
			if (err_num_count > 5) {
				lcm_error_code_report(1001);
				err_num_count = 0;
			}
		}
		...
		if (status & FRAME_DONE_INT_FLAG) {
			...
			mtk_crtc = dsi->ddp_comp.mtk_crtc;
#if defined(CONFIG_VIVO_SMART_AOD_PANEL) || defined(CONFIG_DRM_PANEL_PD2257_SAMSUNG_S6E8FC3_FHDP_VIDEO)
			mtk_notify_fingerprint_status(mtk_crtc);
#endif
			if (mtk_crtc) {
	...
	int force_lcm_update)
{
	...
	if (ext && ext->funcs && ext->funcs->dsi_panel_reset_and_powerctl)
	...
	int force_lcm_update)
{
	...
	if (dsi->panel && (!new_doze_state || force_lcm_update)) {
#if defined(CONFIG_VIVO_SMART_AOD_PANEL)
		if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->suspend_by_hs_mode) {
			...
			LCD_INFO("panel_suspend_by_hs_mode function is NULL\n");
		}
#endif

#ifdef CONFIG_DRM_PANEL_PD2257_SAMSUNG_S6E8FC3_FHDP_VIDEO
		vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SUSPEND_SETTING, 0);
		mdelay(160);
#endif
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (fps_chg_index & DYNFPS_DSI_MIPI_CLK) {
...
#ifdef CONFIG_VIVO_LCM_SWITCH_PORCH
		...
		vivo_panel_dfps_switch(fps_src, fps_dst);
#else
		mtk_dsi_porch_setting(comp, handle, DSI_HFP, dsi->hfp_byte);
