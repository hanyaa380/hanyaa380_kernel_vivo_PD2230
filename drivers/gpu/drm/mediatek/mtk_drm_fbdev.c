...
#else
	struct device_node *chosen_node;
	struct vivo_display *vdd = get_vivo_vdisp();
	...
	if (chosen_node) {
		...
		if (videolfb_tag) {
			...
			*fps = videolfb_tag->fps;
			...
			vdd->islcmfound = videolfb_tag->islcmfound;
			vdd->fps = videolfb_tag->fps;
			vdd->lcm_software_id = videolfb_tag->lcm_software_id;
			vdd->lcm_especial_id = videolfb_tag->lcm_especial_id;
			vdd->silent_reboot = videolfb_tag->silent_reboot;
			vdd->bl_hw_version = videolfb_tag->bl_hw_version;
			vdd->lcm_imei_str[0] = videolfb_tag->lcm_imei_str[0];
			vdd->lcm_imei_str[1] = videolfb_tag->lcm_imei_str[1];
			vdd->lcm_imei_str[2] = videolfb_tag->lcm_imei_str[2];
			vdd->lcm_imei_str[3] = videolfb_tag->lcm_imei_str[3];
