...
static struct mtk_drm_property mtk_crtc_property[CRTC_PROP_MAX] = {
	...
	{DRM_MODE_PROP_ATOMIC, "HDR_BRIGHTNESS_ENABLE", 0, UINT_MAX, 0},
...
#ifdef CONFIG_DRM_PANEL_PD2257_SAMSUNG_S6E8FC3_FHDP_VIDEO
...
static int mtk_drm_hdr_setbacklight(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle, unsigned int level)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);

	if (comp && comp->funcs && comp->funcs->io_cmd)
		comp->funcs->io_cmd(comp, cmdq_handle, DSI_SET_HDR_FLAG, &level);
...
int mtk_drm_crtc_set_panel_hbm(struct drm_crtc *crtc, int en)
{
	...
	if (state == -1)
		...
		&& !((en == PANEL_HBM_UD_OFF) && (vdisp->ud_hbm_sync.ud_flag > 0))		
		/*flag change from 1 ==> 0 need to set hbm*/
		&& !((en == PANEL_HBM_UD_OFF) && (atomic_read(&vdisp->oled_hbm_state) == PANEL_HBM_AOD_UD_ON))) {
		/*AOD turn on hbm and have set brightness once, then unlock success, need to turn off hbm*/
		return -EINVAL;
	...
	if (en == PANEL_HBM_UD_ON)
 		vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SET_DIMING_OFF, 0);
...
static void mtk_crtc_cmdq_timeout_cb(struct cmdq_cb_data data)
{
	...
	static unsigned int err_num_count;
	dma_addr_t trig_pc;
	u64 *inst;
	...
	err_num_count++;
	/*err cout more than 5 ,and report error code 1000*/
	if (err_num_count > 5) {
...
/*	
		mtk_drm_crtc_set_panel_hbm(crtc, ud_flag);
		mtk_drm_crtc_hbm_wait(crtc, ud_flag);
*/
		//if (!state->prop_val[CRTC_PROP_DOZE_ACTIVE])
		//	mtk_atomic_hbm_bypass_pq(crtc, cmdq_handle, ud_flag);
		...
		if (ud_flag & UD_FLAG_DC_DIMMING_MASK) {
		...
		} else if (!vdisp->local_hbm.support) {
			...
			if (state->shooter_flag != 0) {
				...
				if (/* TODO: need_update_fpd_idx && */pf_idx != (unsigned int)-1) {
					...
					LCD_INFO("%s: write pf_idx to pf0:%u\n",
						  __func__, pf_idx);
					cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
						       addr, pf_idx, ~0);
				}
	...
	/*at aod mode to set smart aod area, when flag is true*/
	if ((vdisp->smart_aod_update) && (atomic_read(&vdisp->doze_mode_state) != LCD_PANEL_ON)) {
		vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SET_AOD_AREA, 0);
		vdisp->smart_aod_update = false;
		LCD_INFO("set smart aod area.\n");
	}
