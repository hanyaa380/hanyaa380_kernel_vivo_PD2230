...
unsigned int phone_shutdown_state;
static void mtk_drm_shutdown(struct platform_device *pdev)
{
	...
	if (!strcmp(vdisp->panel_name, "panel_pd2230_hx_ft8720p_fhdplus_vdo")) {
		DDPMSG("%s\n", __func__);
	} else {
		mdss_dsi_panel_reset_and_powerctl(POWER_CTRL_DISP_PANEL_POWER_SUHTDOWN_SEQ);
	}
