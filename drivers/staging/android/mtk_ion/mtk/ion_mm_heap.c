...
#ifdef CONFIG_PM
static DEFINE_RATELIMIT_STATE(ion_heap_mem_pm_log, 5 * HZ, 2);
void shrink_ion_by_scenario(int need_lock)
{
	...
	if (__ratelimit(&ion_heap_mem_pm_log))
		printk("%s %d <%d,%s><%d,%s> stop to shrink the ion memory!!",
			__func__, __LINE__, current->pid, current->comm,
			current->group_leader ? current->group_leader->pid : 0,
			current->group_leader ? current->group_leader->comm : "unknow");
	return;
