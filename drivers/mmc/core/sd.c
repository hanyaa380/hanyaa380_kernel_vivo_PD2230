...
static void mmc_sd_detect(struct mmc_host *host)
{
	...
	if (!retries && !fast_power_off) {
		...
		if(host->ops->get_reset_num && (host->ops->get_reset_num(host) > 3)){
			pr_err("%s: reset_num > 3, remove SD\n", mmc_hostname(host));
			mmc_card_set_removed(host->card);
			err = -1;
		} else
			err = _mmc_detect_card_removed(host);
...
static int mmc_sd_runtime_suspend(struct mmc_host *host)
{
	...
	pr_err("%s: doing aggressive suspend, caps: 0x%x\n",
...
err:
	...
	pr_err("%s: error %d whilst initialising SD card, set vivo bad_card=1\n",
		mmc_hostname(host), err);
	if (err != -EAGAIN && host->ops->remove_bad_sdcard) {
		pr_err("%s: set bad card and remove it\n", mmc_hostname(host));
