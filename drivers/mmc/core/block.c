...
static void mmc_blk_issue_discard_rq(struct mmc_queue *mq, struct request *req)
{
	...
	do {
		...
		if (!err) {
			...
			if (((end_time - start_time) > 500 * 1000 * 1000)) {
				__u64 temp_time = sched_clock();
				pr_notice("mmc over 500ms discard nr:%d busy time:%llu\n",
					nr, end_time - start_time);
				if (mmc_host_g && (temp_time - msdc_dump_time > 1000 * 1000 * 1000)) {
...
static void mmc_blk_issue_flush(struct mmc_queue *mq, struct request *req)
{
	...
	if (((end_time - start_time) > 500 * 1000 * 1000)) {
		__u64 temp_time = sched_clock();
		pr_notice("mmc over 500ms flush cache busy time:%llu\n", end_time - start_time);
		if (mmc_host_g && (temp_time - msdc_dump_time > 1000 * 1000 * 1000)) {
			mmc_cmd_dump(NULL, NULL, NULL, mmc_host_g, 100);
			msdc_dump_time = sched_clock();
		}
...
static void mmc_blk_mq_rw_recovery(struct mmc_queue *mq, struct request *req)
{
	...
	if (!mmc_host_is_spi(mq->card->host) &&
		...
		/*when recovery failed,remove bad card*/
/*
