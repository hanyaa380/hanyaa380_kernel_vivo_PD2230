...
static __u32 global_write_size;
static __u32 global_read_size;
static __u32 temp_write_size;
static __u32 temp_read_size;
static __u64 temp_write_time;
static __u64 temp_read_time;
#ifndef MS
...
#ifdef CONFIG_MMC_MTK_PRO
extern struct mmc_host *mmc_host_g;
extern void mmc_cmd_dump(char **buff, unsigned long *size, struct seq_file *m,
				struct mmc_host *mmc, u32 latest_cnt);
static __u64 msdc_dump_time;
#endif
...
#ifdef CONFIG_MMC_MTK_PRO
	if (temp_write_time == 0 && temp_read_time == 0) {
		temp_write_time = sched_clock();
		temp_read_time = sched_clock();
	}

	rw = tsk->arg & (1 << 30);  /* write: 0, read: 1 */
	bytes = (tsk->arg & 0xFFFF) << SECTOR_SHIFT;
	dma_time = tsk->t[tsk_dma_end] - tsk->t[tsk_dma_start];
	queue_dma_time = tsk->t[tsk_dma_end] - tsk->t[tsk_req_start];

	if (rw == 0) { /* write */
		global_write_size += (bytes/4096);
	} else if (rw == 1) { /* read */
		global_read_size += (bytes/4096);
	}
	if (global_write_size - temp_write_size >= 25600) {
		pr_notice("mmc dma write over 100MB consuming time %llu ms\n", MS(sched_clock() - temp_write_time));
		temp_write_time = sched_clock();
		temp_write_size = global_write_size;
	}
	if (global_read_size - temp_read_size >= 25600) {
		pr_notice("mmc dma read over 100MB consuming time %llu ms\n", MS(sched_clock() - temp_read_time));
		temp_read_time = sched_clock();
		temp_read_size = global_read_size;
	}

	if ((queue_dma_time > 500 * 1000 * 1000)) {
		pr_notice("mmc dma over 500ms tag:%d, rw:%d size:%d dma time:%llu "
			"queue_dma_time:%llu w_size:%d r_size:%d\n",
			tsk->task_id, rw, bytes, dma_time, queue_dma_time, global_write_size, global_read_size);
		if (mmc_host_g) {
			__u64 temp_time = sched_clock();
			if (temp_time - msdc_dump_time > (__u64)(1000 * 1000 * 1000)) {
				mmc_cmd_dump(NULL, NULL, NULL, mmc_host_g, 100);
				//pr_notice("temp_time: %ull msdc_dump_time: %ull", temp_time, msdc_dump_time);
				msdc_dump_time = sched_clock();
			}
		}
	}
#endif
...
void mt_biolog_cqhci_complete(unsigned int task_id)
{
	int write;
	__u32 bytes;
	__u64 end_time, busy_time, queue_isdone_time;
	struct mt_bio_context *ctx;
	struct mt_bio_context_task *tsk;
	struct mtk_btag_throughput_rw *tp;
	unsigned long flags;
	...
	queue_isdone_time = tsk->t[tsk_isdone_end] - tsk->t[tsk_req_start];
	if ((queue_isdone_time > 500 * 1000 * 1000) && bytes)
		pr_notice("mmc queue over 500ms tag:%d, rw:%d size:%d busy_time:%llu queue_isdone_time:%llu\n",
			task_id, write, bytes, busy_time, queue_isdone_time);
