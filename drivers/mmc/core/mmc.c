...
static int mmc_decode_cid(struct mmc_card *card)
{
	...
	case 4: /* MMC v4 */
		...
		card->cid.month		= UNSTUFF_BITS(resp, 12, 4);
		card->cid.year		= UNSTUFF_BITS(resp, 8, 4) + 1997;
		mmc_prod_name_g[0]	= card->cid.prod_name[0];
		mmc_prod_name_g[1]	= card->cid.prod_name[1];
		mmc_prod_name_g[2]	= card->cid.prod_name[2];
		mmc_prod_name_g[3]	= card->cid.prod_name[3];
		mmc_prod_name_g[4]	= card->cid.prod_name[4];
		mmc_prod_name_g[5]	= card->cid.prod_name[5];
		break;
...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->cid.manfid == CID_MANFID_MICRON) { //add by hezerui for modify device version bit
	...
	if (card->ext_csd.generic_cmd6_time < DEFAULT_CMD6_TIMEOUT_MS)
		card->ext_csd.generic_cmd6_time = DEFAULT_CMD6_TIMEOUT_MS;
	/* eMMC v5 or later */
...
static void mmc_select_driver_type(struct mmc_card *card)
{
	...
	if (card->cid.manfid == 0x9b) /* YMTC use 40Ω */
		drive_strength = 0x4;
	if (is_PD2281F_EX) {
		if (card->cid.manfid == 0x9b)
			drive_strength = 0x1;
...
struct mmc_host *mmc_host_g;
unsigned int mmc_manfid_g;
