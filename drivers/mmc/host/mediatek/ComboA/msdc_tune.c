...
int sdcard_hw_reset(struct mmc_host *mmc)
{
	...
	if (ret) {
		...
		host->reset_num++;
		if(host->reset_num == 1){
			host->reset_start_time = ktime_get_boot_ns();
		}
		pr_notice("msdc%d power reset success, reset_num=%d\n", host->id, host->reset_num);
		if(host->reset_num > 3){
			reset_end_time = ktime_get_boot_ns();
			if((reset_end_time - host->reset_start_time) < 105000000000){//90s+15s
				host->reset_start_time = 0;
				pr_notice("since reset time over 3 times, set bad card!\n");
				msdc_set_bad_card_and_remove(host);
				ret = -1;
				return ret;
			}
