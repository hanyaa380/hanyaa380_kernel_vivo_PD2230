...
int mtkts_bts_get_hw_temp_ch4(void)
{
	int t_ret = 0;
	int t_ret2 = 0;
...
#if defined(CONFIG_PD2230_BOARD_TEMP)
        t_ret = t_ret - 3000;
#endif
...
static void mtkts_bts_prepare_table(int table_num)
{
...
static int mtkts_bts_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktschargeric", num_trip, NULL,
...
static int mtkts_bts_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		mtkts_bts_printk("[%s] fail to get auxadc iio ch4: %d\n",
			__func__, ret);
		return ret;
	}
	...
	if (!mtkts_AP_dir) {
		...
		entry = proc_create("btsch4", 0664, mtkts_AP_dir,
							&mtkts_AP_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

	entry = proc_create("btsch4_param", 0664, mtkts_AP_dir,
				&mtkts_AP_param_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
