...
static int mtkts_flight_debug_log;
...
#define mtkts_flight_dprintk(fmt, args...)   \
do {                                    \
	if (mtkts_flight_debug_log) {                \
		pr_debug("[Thermal/TZ/BTSFLIGHT]" fmt, ##args); \
	}                                   \
} while (0)

#define mtkts_flight_printk(fmt, args...) \
pr_notice("[Thermal/TZ/BTSFLIGHT]" fmt, ##args)
...
static int g_RAP_ADC_channel = AUX_IN3_NTC;

static int g_flight_TemperatureR;
...
static __s32 mtkts_flight_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	...
	if (Res >= BTSNRPA_Temperature_Table[0].TemperatureR) {
		...
		/* mtkts_flight_dprintk("%d : RES1 = %d,TMP1 = %d\n",__LINE__,
		 * RES1,TMP1);
		 */
		...
		for (i = 0; i < asize; i++) {
			if (Res >= BTSNRPA_Temperature_Table[i].TemperatureR) {
				...
				TMP2 = BTSNRPA_Temperature_Table[i]
					.BTSNRPA_Temp;
				/* mtkts_flight_dprintk("%d :i=%d, RES2 = %d,
			...
			/* mtkts_flight_dprintk("%d :i=%d, RES1 = %d,
...
static __s32 mtk_ts_flight_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__u64 dwVCriAP2 = 0;
	...
	g_flight_TemperatureR = TRes;

	/* convert register to temperature */
	BTSNRPA_TMP = mtkts_flight_thermistor_conver_temp(TRes);

	return BTSNRPA_TMP;
}

static int get_hw_flight_temp(void)
{
#if defined(CONFIG_MEDIATEK_MT6577_AUXADC)
	int val = 0;
	int ret = 0, output;
	...
	mtkts_flight_dprintk("%s val=%d\n", __func__, val);

	if (ret < 0) {
		mtkts_flight_printk("IIO channel read failed %d\n", ret);
		return ret;
	}
	...
	mtkts_flight_dprintk("APtery output mV = %d\n", ret);
	output = mtk_ts_flight_volt_to_temp(ret);
	mtkts_flight_dprintk("BTSNRPA output temperature = %d\n", output);
	return output;
}

static DEFINE_MUTEX(BTSFLIGHT_lock);
/*int ts_flight_at_boot_time = 0;*/
int mtkts_flight_get_hw_temp(void)
{
	int t_ret = 0;
...
#define DELTA_TEMP 2000
#endif
	mutex_lock(&BTSFLIGHT_lock);

	/* get HW AP temp (TSAP) */
	/* cat /sys/class/power_supply/AP/AP_temp */
	t_ret = get_hw_flight_temp();
	...
	if ((t_ret > 100000) || (t_ret < -30000)) {
		...
		pre_temp1 = t_ret;
		mtkts_flight_dprintk("[Power/BTSNRPA_Thermal] pre_temp1=%d\n",
								pre_temp1);
	}
#endif

	mutex_unlock(&BTSFLIGHT_lock);

	if (t_ret > 38000)	/* abnormal high temp */
		mtkts_flight_printk("T_btsflight=%d\n", t_ret);

	mtkts_flight_dprintk("[%s] T_btsflight, %d\n", __func__,
									t_ret);
	return t_ret;
}

static int mtkts_flight_get_temp(struct thermal_zone_device *thermal, int *t)
{
	*t = mtkts_flight_get_hw_temp();

	if ((int)*t > 52000)
		mtkts_flight_dprintk("T=%d\n", (int)*t);
...
static int mtkts_flight_bind(struct thermal_zone_device *thermal,
			      struct thermal_cooling_device *cdev)
{
	...
	mtkts_flight_dprintk("[%s] binding OK, %d\n", __func__,
								table_val);
	return 0;
}

static int mtkts_flight_unbind(struct thermal_zone_device *thermal,
				struct thermal_cooling_device *cdev)
{
	int table_val = 0;
	...
	mtkts_flight_dprintk("[%s] unbinding OK\n", __func__);
	return 0;
}

static int mtkts_flight_get_mode(struct thermal_zone_device *thermal,
...
static int mtkts_flight_set_mode(struct thermal_zone_device *thermal,
				  enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
...
static int mtkts_flight_get_trip_type(
...
static int mtkts_flight_get_trip_temp(
...
static int mtkts_flight_get_crit_temp(
...
static struct thermal_zone_device_ops mtkts_flight_dev_ops = {
	.bind = mtkts_flight_bind,
	.unbind = mtkts_flight_unbind,
	.get_temp = mtkts_flight_get_temp,
	.get_mode = mtkts_flight_get_mode,
	.set_mode = mtkts_flight_set_mode,
	.get_trip_type = mtkts_flight_get_trip_type,
	.get_trip_temp = mtkts_flight_get_trip_temp,
	.get_crit_temp = mtkts_flight_get_crit_temp,
};



static int mtkts_flight_read(struct seq_file *m, void *v)
{
...
static int mtkts_flight_register_thermal(void);
static void mtkts_flight_unregister_thermal(void);

static ssize_t mtkts_flight_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	struct btsnrpa_data *ptr_flight_data = kmalloc(
					sizeof(*ptr_flight_data), GFP_KERNEL);

	if (ptr_flight_data == NULL)
		return -ENOMEM;

	len = (count < (sizeof(ptr_flight_data->desc) - 1)) ?
				count : (sizeof(ptr_flight_data->desc) - 1);

	if (copy_from_user(ptr_flight_data->desc, buffer, len)) {
		kfree(ptr_flight_data);
		return 0;
	}

	ptr_flight_data->desc[len] = '\0';

	if (sscanf
	    (ptr_flight_data->desc,
	     "%d %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d",
		&num_trip,
		&ptr_flight_data->trip[0], &ptr_flight_data->t_type[0],
		ptr_flight_data->bind0,
		&ptr_flight_data->trip[1], &ptr_flight_data->t_type[1],
		ptr_flight_data->bind1,
		&ptr_flight_data->trip[2], &ptr_flight_data->t_type[2],
		ptr_flight_data->bind2,
		&ptr_flight_data->trip[3], &ptr_flight_data->t_type[3],
		ptr_flight_data->bind3,
		&ptr_flight_data->trip[4], &ptr_flight_data->t_type[4],
		ptr_flight_data->bind4,
		&ptr_flight_data->trip[5], &ptr_flight_data->t_type[5],
		ptr_flight_data->bind5,
		&ptr_flight_data->trip[6], &ptr_flight_data->t_type[6],
		ptr_flight_data->bind6,
		&ptr_flight_data->trip[7], &ptr_flight_data->t_type[7],
		ptr_flight_data->bind7,
		&ptr_flight_data->trip[8], &ptr_flight_data->t_type[8],
		ptr_flight_data->bind8,
		&ptr_flight_data->trip[9], &ptr_flight_data->t_type[9],
		ptr_flight_data->bind9,
		&ptr_flight_data->time_msec) == 32) {

		down(&sem_mutex);
		mtkts_flight_dprintk(
			"[%s] mtkts_flight_unregister_thermal\n", __func__);

		mtkts_flight_unregister_thermal();

		if (num_trip < 0 || num_trip > 10) {
			#ifdef CONFIG_MTK_AEE_FEATURE
			aee_kernel_warning_api(__FILE__, __LINE__,
					DB_OPT_DEFAULT, "mtkts_flight_write",
					"Bad argument");
			#endif
			mtkts_flight_dprintk(
				"[%s] bad argument\n", __func__);
		...
		for (i = 0; i < num_trip; i++)
			g_THERMAL_TRIP[i] = ptr_flight_data->t_type[i];
		...
		for (i = 0; i < 20; i++) {
			g_bind0[i] = ptr_flight_data->bind0[i];
			g_bind1[i] = ptr_flight_data->bind1[i];
			g_bind2[i] = ptr_flight_data->bind2[i];
			g_bind3[i] = ptr_flight_data->bind3[i];
			g_bind4[i] = ptr_flight_data->bind4[i];
			g_bind5[i] = ptr_flight_data->bind5[i];
			g_bind6[i] = ptr_flight_data->bind6[i];
			g_bind7[i] = ptr_flight_data->bind7[i];
			g_bind8[i] = ptr_flight_data->bind8[i];
			g_bind9[i] = ptr_flight_data->bind9[i];
		}
		...
		for (i = 0; i < num_trip; i++)
			trip_temp[i] = ptr_flight_data->trip[i];

		interval = ptr_flight_data->time_msec / 1000;
		...
		mtkts_flight_dprintk(
			"[%s] mtkts_flight_register_thermal\n", __func__);
		mtkts_flight_register_thermal();
		up(&sem_mutex);
	...
	mtkts_flight_dprintk("[%s] bad argument\n", __func__);
	#ifdef CONFIG_MTK_AEE_FEATURE
	aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"mtkts_flight_write", "Bad argument");
	#endif
	kfree(ptr_flight_data);
	return -EINVAL;
}

void mtkts_flight_prepare_table(int table_num)
{
...
static int mtkts_flight_param_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", g_RAP_pull_up_R);
...
static ssize_t mtkts_flight_param_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	struct param_data {
	...
	if (sscanf
		...
		if (!strcmp(ptr_param_data->pull_R, "PUP_R")) {
			...
			mtkts_flight_dprintk("g_RAP_pull_up_R=%d\n",
							g_RAP_pull_up_R);
		} else {
			kfree(ptr_param_data);
			mtkts_flight_printk(
					"[mtkts_flight_write] bad PUP_R argument\n");
			return -EINVAL;
		}

		if (!strcmp(ptr_param_data->pull_V, "PUP_VOLT")) {
			g_RAP_pull_up_voltage = ptr_param_data->valV;
			mtkts_flight_dprintk("g_Rat_pull_up_voltage=%d\n",
							g_RAP_pull_up_voltage);
		} else {
			kfree(ptr_param_data);
			mtkts_flight_printk(
					"[mtkts_flight_write] bad PUP_VOLT argument\n");
			return -EINVAL;
		}

		if (!strcmp(ptr_param_data->overcrilow, "OVER_CRITICAL_L")) {
			g_TAP_over_critical_low = ptr_param_data->over_cri_low;
			mtkts_flight_dprintk("g_TAP_over_critical_low=%d\n",
					      g_TAP_over_critical_low);
		} else {
			kfree(ptr_param_data);
			mtkts_flight_printk(
					"[mtkts_flight_write] bad OVERCRIT_L argument\n");
			return -EINVAL;
		}

		if (!strcmp(ptr_param_data->NTC_TABLE, "NTC_TABLE")) {
			g_RAP_ntc_table = ptr_param_data->ntc_table;
			mtkts_flight_dprintk("g_RAP_ntc_table=%d\n",
							g_RAP_ntc_table);
		} else {
			kfree(ptr_param_data);
			mtkts_flight_printk(
					"[mtkts_flight_write] bad NTC_TABLE argument\n");
			return -EINVAL;
		}
		...
		mtkts_flight_dprintk("adc_channel=%d\n", adc_channel);
		mtkts_flight_dprintk("g_RAP_ADC_channel=%d\n",
						g_RAP_ADC_channel);

		mtkts_flight_prepare_table(g_RAP_ntc_table);

		kfree(ptr_param_data);
		return count;
	}

	mtkts_flight_printk("[mtkts_flight_write] bad argument\n");
	kfree(ptr_param_data);
	return -EINVAL;
}

/* int  mtkts_flight_register_cooler(void)
 * {
 *	cooling devices
 *	cl_dev_sysrst = mtk_thermal_cooling_device_register(
 *				"mtktsbtsnrpatery-sysrst", NULL,
 *				&mtkts_flight_cooling_sysrst_ops);
 *	return 0;
 * }
 */

#if 0
static void mtkts_flight_cancel_thermal_timer(void)
{
	/* cancel timer
	 * mtkts_flight_printk("mtkts_flight_cancel_thermal_timer\n");
	 *
...
static void mtkts_flight_start_thermal_timer(void)
{
	/* mtkts_flight_printk("mtkts_flight_start_thermal_timer\n");
...
static int mtkts_flight_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtkt_flash_light", num_trip,
					NULL, &mtkts_flight_dev_ops, 0, 0, 0,
					interval * 1000);

	return 0;
}

/* void mtkts_flight_unregister_cooler(void) */
/* { */
	/* if (cl_dev_sysrst) { */
...
static int mtkts_flight_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_flight_read, NULL);
}

static const struct file_operations mtkts_flight_fops = {
	.owner = THIS_MODULE,
	.open = mtkts_flight_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = mtkts_flight_write,
	.release = single_release,
};


static int mtkts_flight_param_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_flight_param_read, NULL);
}

static const struct file_operations mtkts_flight_param_fops = {
	.owner = THIS_MODULE,
	.open = mtkts_flight_param_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = mtkts_flight_param_write,
	.release = single_release,
};


#if defined(CONFIG_MEDIATEK_MT6577_AUXADC)
static int mtkts_flight_probe(struct platform_device *pdev)
{
	int err = 0;
	int ret = 0;
	...
	if (!pdev->dev.of_node) {
		mtkts_flight_printk("[%s]Only DT based supported\n",
		__func__);
		return -ENODEV;
	}
	...
	if (ret) {
		mtkts_flight_printk("[%s] fail to get auxadc iio ch0: %d\n",
			__func__, ret);
		return ret;
	}

	g_ADC_channel = thermistor_ch3->channel->channel;
	mtkts_flight_printk("[%s]get auxadc iio ch: %d\n", __func__,
...
static struct platform_driver mtk_thermal_flight_driver = {
	.remove = NULL,
	.shutdown = NULL,
	.probe = mtkts_flight_probe,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
...
static int __init mtkts_flight_init(void)
{
	struct proc_dir_entry *entry = NULL;
	struct proc_dir_entry *mtkts_flight_dir = NULL;
#if defined(CONFIG_MEDIATEK_MT6577_AUXADC)
	int err = 0;
#endif
...
#if defined(CONFIG_MEDIATEK_MT6577_AUXADC)
	err = platform_driver_register(&mtk_thermal_flight_driver);
	if (err) {
		mtkts_flight_printk("thermal driver callback register failed.\n");
		return err;
	}
#endif
	/* setup default table */
	...
	mtkts_flight_dir = mtk_thermal_get_proc_drv_therm_dir_entry();
	if (!mtkts_flight_dir) {
		mtkts_flight_dprintk(
			"[%s]: mkdir /proc/driver/thermal failed\n", __func__);
	} else {
		entry = proc_create("flight_temp", 0664, mtkts_flight_dir,
				&mtkts_flight_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry = proc_create("flight_temp_param", 0664, mtkts_flight_dir,
				&mtkts_flight_param_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
	}
	...
	mtkts_flight_register_thermal();
#if 0
	mtkTTimer_register("mtktsbtsnrpa", mtkts_flight_start_thermal_timer,
					mtkts_flight_cancel_thermal_timer);
#endif
	return 0;
}

static void __exit mtkts_flight_exit(void)
{
	mtkts_flight_dprintk("[%s]\n", __func__);
	mtkts_flight_unregister_thermal();
#if 0
...
#endif
	/* mtkts_flight_unregister_cooler(); */
}

module_init(mtkts_flight_init);
module_exit(mtkts_flight_exit);
