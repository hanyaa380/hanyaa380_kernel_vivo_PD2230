				...
				unsigned int offset, unsigned int size)
{
	...
	if(pdata->pdrv->addr_offset > 0) {
		offset += pdata->pdrv->addr_offset;
	}
					 ...
					 unsigned int index)
{
	...
	/*zhenrong add for otp addr offset beg*/
	of_property_read_u32(client->dev.of_node, "addr-offset", &addr_offset);
	pinst->addr_offset = addr_offset;
	/*zhenrong add for otp addr offset end*/
	return ret;
}
