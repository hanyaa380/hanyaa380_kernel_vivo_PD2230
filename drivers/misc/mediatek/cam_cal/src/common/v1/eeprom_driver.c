...
extern unsigned char eeprom_slave_addr
	[IMGSENSOR_SENSOR_IDX_MAX_NUM]; //vivo lishuoyxqd otp remake add 20220613

#define NEW_UNLOCK_IOCTL
#ifndef NEW_UNLOCK_IOCTL
static int EEPROM_drv_ioctl(struct inode *a_pstInode, struct file *a_pstFile,
...
#endif
{
	...
	struct stCAM_CAL_CMD_INFO_STRUCT
		tmp_cmdInf; //vivo lishuoyxqd otp remake add 20220613
	...
	case CAM_CALIOC_G_READ:
		...
		pr_debug("SensorID=%x DeviceID=%x idx=%d\n", ptempbuf->sensorID,
			 ptempbuf->deviceID,
			 IMGSENSOR_SENSOR_IDX_MAP(ptempbuf->deviceID));

		if (eeprom_slave_addr[IMGSENSOR_SENSOR_IDX_MAP(
			    ptempbuf->deviceID)] !=
		    0) { //vivo lishuoyxqd otp remake path  20220613
			...
			tmp_cmdInf.client->addr =
				eeprom_slave_addr[IMGSENSOR_SENSOR_IDX_MAP(
					ptempbuf->deviceID)] >>
				1;
			i4RetValue = Common_read_region(tmp_cmdInf.client,
							ptempbuf->u4Offset,
							pu1Params,
							ptempbuf->u4Length);
		} else { //vivo lishuoyxqd: mtk origin patch  20220613
			pcmdInf = EEPROM_get_cmd_info_ex(ptempbuf->sensorID,
							 ptempbuf->deviceID);

			/* Check the max size if specified */
			if (pcmdInf != NULL && (pcmdInf->maxEepromSize != 0) &&
			    (pcmdInf->maxEepromSize <
			     (ptempbuf->u4Offset + ptempbuf->u4Length))) {
				pr_debug(
					"Error!! not support address >= 0x%x!!\n",
					pcmdInf->maxEepromSize);
				kfree(pBuff);
				kfree(pu1Params);
				return -EFAULT;
