...
int disp_lcm_suspend(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->suspend_power)
			if ((is_atboot) || (phone_shutdown_state))
...
int disp_lcm_setvivocabc_open(struct disp_lcm_handle *plcm, void *handle, char leveldimming, char levelsetting)
{
	struct LCM_DRIVER *lcm_drv = NULL;
	int ret = 0;
	...
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
		if (lcm_drv->lcm_cabc_open) {
			lcm_drv->lcm_cabc_open(handle, leveldimming, levelsetting);
		} else {
			DISPERR("FATAL ERROR, lcm_drv->lcm_cabc_open is null\n");
			ret = -1;
		}
	} else {
		DISPERR("lcm_drv cabc is null\n");
		ret = -1;
	}
	return ret;
}
int disp_lcm_setvivocabc_close(struct disp_lcm_handle *plcm, void *handle, char level)
{
	struct LCM_DRIVER *lcm_drv = NULL;
	int ret = 0;
	DISPFUNC();
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
		if (lcm_drv->lcm_cabc_close) {
			lcm_drv->lcm_cabc_close(handle, level);
		} else {
			DISPERR("FATAL ERROR, lcm_drv->lcm_cabc_close is null\n");
			ret = -1;
		}
	} else {
		DISPERR("lcm_drv cabc is null\n");
