...
struct tag_video_lfb {
	...
	u32 rf_hw_vision;
	u32 lcm_imei_str[4] ;
	/*product_id must be define in lk atags.c,
	or it will cause nullptr and driver error*/
    u32 product_id;
	u32 bl_hw_version;
...
#endif
unsigned int pre_bklt_level = 0xff;
unsigned int lcm_software_id = 0xff;
unsigned int lcm_max_brightness;
unsigned int panel_max_bl_level = 2047;
unsigned int silentreboot;
unsigned int lcm_id_version;
unsigned int lcm_ldo_vision;
unsigned int rf_hw_id;
unsigned int lcm_imei_str[4];
unsigned int product_hw_id;
...
static int panel_parse_vivo_panel_info(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		LCM_INFO("materiel_id = %s lcm_ids=%s\n",materiel_id,lcm_ids);
		panel_supply[i].materiel_id = materiel_id;
		panel_supply[i].lcm_ids = lcm_ids;
	}

	num_panel_supply = count;
...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	if (!prop)
		DISPERR("[DT][videolfb] project_name parse failed\n");
	else {
		strncpy((char *)project_name, prop, sizeof(project_name));
		project_name[size] = '\0';
		DISPERR("[DT][videolfb] project_name    = %s\n",
			project_name);
	}
	...
	if (videolfb_tag) {
		...
		DISPERR("[DT][videolfb] mtkfb lcm name orignal find %s\n", mtkfb_lcm_name);
	if ((!strcmp(project_name, "PD2139JF_EX")) || (!strcmp(project_name, "PD2216F_EX"))) {
		if (!strcmp(mtkfb_lcm_name, "pd2036_boe_nt36525b_noflash_hdp_dsi_vdo")||
			!strcmp(mtkfb_lcm_name, "pd2036_inx_ili9882q_noflash_hdp_dsi_vdo")){
			DISPERR("[DT][videolfb] PD2139 mtkfb lcm name match succed\n");
		}
		else {
			memset((void *)mtkfb_lcm_name, 0, sizeof(mtkfb_lcm_name));
			lcm_id_version = videolfb_tag->lcm_id;
			if ((lcm_id_version == 0x07) || (lcm_id_version == 0x0f)) {
				strncpy((char *)mtkfb_lcm_name, "pd2036_boe_nt36525b_noflash_hdp_dsi_vdo",
				sizeof("pd2036_boe_nt36525b_noflash_hdp_dsi_vdo"));
				mtkfb_lcm_name[strlen("pd2036_boe_nt36525b_noflash_hdp_dsi_vdo")] = '\0';
			}else if (lcm_id_version == 0x02) {
				strncpy((char *)mtkfb_lcm_name, "pd2036_inx_ili9882q_noflash_hdp_dsi_vdo",
				sizeof("pd2036_inx_ili9882q_noflash_hdp_dsi_vdo"));
				mtkfb_lcm_name[strlen("pd2036_inx_ili9882q_noflash_hdp_dsi_vdo")] = '\0';
			}
		}
	}
		...
		if (lcm_max_brightness < 0x42)
			panel_max_bl_level = 2039;
		else
			panel_max_bl_level = 2020;
		silentreboot = videolfb_tag->silentreboot;
		lcm_id_version = videolfb_tag->lcm_id;
		lcm_software_id = videolfb_tag->lcm_id;
		lcm_ldo_vision = videolfb_tag->lcm_ldo_vision;
		rf_hw_id = videolfb_tag->rf_hw_vision;
		...
		product_hw_id = videolfb_tag->product_id;
...
static void mtkfb_shutdown(struct platform_device *pdev)
{
	...
	if (primary_display_is_sleepd()) {
		if (!strcmp(mtkfb_lcm_name, "pd2036_boe_icnl9911_noflash_hdp_dsi_vdo") ||
			!strcmp(mtkfb_lcm_name, "pd2074_txd_nt36525a_noflash_hdp_dsi_vdo")) {
