...
unsigned int lcm_low_backlight_cabc_status = 1;
unsigned int vivo_lcm_cabc_backlight = 1681;
unsigned int vivo_esd_check_ps_status = 0;
...
unsigned int panel_reset_state; /*0: power off ; 1: power on*/
...
{
	LCM_INFO("report_lcm_id lcm_id_version = %d\n", lcm_id_version);
	if ((!strcmp(project_name, "PD2139JF_EX")) || (!strcmp(project_name, "PD2216F_EX"))){
		if	(lcm_id_version == 0x07)
			return 0x30;
		else if (lcm_id_version == 0x0f)
			return 0x32;
		else if (lcm_id_version == 0x02)
			return 0x63;
		else if (lcm_id_version == 0x03)
			return 0x92;
		else if (lcm_id_version == 0x06)
			return 0x93;
		else
			return 0xFF;
	}
...
{
	unsigned int temp_lcmid;
	DISPINFO("using new interface mdss_report_lcm_id\n");
	temp_lcmid = report_lcm_id();
	return temp_lcmid;
}
...
int primary_display_resume(void)
{
	...
	if (lcm_drv->lcm_reset) {
		/*3C 40 50 15 15 are respectively for PD1901BOE AUO lianchuang tianma +ft8006p,
		ft8006p don't need reset here or it will reset twice when resume*/
		/*0x62 is for PD1901 qunchuang ili9881h*/
		if (!strcmp(project_name, "PD1987") || !strcmp(project_name, "PD2014")) {
			lcm_drv->lcm_reset();
		} else if ((!strcmp(project_name, "PD2036F_EX"))||(!strcmp(project_name, "PD2139F_EX")) || (!strcmp(project_name, "PD2139JF_EX")) || (!strcmp(project_name, "PD2216F_EX"))) {
			if (lcm_id == 0x31 || lcm_id == 0xA1 || lcm_id == 0xB1) {
				DISPINFO("pd2036f_ex nt36525b need reset keep high befor LP11\n");
				lcm_drv->lcm_reset();
			}
		} else if (lcm_id != 0x62 && lcm_id != 0x3C && lcm_id != 0x40 && lcm_id != 0x50 && lcm_id != 0x15  && lcm_id != 0x01 && lcm_id != 0x02 && lcm_id != 0x20 && lcm_id != 0x21 && lcm_id != 0x14 && lcm_id != 0x22 && lcm_id != 0x11 && lcm_id != 0x00 && lcm_id != 0x35) {
			lcm_drv->lcm_reset();
		} else {
		DISPERR("LCM no need reset keep high befor LP11\n");
		}
	}
...
int _set_vivo_cabc_open(unsigned char leveldimming, unsigned char levelsetting)
{
	int ret = 0;
	...
	if (primary_display_is_video_mode()) {
		...
		disp_lcm_setvivocabc_open(pgc->plcm, cmdq_handle_cabcopen, leveldimming, levelsetting);
		_cmdq_flush_config_handle_mira(cmdq_handle_cabcopen, 0);
		DISPMSG("[BL]_set_vivo_cabc_open ret=%d\n", ret);
	} else {
...
int _set_vivo_cabc_close(unsigned char level)
{
	int ret = 0;
	...
	if (primary_display_is_video_mode()) {
		...
		disp_lcm_setvivocabc_close(pgc->plcm, cmdq_handle_cabcclose, level);
		_cmdq_flush_config_handle_mira(cmdq_handle_cabcclose, 0);
		DISPMSG("[BL]_set_vivo_cabc_close ret=%d\n", ret);
	} else {
...
void primary_display_setvivocabc_open_unlock(unsigned char leveldimming, unsigned char levelsetting)
{
	primary_display_idlemgr_kick((char *)__func__, 0);
	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State primary_display_setvivocabc_open invald\n");
	} else {
	_set_vivo_cabc_open(leveldimming, levelsetting);
	}
}


void primary_display_setvivocabc_open(unsigned char leveldimming, unsigned char levelsetting)
{
	_primary_path_switch_dst_lock();
	_primary_path_lock(__func__);
	primary_display_setvivocabc_open_unlock(leveldimming, levelsetting);
	_primary_path_unlock(__func__);
	_primary_path_switch_dst_unlock();
 }


void primary_display_setvivocabc_close_unlock(unsigned char level)
{
	primary_display_idlemgr_kick((char *)__func__, 0);
	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State primary_display_setvivocabc_close invald\n");
	} else {
	_set_vivo_cabc_close(level);
	}
}

void primary_display_setvivocabc_close(unsigned char level)
{

	_primary_path_switch_dst_lock();
	_primary_path_lock(__func__);

	primary_display_setvivocabc_close_unlock(level);
...
int primary_display_setbacklight(unsigned int level)
{
	...
	if (last_level == level && 0 == esd_checking)
		return 0;
...
/**** materiel_id *****/
extern struct vivo_panel_supply *panel_supply;
extern unsigned int num_panel_supply;
static ssize_t materiel_id_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
	int i = 0;

	if (!panel_supply || !num_panel_supply)
		return -EINVAL;
	LCM_INFO("materiel_id_show begin\n");
	for (i = 0; i < num_panel_supply; i++)
		count += snprintf(buf + count, PAGE_SIZE - count, "%s\t%s\n",
				panel_supply[i].materiel_id, panel_supply[i].lcm_ids);
	return count;
}
/**** materiel_id *****/
...
static ssize_t lcm_color_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%02x\n", 0x01);/*Most of the panel are black , modifying the default value*/
}
static ssize_t lcm_id_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	unsigned int lcm_id = 0;
	lcm_id = bl_hw_version << 24 | report_lcm_id();
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret) {
		LCM_ERR("Invalid input for lcm vivo_sre_check_enable_store\n");
		/* return -EINVAL; */
	}
...
{
	if(lcm_cabc_status && (lcm_low_backlight_cabc_status == 0)){
		LCM_INFO("current state is low backlight");
		return sprintf(buf, "%02x\n", lcm_low_backlight_cabc_status);
	}else{
		return sprintf(buf, "%02x\n", lcm_cabc_status);
	}
}
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	LCM_INFO("vincent:read lcm_cabc_status is %d, pre_cabc_level is %d, vivo_lcm_cabc_backlight is %d\n", lcm_cabc_status, pre_cabc_level,vivo_lcm_cabc_backlight);

	if (pre_cabc_level == lcm_cabc_status) {
		LCM_INFO("%s: lcm_cabc_status is %d, pre_cabc_level is %d, same as last time , return\n", __func__, lcm_cabc_status, pre_cabc_level);
		return count;
		}

	if (vivo_lcm_cabc_backlight <= 0) {
		lcm_low_backlight_cabc_status = 0;
		LCM_INFO("backlihgt is off now, please set CABC after backlight on\n");
		return count;
	} else if (lcm_cabc_status && (vivo_lcm_cabc_backlight < 788) && ((!strcmp(project_name, "PD1987") || !strcmp(project_name, "PD1987F_EX")))) {
		LCM_INFO("current level is %d ,less than %d , close CABC\n", vivo_lcm_cabc_backlight, 788);
		primary_display_setvivocabc_close(0);
		lcm_low_backlight_cabc_status = 0;
	} else if (lcm_cabc_status && (vivo_lcm_cabc_backlight < 788) && ((!strcmp(project_name, "PD2014") || !strcmp(project_name, "PD2014F_EX")))) {
		LCM_INFO("current level is %d ,less than %d , close CABC\n", vivo_lcm_cabc_backlight, 788);
		primary_display_setvivocabc_close(0);
		lcm_low_backlight_cabc_status = 0;
	}  else if (lcm_cabc_status && (vivo_lcm_cabc_backlight < 1524) && (!strcmp(project_name, "PD1901"))) {
		LCM_INFO("current level is %d ,less than %d , close CABC\n", vivo_lcm_cabc_backlight, 1524);
		primary_display_setvivocabc_close(0);
		lcm_low_backlight_cabc_status = 0;
	} else if (lcm_cabc_status && (vivo_lcm_cabc_backlight < 788) && (!strcmp(project_name, "PD2036F_EX"))) {
		LCM_INFO("current level is %d ,less than %d , dont set CABC\n", vivo_lcm_cabc_backlight, 788);
	}else if (lcm_cabc_status && (vivo_lcm_cabc_backlight < 788) && (!strcmp(project_name, "PD2139F_EX")) && (!strcmp(project_name, "PD2139JF_EX")) && (!strcmp(project_name, "PD2216F_EX"))) {
		LCM_INFO("current level is %d ,less than %d , dont set CABC\n", vivo_lcm_cabc_backlight, 788);
	}else if (lcm_cabc_status && (vivo_lcm_cabc_backlight < 788) && (!strcmp(project_name, "PD2074F_EX"))) {
		LCM_INFO("lcm_cabc_status is %d ,current level is %d ,less than %d , dont set CABC\n", lcm_cabc_status,vivo_lcm_cabc_backlight, 788);
	} else {
		if (lcm_cabc_status == 3) {
			lcm_low_backlight_cabc_status = 1;
			primary_display_setvivocabc_open(0x2c, 0x3);
		} else if (lcm_cabc_status == 2) {
			lcm_low_backlight_cabc_status = 1;
			primary_display_setvivocabc_open(0x2c, 0x2);
		} else if (lcm_cabc_status == 1) {
			lcm_low_backlight_cabc_status = 1;
			primary_display_setvivocabc_open(0x2c, 0x1);
		} else {
			lcm_cabc_status = 0;
			lcm_low_backlight_cabc_status = 0;
			primary_display_setvivocabc_close(0);
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	LCM_INFO("%s: vincent+dimming_enable=%d, temp = %d, lcm_software_id=0x%x\n", __func__, dimming_for_camera, temp, lcm_id_version);
	if (!strcmp(project_name, "PD1987")) {
		if (lcm_id_version == 0x61 || lcm_id_version == 0x63 || lcm_id_version == 0x65) {
			dimming_for_camera = temp;
			primary_display_setvivocabc_open(0x2d, dimming_for_camera);
		}
	} else if (!strcmp(project_name, "PD2014")) {
		if (lcm_id_version == 0x4a || lcm_id_version == 0x5b || lcm_id_version == 0x44) {
			dimming_for_camera = temp;
			primary_display_setvivocabc_open(0x2d, dimming_for_camera);
		}
	}	else if ((lcm_id_version == 0x4f)  || (lcm_id_version == 0x53)  || (lcm_id_version == 0x4E) || (lcm_id_version == 0x54)) {
		dimming_for_camera = temp;
		primary_display_setvivocabc_open(0x2d, dimming_for_camera);
...
{
	unsigned int vivo_esd_check_enable = 0;
	struct LCM_PARAMS *lcm_param = disp_lcm_get_params(pgc->plcm);
	vivo_esd_check_enable = lcm_param->dsi.esd_check_enable;
	printk("vivo_esd_check_enable_show vivo_esd_check_enable is %d\n", vivo_esd_check_enable);
	return sprintf(buf,"%d\n", vivo_esd_check_enable);
}

static ssize_t lcm_esd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("lcm_esd_show vivo_esd_check_ps_status is %d\n", vivo_esd_check_ps_status);
	return sprintf(buf, "%02x\n", vivo_esd_check_ps_status);
}

static ssize_t lcm_esd_store(struct device *dev,
						struct device_attribute *attr, const char *buf, size_t count)
{
	int ret, temp;
	ret = kstrtoint(buf, 10, &temp);
	...
	if(vivo_esd_check_ps_status)
		primary_display_esd_check_enable(1);
	printk("vivo read vivo_esd_check_ps_status is %d\n", vivo_esd_check_ps_status);
	return count;
}

static ssize_t lcm_sre_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%02x\n", 0x00);
}

static ssize_t lcm_sre_store(struct device *dev,
						struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static ssize_t lcm_brightness_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	unsigned int lcm_id = 0;
	...
	if (lcm_id == 0x13 || lcm_id == 0x20 || lcm_id == 0x30 || 0x02 == lcm_id || 0x21 == lcm_id || 0x14 == lcm_id || 0x22 == lcm_id || 0x50 == lcm_id)
		brighness = 0x01;
	if (lcm_id_version == 0x4A || lcm_id_version == 0x4C || lcm_id_version == 0x4B \
		|| lcm_id_version == 0x4E || lcm_id_version == 0x51 || lcm_id_version == 0x4f \
		|| lcm_id_version == 0x52 || lcm_id_version == 0x53 || lcm_id_version == 0x54)
		brighness = 0x01;
	return sprintf(buf, "%02x\n", brighness);
}

/********************************************************************
For some high power consumption panels , such as PD1901 Ft8006P,
In engineer mode , power consumption is higher than others,
lcm_vddi_high is used to flag this this kind or panels,we need match special power stantard
*********************************************************************/

static ssize_t lcm_vddi_high_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	unsigned int vddi_high = 0;
	if (!strcmp(project_name, "PD1901")) {
		/*4 manufactures use FT8006P*/
		if (lcm_software_id == 0x60 || lcm_software_id == 0x63 || lcm_software_id == 0x74|| lcm_software_id == 0x78)
			vddi_high = 0x01;
	}
	return sprintf(buf, "%02x\n", vddi_high);
}
...
static ssize_t panel_bk_lcm_ic_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{

	LCM_INFO("[LCM][disp] bk_lcmic_id = %d\n", bk_lcmic_id);
...
static DEVICE_ATTR(vivo_esd_check_enable, 0644, vivo_esd_check_enable_show, NULL);
static DEVICE_ATTR(vivo_esd_check_ps, 0644, lcm_esd_show, lcm_esd_store);
#ifdef CONFIG_LCM_DRE_SUPPORT
...
static DEVICE_ATTR(lcm_sre, 0644, lcm_sre_show, lcm_sre_store);
static DEVICE_ATTR(lcm_brightness, 0644, lcm_brightness_show, NULL);
static DEVICE_ATTR(lcm_vddi_high, 0644, lcm_vddi_high_show, NULL);
static DEVICE_ATTR(lcm_diming, 0644, lcm_diming_show, lcm_diming_store);
static DEVICE_ATTR(imei_ctl, 0444, lcm_imei_show, NULL);
static DEVICE_ATTR(bk_lcm_ic, 0444, panel_bk_lcm_ic_show, NULL);
...
static struct attribute *disp_lcm_sys_attrs[] = {
	...
	&dev_attr_lcm_sre.attr,
	&dev_attr_lcm_brightness.attr,
	&dev_attr_lcm_vddi_high.attr,
...
int incell_tp_reset_output(int status)
{
	int ret = 0;
	struct LCM_DRIVER *lcm_drv = pgc->plcm->drv;
	if (!panel_off_state) {
...
int incell_tp_reset_output_paneloff(int enable)
{
	...
	if (lcm_drv->tp_reset)
			lcm_drv->tp_reset(enable);
	return ret;
}



int incell_avdd_avee_output(int status)
{
	int ret = 0;
	struct LCM_DRIVER *lcm_drv = pgc->plcm->drv;

	if (lcm_drv->avdd_avee_output)
		lcm_drv->avdd_avee_output(status);
	return ret;
}
