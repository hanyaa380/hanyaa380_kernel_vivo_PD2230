...
#endif
//B230308-2909, vivo add for lcm error code report begin
extern int lcm_error_code_report(unsigned int error_code);
//B230308-2909, vivo add for lcm error code report end
	...
	while (1) {
		...
		do {
			...
			DISPERR(
				...
				i);
			//B230308-2909, vivo add for lcm error code report begin
			lcm_error_code_report(1004);//error type:1004
			//B230308-2909, vivo add for lcm error code report end
...
#if defined(CONFIG_PD2226_ESD_CHECK) || defined(CONFIG_ESD_CHECK_ENABLE)
			mtkfb_set_backlight_level(bl_brightness_hal);
#endif
