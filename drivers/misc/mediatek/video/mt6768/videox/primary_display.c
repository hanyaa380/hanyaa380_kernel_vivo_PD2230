	...
	int is_lcm_inited)
{
	...
	_primary_path_unlock(__func__);
	disp_creat_sys_file();
	if (strstr(mtkfb_lcm_name, "pd2226"))
		vivo_sre_enable = 1;
	return ret;
}
...
#ifdef CONFIG_PD2226_MAX_BACKLIGHT
atomic_t current_level = ATOMIC_INIT(0);
#endif
#if defined(CONFIG_PD2226_ESD_CHECK) || defined(CONFIG_ESD_CHECK_ENABLE)
...
#ifdef CONFIG_PD2226_MAX_BACKLIGHT
	atomic_set(&current_level, level);
#endif
#if defined(CONFIG_PD2226_ESD_CHECK) || defined(CONFIG_ESD_CHECK_ENABLE)
						...
						struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%s\n",lcm_drv ? lcm_drv->name : 0);
}

//B230225-2098, vivo add for lcm error code report begin
unsigned int lcm_error_code;

int lcm_error_code_report(unsigned int error_code)
{
	lcm_error_code = error_code;
	return 0;
}
EXPORT_SYMBOL(lcm_error_code_report);

static ssize_t error_code_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	DISPMSG("lcm_error_code = %d\n", lcm_error_code);
	return snprintf(buf, PAGE_SIZE, "%04x\n", lcm_error_code);
}
//B230225-2098, vivo add for lcm error code report end

#ifdef CONFIG_PD2226_MAX_BACKLIGHT
atomic_t lcm_sre_state = ATOMIC_INIT(0);
static ssize_t elec_sre_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%04x\n", atomic_read(&lcm_sre_state));
}
static ssize_t elec_sre_store(struct device *dev,
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(atomic_read(&lcm_sre_state) != temp)
	{
		if(temp == 0)
			atomic_set(&lcm_sre_state, 0);
		else if(temp == 1)
			atomic_set(&lcm_sre_state, 1);
		else
		{
			pr_err("Invalid level: temp=%d\n", temp);
			return -EINVAL;
		}
		if(atomic_read(&current_level) > 3000)
			if (lcm_drv->set_backlight_cmdq)
				lcm_drv->set_backlight_cmdq(NULL,atomic_read(&current_level));
	}
	LCD_INFO("lcm_sre_state=%d", atomic_read(&lcm_sre_state));
...
//B230225-2098, vivo add for lcm error code report begin
static DEVICE_ATTR(error_code, 0644, error_code_show, NULL);
//B230225-2098, vivo add for lcm error code report end
...
#ifdef CONFIG_PD2226_MAX_BACKLIGHT
static DEVICE_ATTR(elec_sre, 0644, elec_sre_show, elec_sre_store);
#endif
...
static struct attribute *disp_lcm_sys_attrs[] = {
	&dev_attr_error_code.attr,/*B230225-2098, vivo add for lcm error code report */
