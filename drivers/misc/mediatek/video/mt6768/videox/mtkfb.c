...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	if ((!strcmp(mtkfb_find_lcm_driver(),
		...
		"pd2281_tm_ili7807s_fhdplus_dsi_vdo"))) {
...
static void mtkfb_shutdown(struct platform_device *pdev)
{
	...
	if (primary_display_is_sleepd()) {
		if (!strcmp(mtkfb_lcm_name, "pd2226_txd_td4160_fhdplus_dsi_vdo") ||
			!strcmp(mtkfb_lcm_name, "pd2226_tm_icnl9916_fhdplus_dsi_vdo")) {
	...
	mdss_dsi_panel_reset_and_powerctl(8);
