 ...
 *****LM3644*****
 FLASH1/2 (mA) ��(Brightness Code*11.725 mA) + 10.9 mA
...
#elif defined(CONFIG_MTK_CAM_PD2226F_EX)
static const int lm3644_current[LM3644_LEVEL_NUM] = {  /*current:mA */
	 33,  51,  65,  75,  100, 120, 140, 198,  246, 304,
	351, 398, 445, 503, 550, 597, 656, 703,  750, 796,
	857, 902, 961, 1008, 1067, 1100};
static const unsigned char lm3644_torch_level[LM3644_LEVEL_NUM] = {
	0x17, 0x24, 0x2E, 0x35, 0x42, 0x4D, 0x63, 0x00, 0x00, 0x00, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
static const unsigned char lm3644_flash_level[LM3644_LEVEL_NUM] = {
	0x01, 0x03, 0x05, 0x07, 0x09, 0x0B, 0x0D, 0x10, 0x14, 0x19,
...
static const int lm3644_current[LM3644_LEVEL_NUM] = {  /*current:mA */
	...
	351, 398, 445, 503, 550, 597, 656, 703,  750, 796,
	857, 902, 961, 1008, 1067, 1100};  //1067, 1100  1019, 1054
#else
static const int lm3644_current[LM3644_LEVEL_NUM] = {  /*current:mA */
   23,  46, 70, 93,117,140,163,199,246,304,
   351,398,445,504,550,597,656,703,750,797,
   855,902,949,996,1019,1054
};
#endif
...
static const unsigned char lm3644_flash_level[LM3644_LEVEL_NUM] = {
	0x01, 0x03, 0x05, 0x07, 0x09, 0x0B, 0x0D, 0x10, 0x14, 0x19,
	0x1D, 0x21, 0x25, 0x2A, 0x2E, 0x32, 0x37, 0x3B, 0x3F, 0x43,
	0x48, 0x4C, 0x50, 0x54, 0x56,0x59};  //vivo lishuo fixed [B210304-3093]  new:0x56,0x59};  old:  0x59,0x5D};
#endif
...
static int state_pin_short = 0; //20210301 lishuo add for I2C short to GND test.
static int state_pin_short_ignore = 0;

static int lm3644_read_reg(struct i2c_client *client, u8 reg)
{
	int val = 0;
	struct lm3644_chip_data *chip = i2c_get_clientdata(client);
	if(state_pin_short_ignore == 0){
		if(state_pin_short == 1){ //20210301 lishuo add for I2C short to GND test.
	        return 0;
		}
	}
...
static int lm3644_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
	...
	struct lm3644_chip_data *chip = i2c_get_clientdata(client);
	if(state_pin_short_ignore == 0){
		if(state_pin_short == 1){ //20210519 zhujianjia add for I2C short to GND test.
	    return 0;
		}
	}
	...
	if (ret < 0){
		...
		state_pin_short = 1;//20210519 zhujianjia add for I2C short to GND test.
	}
	return ret;
}
...
static int lm3644_set_red_flash_level_and_enabe_ch2(void)
{
	...
	ret = lm3644_write_reg(lm3644_i2c_client, LM3644_REG_FLASH_LEVEL_LED2, 0x65);/*0x6E = 1.3A,0x65 = 1.2A*/
...
int lm3644_init(void)
{
	...
	lm3644_pinctrl_set(LM3644_PINCTRL_PIN_HWEN, LM3644_PINCTRL_PINSTATE_HIGH);
	/*msleep(20);*/
	mdelay(2);     //vivo lishuo fix power sequence for [B210207-1256] 20210207
...
#if defined(CONFIG_MTK_CAM_PD2140F_EX) || defined(CONFIG_MTK_CAM_PD2140IF_EX) || defined(CONFIG_MTK_CAM_PD2140) || defined(CONFIG_MTK_CAM_PD2140JF_EX)
		if (device_id == 0x12)
			lm3644_write_reg(lm3644_i2c_client, LM3644_REG_TORCH_LEVEL_LED1, 0x22);/*(Brightnees code x 1.4mA)+0.997ma Torch*/
		else
			lm3644_write_reg(lm3644_i2c_client, LM3644_REG_TORCH_LEVEL_LED1, 0x22);/*(Brightnees code x 1.4mA)+0.997ma Torch*/
#elif defined(CONFIG_MTK_CAM_PD2074F_EX) || defined(CONFIG_MTK_CAM_PD1987) || defined(CONFIG_MTK_CAM_PD2036F_EX) || defined(CONFIG_MTK_CAM_PD2139JF_EX)
        if (device_id == 0x12)
...
static int lm3644_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_DUTY:
		pr_info("FLASH_IOC_SET_DUTY(%d): %d  lm3644_current[fl_arg->arg] =%d device_id=%d\n",
				channel, (int)fl_arg->arg,lm3644_current[(int)fl_arg->arg],device_id);//add by vivo changcheng
	...
	case FLASH_IOC_SET_ONOFF:
		...
		if (fl_arg->arg == 1) {
			...
			if (lock_touch == 0 && lock_touch_sub == 0) {
				...
				//add vivo changcheng  for  setonoff_1 hardware poweron and  setonoff_0 hardware poweroff reset then record duty is changed start
				lm3644_pinctrl_set(LM3644_PINCTRL_PIN_HWEN, LM3644_PINCTRL_PINSTATE_LOW);     //vivo lishuo fix EN always High for [B210207-1256] 20210207
				//add vivo changcheng  for  setonoff_1 hardware poweron and  setonoff_0 hardware poweroff reset then record duty is changed end
			}
		}
		break;
...
static ssize_t lm3644_strobe_store(struct flashlight_arg arg)
{
	...
	if (arg.channel == LM3644_CHANNEL_CH2 && arg.level == 27){
		...
		if(arg.channel == LM3644_CHANNEL_CH2 && arg.level == 26){
			...
			lm3644_write_reg(lm3644_i2c_client, LM3644_REG_FLASH_LEVEL_LED2, 0x65);/*0x6E = 1.3A,0x65 = 1.2A*/
			for (i = 0; i < 10; i++){
...
static bool lm3644_judge(struct i2c_client *client)
{
	lm3644_pinctrl_set(LM3644_PINCTRL_PIN_HWEN, LM3644_PINCTRL_PINSTATE_HIGH);
	device_id = lm3644_read_reg(lm3644_i2c_client, LM3644_DEVICE_ID);
	chip_id = lm3644_read_reg(lm3644_i2c_client, LM3644_CHIP_ID);
	lm3644_pinctrl_set(LM3644_PINCTRL_PIN_HWEN, LM3644_PINCTRL_PINSTATE_LOW);
	pr_info("%s:%d DeviceID(0x%x) IC_ID(0x%x)", __func__, __LINE__, device_id, chip_id);
	if (chip_id != 0x30)
		return true;
	else
		return false;
...
static int lm3644_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	device_id = lm3644_read_reg(lm3644_i2c_client, LM3644_DEVICE_ID);
	chip_id = lm3644_read_reg(lm3644_i2c_client, LM3644_CHIP_ID);
	pr_err("lm3644_i2c_probe lm3644 DEVICE_ID = 0x%x\n", device_id);
	pr_err("lm3644_i2c_probe lm3644 CHIP_ID = 0x%x\n", chip_id);
	if(device_id  != 0x02){
		lm3644_i2c_client->addr = 0x6B;
		device_id = lm3644_read_reg(lm3644_i2c_client, LM3644_DEVICE_ID);
		chip_id = lm3644_read_reg(lm3644_i2c_client, LM3644_CHIP_ID);
		pr_err("test2222 lm3644_i2c_probe lm36413 DEVICE_ID = 0x%x\n", device_id);
		pr_err("test2222 lm3644_i2c_probe lm3644 CHIP_ID = 0x%x\n", chip_id);
	}
	if(device_id<0){  //20210301 lishuo add i2c_smbus_read_byte_data return <0 if i2c failed
	...
	lm3644_pinctrl_set(LM3644_PINCTRL_PIN_HWEN, LM3644_PINCTRL_PINSTATE_LOW);     //vivo lishuo fix EN high after boot for [B210207-1256] 20210207
	pr_debug("Probe done.\n");
