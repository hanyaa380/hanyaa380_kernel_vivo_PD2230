		...
		struct flashlight_operations *dev_ops)
{
	...
	dev_info(&client->dev, "Register device (%d,%d,%d)\n",
			dev_id->type, dev_id->ct, dev_id->part);
		...
		struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_RT_TEMPTURE_DUTY:
		pr_debug("FLASH_IOC_SET_RT_TEMPTURE_DUTY(%d,%d,%d): %d\n",
				type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
		if (fdev->ops) {
			if (fdev->ops->flashlight_ioctl(fdev->client, FLASH_IOC_SET_RT_TEMPTURE_DUTY,
				(unsigned long)&fl_dev_arg)) {
					pr_err("Failed to set level\n");
			}
		}
	...
	case FLASH_IOC_SET_SW_DURATION:
		pr_info("FLASH_IOC_SET_SW_DURATION(%d,%d,%d): %d\n",
				type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
		if (fdev->ops) {
			if (fdev->ops->flashlight_ioctl(fdev->client, FLASH_IOC_SET_SW_DURATION,
				(unsigned long)&fl_dev_arg)) {
					pr_err("Failed to set level\n");
			}
		}
	...
	case FLASH_IOC_SET_SW_DELAY:
		pr_info("FLASH_IOC_SET_SW_DELAY(%d,%d,%d): %d\n",
				type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
		if (fdev->ops) {
			if (fdev->ops->flashlight_ioctl(fdev->client, FLASH_IOC_SET_SW_DELAY,
				(unsigned long)&fl_dev_arg)) {
					pr_err("Failed to set level\n");
			}
		}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	dev_info(&fdev->client->dev,"channel:%d decouple:%d\n",
