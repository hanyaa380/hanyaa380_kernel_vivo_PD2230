...
/*
 * DW9800KAF voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "DW9800KAF_DRV"
...
static int s4DW9800KAF_ReadReg(unsigned short *a_pu2Result)
{
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		initdrv();
		ret = s4DW9800KAF_ReadReg(&InitPos);
	...
	if (s4AF_WriteReg((unsigned short)g_u4TargetPosition) == 0) {
		...
		LOG_INF("vivo move af to=%d", g_u4CurrPosition);
		spin_unlock(g_pAF_SpinLock);
	} else {
...
/* ////////////////////////////////////////////////////////////// */
long DW9800KAF_Ioctl(struct file *a_pstFile,
...
int DW9800KAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
int DW9800KAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int DW9800KAF_GetFileName(unsigned char *pFileName)
{
