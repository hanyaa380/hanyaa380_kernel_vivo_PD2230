...
static int s4AF_ReadReg(u16 a_uAddr, u16 *a_pu2Result)
{
	...
	if (i4RetValue < 0) {
		printk("[DW9781OISAF] I2C read - send failed!!\n");
		return -1;
	}

	i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, pBuff, 2);
	if (i4RetValue < 0) {
		printk("[DW9781OISAF] I2C read - recv failed!!\n");
		return -1;
	}

	*a_pu2Result = (((u16)pBuff[0]) << 8) |pBuff[1] ;
	printk("[DW9781OISAF] pBuff[1] = 0x%x  pBuff[2] = 0x%x  a_pu2Result =0x%x",pBuff[0],pBuff[1],*a_pu2Result);
...
static int s4AF_WriteReg(u16 a_u2Addr, u16 a_u2Data)
{
	...
	if (i4RetValue < 0) {
		printk("[DW9781OISAF] I2C write failed!!\n");
		return -1;
	}
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	printk("[DW9781OISAF] a_u4Position=%d\n", a_u4Position);
	if ((a_u4Position > g_u4AF_MACRO) || (a_u4Position < g_u4AF_INF)) {
		printk("[DW9781OISAF] out of range\n");
		return -EINVAL;
	}
	if (*g_pAF_Opened == 1) {
		...
		if (ret == 0) {
			printk("[DW9781OISAF] Init Pos %6d\n", InitPos);
	...
	printk("[DW9781OISAF] move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition); 
	...
	if (s4AF_WriteReg(0xD013, (unsigned short)g_u4TargetPosition) == 0) {
		...
		printk("[DW9781OISAF] set I2C failed when moving the motor\n");
		ret = -1;
	}
...
{
	printk("[DW9781OISAF] Start\n");

	if (*g_pAF_Opened == 2){

		printk("[DW9781OISAF] Wait\n");
		s4AF_WriteReg(0xD013,800);
		mdelay(4);
		s4AF_WriteReg(0xD013,750);
		mdelay(4);
		s4AF_WriteReg(0xD013,700);
		mdelay(4);
		s4AF_WriteReg(0xD013,650);
		mdelay(4);
		s4AF_WriteReg(0xD013,600);
		mdelay(4);
		s4AF_WriteReg(0xD013,550);
		mdelay(4);
	...
	if (*g_pAF_Opened) {
		printk("[DW9781OISAF] Free\n");

		spin_lock(g_pAF_SpinLock);
		*g_pAF_Opened = 0;
		spin_unlock(g_pAF_SpinLock);
	}
	printk("[DW9781OISAF] End\n");
