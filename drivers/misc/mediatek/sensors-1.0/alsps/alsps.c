...
int als_data_report_t(int value, int status, int64_t time_stamp)
{
	...
	/*Modify be vsensor team begin*/
	if (true/*value != last_als_report_data*/) {
		/*Modify by vsensor team end REASON:we need light sensor to continue report the data*/
		event.handle = ID_LIGHT;
		event.flush_action = DATA_ACTION;
		event.word[0] = value;
		event.status = status;
...
int ps_data_report_t(int value, int status, int64_t time_stamp)
{
	...
	pr_err("[ALS/PS]prox_data: %d, %d, now:%lld evt:%lld\n", distance, raw_data, ktime_get_boot_ns(), event.time_stamp);
