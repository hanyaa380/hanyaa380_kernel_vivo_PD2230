...
static void SCP_sensorHub_init_sensor_state(void)
{
	...
	mSensorState[SENSOR_TYPE_LCM_ESDCHECK].sensorType = SENSOR_TYPE_LCM_ESDCHECK;
	mSensorState[SENSOR_TYPE_LCM_ESDCHECK].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[SENSOR_TYPE_LCM_ESDCHECK].timestamp_filter = false;
...
static void sensorHub_shutdown(struct platform_device *pdev)
{
	...
	pr_err("sensorHub_shutdown!\n");
	cmd_args[0] = SENSOR_COMMAND_PROXIMITY_SHUTDOWN;
	ret = sensor_set_vsen_cmd_to_hub(ID_PROXIMITY, cmd_args, ARRAY_SIZE(cmd_args));
	if (ret < 0) {
