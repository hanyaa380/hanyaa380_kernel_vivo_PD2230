...
#if defined(CONFIG_WD_SBU_POLLING)
...
#if !defined(CONFIG_MACH_MT6877) && !defined(CONFIG_MACH_MT6893) && !defined(CONFIG_MACH_MT6833)
	if (of_device_is_compatible(dev->of_node, "mediatek,usb_type_c")) {
		dev_info(dev, "%s find node mediatek,usb_type_c\n", __func__);
		np = dev->of_node;
	} else {
#endif
		...
		dev_info(dev, "%s find node type_c_port0\n", __func__);
		dev->of_node = np;
#if !defined(CONFIG_MACH_MT6877) && !defined(CONFIG_MACH_MT6893) && !defined(CONFIG_MACH_MT6833)
	}
#endif
 ...
 */
#if defined(TCPC_ENABLE_ANYMSG) && !defined(CONFIG_MACH_MT6833)
