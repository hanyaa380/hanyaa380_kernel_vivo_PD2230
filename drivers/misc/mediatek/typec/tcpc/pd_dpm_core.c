...
enum PDMSG_UVDM_VIVO_CMD {
	PDMSG_UVDM_VIVO_CMD_VCONN_SWAP = 0x1,
	PDMSG_UVDM_VIVO_CMD_VCONN_RDY = 0x2,
	PDMSG_UVDM_VIVO_CMD_CABLE_INFO = 0x3,
};

bool vivo_ufp_notify_uvdm(struct pd_port *pd_port, struct svdm_svid_data *svid_data)
{
	uint32_t reply_cmd[3];
	uint32_t recv_code[3];
	uint32_t cmd = PD_UVDM_HDR_CMD(pd_port->uvdm_data[0]);
	struct tcpc_device *tcpc = pd_port->tcpc;

	DC_INFO("%s: dc_dfp_state:0x%x, cmd=0x%x\n", __func__, pd_port->dc_dfp_state, cmd);
	switch (pd_port->dc_dfp_state) {
	case PDMSG_UVDM_VIVO_CMD_VCONN_SWAP:
		recv_code[0] = pd_port->uvdm_data[0];
		recv_code[1] = pd_port->uvdm_data[1];
		DC_INFO("%s: VCONN_SWAP recv_code: 0x%x, 0x%x, vcon on\n", __func__, recv_code[0], recv_code[1]);

		pd_set_vconn(pd_port, PD_ROLE_VCONN_ON);

		reply_cmd[0] = PD_UVDM_HDR(USB_VID_VIVO, PDMSG_UVDM_VIVO_CMD_VCONN_RDY);
		pd_reply_custom_vdm(pd_port, TCPC_TX_SOP, 1, reply_cmd);
		msleep(100);
		tcpci_notify_uvdm(tcpc, true);
		pd_port->dc_dfp_state = PDMSG_UVDM_VIVO_CMD_CABLE_INFO;
		break;
	case PDMSG_UVDM_VIVO_CMD_CABLE_INFO:
		recv_code[0] = pd_port->uvdm_data[0];
		recv_code[1] = pd_port->uvdm_data[1];
		DC_INFO("%s: CABLE_INFO recv_code: 0x%x, 0x%x, vcon off\n", __func__, recv_code[0], recv_code[1]);

		reply_cmd[0] = PD_UVDM_HDR(USB_VID_VIVO, PDMSG_UVDM_VIVO_CMD_CABLE_INFO);
		pd_reply_custom_vdm(pd_port, TCPC_TX_SOP, 1, reply_cmd);
		pd_port->dc_dfp_state = DP_DFP_U_NONE;
...
static const struct svdm_svid_ops svdm_svid_ops[] = {
...
#if IS_ENABLED(CONFIG_VIVO_CHARGING_NEW_ARCH)
	{
		...
		.ufp_notify_uvdm = vivo_ufp_notify_uvdm,
	},
#endif
};
