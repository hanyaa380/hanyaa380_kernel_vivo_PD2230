	...
	*cc2 = TYPEC_CC_VOLT_OPEN;
	...
	if (chip->type_c_param.attach_state == CABLE_STATE_AS_UFP) {
		...
		if (chip->type_c_param.active_cable_attach) {
			// vivo panyaqin add begin
			// for some hub, the cc logic can't recognize the insert,
			// so will not notify usb switch to host
			*active_cc |= TYPEC_CC_VOLT_RD;
			// vivo panyaqin add end
		} else {
			*active_cc |= TYPEC_CC_VOLT_RD;
		}
 	}
				...
				const struct i2c_device_id *id)
{
	...
	/* set cc snk when init */
	sgm7220_update_reg(chip->client, REG_SET, SET_MODE_SELECT_SNK << SET_MODE_SELECT_SHIFT, SET_MODE_SELECT);
	sgm7220_update_reg(chip->client, REG_INT, 1, 1);
