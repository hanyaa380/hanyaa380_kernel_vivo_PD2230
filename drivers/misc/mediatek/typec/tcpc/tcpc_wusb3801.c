...
#endif	/* __TEST_CC_PATCH__ */
bool wusb_power_status_change = false;
...
static int wusb3801_get_cc(struct tcpc_device *tcpc, int *cc1, int *cc2)
{
	...
	int ret = 0, cc_orientation = 0, attach_state = 0, current_det = 0, active_cable_attach = 0;

	ret = wusb3801_i2c_read8(chip->tcpc, WUSB3801_REG_STATUS);
	if(ret < 0) {
		pr_err("%s: read reg fail!\n", __func__);
		return 0;
	}

	cc_orientation = (ret & 0x03);
	active_cc = (cc_orientation & 0x01)? cc1 : cc2;
	secondary_cc = (cc_orientation & 0x01)? cc2 : cc1;
	attach_state = (ret & 0x1c) >> 2;
	current_det = (ret & 0x60) >> 5;
	*active_cc = 0;
	*secondary_cc = 0;

	ret = wusb3801_i2c_read8(chip->tcpc, WUSB3801_REG_INTERRUPT);
	if(ret < 0) {
		pr_err("%s: read reg fail!\n", __func__);
		return 0;
	}
	active_cable_attach = (ret & 0x01);
	...
	if (attach_state == 2) {
		...
		if (active_cable_attach)
			*active_cc |= TYPEC_CC_VOLT_RA;
		else
			*active_cc |= TYPEC_CC_VOLT_RD;
	} else if (attach_state > 2) {
		switch (attach_state) {
			case 3:     //audio accessory
				*active_cc |= TYPEC_CC_VOLT_RA;
				*secondary_cc |= TYPEC_CC_VOLT_RA;
				break;
			case 4:     // debug accessory
				*active_cc |= TYPEC_CC_VOLT_RD;
...
static void wusb3801_irq_work_handler(struct kthread_work *work)
{
	...
	pr_info("%s WUSB3801_REG_STATUS : 0x%02x, int_sts[0x%02x]\n", __func__, rc, int_sts);
...
//				tcpci_report_usb_port_attached(chip->tcpc);
	...
	default:
	 ...
	 wusb3801_get_cc(chip->tcpc, &cc[0],  &cc[1]);
	 chip->tcpc->typec_remote_cc[0] = (uint8_t)cc[0];
	 chip->tcpc->typec_remote_cc[1] = (uint8_t)cc[1];
work_unlock:	
	tcpci_unlock_typec(tcpc);
	enable_irq(chip->irq);
}
	...
	*pwr_status = 0;
	...
	if (val & 0x01) {
		...
		*pwr_status |= TCPC_REG_POWER_STATUS_VBUS_PRES;
		wusb_power_status_change = false;
	}

#ifdef CONFIG_TCPC_VSAFE0V_DETECT_IC
	if ((wusb_power_status_change) && (val & 0x20)) {
...
static int wusb3801_set_cc(struct tcpc_device *tcpc, int pull)
{
	...
	ret &= 0xf9;
	ret = wusb3801_i2c_write8(chip->tcpc, WUSB3801_REG_CONTROL0, (value | ret));
	if (ret < 0) {
...
int wusb3801_vendor_defined_cmd(struct tcpc_device *tcpc, uint32_t mask)
{
	struct wusb3801_chip *chip = tcpc_get_dev_data(tcpc);

	if (mask == 1) {
		wusb_power_status_change = true;
	} else if (mask == 0) {
		wusb_power_status_change = false;
	} else if (mask == 2) {
		wusb_power_status_change = false;
...
static struct tcpc_ops wusb3801_tcpc_ops = {
    ...
    .vendor_defined_cmd = wusb3801_vendor_defined_cmd,
};
...
static int mt_parse_dt(struct wusb3801_chip *chip, struct device *dev)
{
	...
	if (!np) {
		pr_err("%s find node wusb3801_type_c_port0 fail\n", __func__);
		return -ENODEV;
	}
...
//huanglei add for reg 0x08& 0x0F write zero fail begin
    ...
    wusb3801_i2c_write8(chip->tcpc,
        WUSB3801_REG_TEST_09, 0x00);
	wusb3801_i2c_write8(chip->tcpc, WUSB3801_REG_CONTROL0, 0x80);
	wusb3801_i2c_read8(chip->tcpc, WUSB3801_REG_INTERRUPT);
//huanglei add for reg 0x08& 0x0F write zero fail end  
	pr_info("%s probe OK!\n", __func__);
	return 0;
...
static int __init wusb3801_init(void)
{
	...
	np = of_find_node_by_name(NULL, "wusb3801_usb_typec");
	if (np != NULL)
		pr_info("wusb3801_usb_typec node found...\n");
	else
		pr_info("wusb3801_usb_typec node not found...\n");
