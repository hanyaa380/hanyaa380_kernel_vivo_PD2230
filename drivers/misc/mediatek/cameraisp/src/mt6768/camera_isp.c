...
static unsigned int g_virtual_cq_cnt_a = 0;
static unsigned int g_virtual_cq_cnt_b = 0;
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
    ...
    case ISP_SET_VIR_CQCNT:
        if (copy_from_user(&temp, (void *)Param,
		    sizeof(unsigned int)*2) == 0) {
		...
		spin_lock_irqsave((spinlock_t *)(&virtual_cqcnt_lock), flags);
		g_virtual_cq_cnt[0] = temp[0];
		g_virtual_cq_cnt[1] = temp[1];
		...
		spin_unlock_irqrestore((spinlock_t *)(&virtual_cqcnt_lock), flags);
        break;	
	default:
	{
