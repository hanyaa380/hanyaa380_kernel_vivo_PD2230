...
/*chenhan add for ois*/
#if defined(CONFIG_MTK_CAM_PD2204F_EX) || defined(CONFIG_MTK_CAM_PD2282)
#include "ois_core.h"
...
irqreturn_t ISP_Irq_CAM(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	...
	/*chenhan  add for ois*/
    #if defined(CONFIG_MTK_CAM_PD2204F_EX) || defined(CONFIG_MTK_CAM_PD2282)
    struct ois_vsync_info VsyncInfo = { 0, };

    #endif
    /*add end*/
	//add by vivo zhangyinpeng for slowflash start
	#if defined(CONFIG_MTK_CAM_PD2282)
	struct sw_flash_data *slowflash;
	slowflash = mtk_cam_get_swflashdata();
	#endif
	//add by vivo zhangyinpeng for slowflash end
	/* Avoid touch hwmodule when clock is disable. */
	/* DEVAPC will moniter this kind of err */
...
/*chenhan add for ois*/
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		VsyncInfo.vsync_cnt = (u64)cur_v_cnt;
		VsyncInfo.sof_timestamp_boot = ktime_get_boot_ns();
		VsyncInfo.module_idx = (u64)module;
		ois_interface_dispatcher(AFIOC_X_OIS_VSYNC, (void *)(&VsyncInfo));
	}
#endif
/*add end*/
//add by vivo zhangyinpeng for slowflash start
#if defined(CONFIG_MTK_CAM_PD2282)
		//pr_info("[%s] trigger sw_flash cb (sof latecy:%lld ns), is_registered(%d)\n",
		//		__func__, ktime_get_boot_ns(), slowflash->is_registered);
		if((slowflash != NULL) && (slowflash->is_registered == true) && (slowflash->flash_func_cb != NULL)) {
			slowflash->flash_func_cb();
		}
#endif
//add by vivo zhangyinpeng for slowflash end
	spin_lock(&(IspInfo.SpinLockIrq[module]));
