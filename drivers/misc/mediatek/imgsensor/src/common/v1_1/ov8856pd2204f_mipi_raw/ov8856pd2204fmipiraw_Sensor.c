...
//#include "ov8856pd2204fmipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856PD2204F_SENSOR_ID,
	...
	.custom6 = { /*user for dualcam(sensor crop 2080*1560  1xFor Bokeh，For GW1P,)*/
		.pclk = 143870000, /*record different mode's pclk*/
		.linelength  = 1932, /*record different mode's linelength*/
		.framelength = 2482, /*record different mode's framelength*/
		.startx = 0, /*record different mode's startx of grabwindow*/
	...
	.custom7 = {/*user for dualcam(sensor crop 2080*1560  2xFor Bokeh，For GW1P,)*/
		.pclk = 143870000, /*record different mode's pclk*/
		.linelength  = 1932, /*record different mode's linelength*/
		.framelength = 2482, /*record different mode's framelength*/
		.startx = 0, /*record different mode's startx of grabwindow*/
		.starty = 0, /*record different mode's starty of grabwindow*/
		.grabwindow_width  = 1040,
		.grabwindow_height = 780,
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 300,
		.mipi_pixel_rate = 152530000,
		},
	.custom8 = {/*user for dualcam(sensor crop 2xFor Bokeh，For GW1P,)*/
		.pclk = 143870000, /*record different mode's pclk*/
		.linelength  = 1932, /*record different mode's linelength*/
		.framelength = 2482, /*record different mode's framelength*/
		.startx = 0, /*record different mode's startx of grabwindow*/
		.starty = 0, /*record different mode's starty of grabwindow*/
		.grabwindow_width  = 904,
		.grabwindow_height = 680,
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 300,
		.mipi_pixel_rate = 152530000,
		},
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[15] = {  
	...
	{ 3264, 2448, 	0,		0,	 3264, 2448, 3264, 2448, 0, 0, 3264, 2448,  0,  0, 3264, 2448}, // custom2->custom1  专业模式	 
	{ 3264, 2448, 	592,	444, 2080, 1560, 2080, 1560, 0, 0, 2080, 1560,  0,  0, 2080, 1560}, // custom1->custom2	   
	...
	{ 3264, 2448, 	592,	444, 2080, 1560, 2080, 1560, 0, 0, 2080, 1560,  0,  0, 2080, 1560}, // custom6
	{ 3264, 2448, 	1112,	834, 1040, 780, 1040, 780, 0, 0, 1040, 780,  0,  0, 1040, 780}, // custom7
	{ 3264, 2448, 	1180,	884, 904, 680, 904, 680, 0, 0, 904, 680,  0,  0, 904, 680}, // custom8
...
extern int vivo_main2_otp_read_ov8856pd2204f(void);
extern otp_error_code_t MAIN2_OTP_ERROR_CODE_OV8856PD2204F;
#endif
MUINT32  sn_inf_main2_ov8856pd2204f[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main2_ov8856pd2204f[4];  
...
static kal_uint16 addr_data_pair_custom6[] = {   /* 2080*1560   user for dualcam(sensor crop   1xFor Bokeh，For GW1P,)*/
	0x0302, 0x2f,
	0x0303, 0x00,
	0x3501, 0xc1,
	0x3502, 0x00,
...
static kal_uint16 addr_data_pair_custom7[] = {  //1040*780
	0x0302, 0x2c,
	0x0303, 0x00,
	0x3501, 0x4c,
	0x3502, 0xa0,
...
static kal_uint16 addr_data_pair_custom8[] = {  //904*680
	0x0302, 0x2c,
	0x0303, 0x00,
	0x3501, 0x4c,
	0x3502, 0xa0,
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor_16_8(0x300B) << 8) | read_cmos_sensor_16_8(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = vivo_main2_otp_read_ov8856pd2204f();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN2_885A_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, MAIN2_OTP_ERROR_CODE_OV8856PD2204F);
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = MAIN2_OTP_ERROR_CODE_OV8856PD2204F;//otp_state
				memcpy( feature_data+2, sn_inf_main2_ov8856pd2204f, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main2_ov8856pd2204f, sizeof(MUINT32)*4);
				#if 0
					for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main2_ov8856pd2204f[%d]= 0x%x\n", i, sn_inf_main2_ov8856pd2204f[i]);
					}
				#endif
			}
				break;
...
UINT32 OV8856PD2204F_MIPI_RAW_SensorInit(
