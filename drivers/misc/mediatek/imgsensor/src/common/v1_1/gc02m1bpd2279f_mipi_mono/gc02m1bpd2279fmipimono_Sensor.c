...
#include "gc02m1bpd2279fmipimono_Sensor.h"
...
extern otp_error_code_t GC02M1BPD2279F_OTP_ERROR_CODE;
MUINT32  sn_inf_main2_gc02m1bpd2279f[13];  /*0: flag   1-12: sndata*/
MUINT32 fpc_main2_gc02m1bpd2279f[4];
MUINT32  material_inf_main2_gc02m1bpd2279f[4];
MUINT32  module_inf_main2_gc02m1bpd2279f[2];
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = GC02M1BPD2279F_SENSOR_ID,
	...
	.custom1 = {
		...
		.startx = 0,	//record different mode's startx of grabwindow
		.starty = 0,	//record different mode's starty of grabwindow
		.grabwindow_width = 1216,	//record different mode's width of grabwindow //1296
		.grabwindow_height = 912,	//record different mode's height of grabwindow //972
		/*     following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario    */
		.mipi_data_lp2hs_settle_dc = 85,	//unit , ns
		/*     following for GetDefaultFramerateByScenario()    */
		.mipi_pixel_rate = 67200000,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 192, 144, 1216, 912},	/*custom1*/
};
#if 1
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x01, 0x6b);//6b
	...
	write_cmos_sensor(0xde, 0x1c);//1c
...
static void custom1_setting(void)
{
	...
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x91, 0x00);
	write_cmos_sensor(0x92, 0x98);
	write_cmos_sensor(0x93, 0x00);
	write_cmos_sensor(0x94, 0xC6);
	write_cmos_sensor(0x95, 0x03);
	write_cmos_sensor(0x96, 0x90);
	write_cmos_sensor(0x97, 0x04);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			//LOG_INF("info:hsz get_board_version()=%s,,[%c]",hw_version,hw_version[13]);
			LOG_INF("i2c write id: imgsensor_info.sensor_id:0x%x, sensor id: 0x%x,hw_version:[%c]\n",imgsensor_info.sensor_id, *sensor_id,hw_version[13]);

			if (*sensor_id == imgsensor_info.sensor_id && hw_version[13] == '1') {
...
static kal_uint32 open(void)
{
	...
	if (imgsensor_info.sensor_id != sensor_id ||  hw_version[13] != '1')
		return ERROR_SENSOR_CONNECT_FAIL;
				  ...
				  UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  MAIN_F8D1_OTP_ERROR_CODE:%d \n", *feature_data,GC02M1BPD2279F_OTP_ERROR_CODE);
		switch (*feature_data) {
			case 0:    //info type: otp state
			printk("*feature_para_len = %d, sizeof(MUINT32) * 23 + 2 =%ld, \n", *feature_para_len, sizeof(MUINT32) * 23 + 2);
			if (*feature_para_len >= sizeof(MUINT32) * 23 + 2) {
			    *(MUINT32 *)(uintptr_t)((feature_data+1)) = GC02M1BPD2279F_OTP_ERROR_CODE;//otp_state
				memcpy(feature_data + 2, sn_inf_main2_gc02m1bpd2279f, sizeof(MUINT32) * 13);
				memcpy(feature_data + 10, material_inf_main2_gc02m1bpd2279f, sizeof(MUINT32) * 4);
				memcpy(feature_data + 15, module_inf_main2_gc02m1bpd2279f, sizeof(MUINT32) * 2);
				memcpy(feature_data + 17, fpc_main2_gc02m1bpd2279f, sizeof(MUINT32) * 4);

				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_gc02m1bpd2279f[%d]= 0x%x\n", i, sn_inf_main_gc02m1bpd2279f[i]);
						}
				#endif
			}
				break;
...
UINT32 GC02M1BPD2279F_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
