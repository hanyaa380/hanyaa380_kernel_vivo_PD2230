 ...
 * Filename:
 * ---------
 *	 s5kjn1sqpd2279fmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kjn1sqpd2279fmipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1SQPD2279F_SENSOR_ID,
...
extern otp_error_code_t S5KJN1SQPD2279F_OTP_ERROR_CODE;
#endif
extern MUINT32 sn_inf_main_s5kjn1sqpd2279f[13];  /*0 flag   1-12 data*/
extern MUINT32 material_inf_main_s5kjn1sqpd2279f[4];
extern MUINT32 af_calib_inf_main_s5kjn1sqpd2279f[6];
extern unsigned char crosstalk_data_s5kjn1sqpd2279f[ FOUR_CELL_SIZE];  /*0 flag   1-836 data*/
extern MUINT32 module_inf_main_s5kjn1sqpd2279f[2];
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjn1sqpd2279f temperature
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJN1SQPD2279F_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjn1sqpd2279f, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjn1sqpd2279f, sizeof(MUINT32)*4);
				memcpy( feature_data+12, af_calib_inf_main_s5kjn1sqpd2279f, sizeof(MUINT32)*6);
				memcpy( feature_data+15, module_inf_main_s5kjn1sqpd2279f, sizeof(MUINT32)*2);
			#if 0
				for (i = 0 ; i<12 ; i++ ){
					printk("sn_inf_main_s5kjn1sqpd2279f[%d]= 0x%x\n", i, sn_inf_main_s5kjn1sqpd2279f[i]);
				}
			#endif
			}
				break;
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/* get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			for(i = 0; i< FOUR_CELL_SIZE; i++)
				data[i+2] = crosstalk_data_s5kjn1sqpd2279f[i];
...
UINT32 S5KJN1SQPD2279F_MIPI_RAW_SensorInit(
