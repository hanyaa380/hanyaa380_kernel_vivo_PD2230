 ...
 * Filename:
 * ---------
 *	 s5kgw3sp13pd2215fmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kgw3sp13pd2215fmipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KGW3SP13PD2215F_SENSOR_ID,
	...
	.hs_video = { /*hs_video 120fps*/
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 600000000, //1848
		.max_framerate = 1200,
		},
	.slim_video = { /* hs_video 240fps*/
		...
		.grabwindow_height = 1080,
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 720000000, //1848
		.max_framerate = 2400,
		},
	.custom1 = { /*4032x3024@30.09fps*/
		.pclk =797330000,
		.linelength = 7360,
		.framelength = 3600,
		.startx =0,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {  
	...
	{9216, 6912,  576,  432, 8064, 6048, 4032, 3024, 0, 0, 4032, 3024, 0, 0, 4032, 3024}, /* custom1 dual_cam */
...
{/* Preview mode setting 4608*3456*/
	...
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 ...
	 /* custom1 mode setting 4032*3024 30fps*/
	{0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x1200, 0x0D70, 0x00, 0x00, 0x00, 0x00,
	 0x01, 0x30, 0x0276, 0x0BC0, 0x00, 0x00, 0x0000, 0x0000},
};
...
extern int MAIN_7309_otp_read_gw3_pd2215f(void);
extern otp_error_code_t S5KGW3SP13PD2215F_OTP_ERROR_CODE;
MUINT32  sn_inf_main_s5kgw3sp13pd2215f[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kgw3sp13pd2215f[4];
MUINT32  af_calib_inf_main_s5kgw3sp13pd2215f[6];
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000) ;
			...
			if ( *sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = MAIN_7309_otp_read_gw3_pd2215f();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN_S5KGW3_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, S5KGW3SP13PD2215F_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kgw3sp13pd2215f temperature 
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		switch (*feature_data) {
			...
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				...
				break;
			case MSDK_SCENARIO_ID_CUSTOM1:
				imgsensor_pd_info.i4BlockNumX = 504;
				imgsensor_pd_info.i4BlockNumY = 376;
				memcpy((void *)PDAFinfo,(void *)&imgsensor_pd_info,sizeof(struct SET_PD_BLOCK_INFO_T));
				break;
            case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			case MSDK_SCENARIO_ID_CUSTOM4:
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)((feature_data+1)) = S5KGW3SP13PD2215F_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kgw3sp13pd2215f, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_s5kgw3sp13pd2215f, sizeof(MUINT32)*4); 
				memcpy( feature_data+12, af_calib_inf_main_s5kgw3sp13pd2215f, sizeof(MUINT32)*6);
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						PK_DBG("sn_inf_main_s5kgw3sp13pd2215f[%d]= 0x%x\n", i, sn_inf_main_s5kgw3sp13pd2215f[i]);
						}
				#endif
			}
				break;
...
UINT32 S5KGW3SP13PD2215F_MIPI_RAW_SensorInit(
