...
#define PFX "MAIN[38EE]_s5kjnspd2279"
...
#include "s5kjnspd2279mipiraw_freq.h"
...
extern bool s5kjnspd2279_vivo_read_eeprom(kal_uint16 addr,  BYTE *data);

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJNSPD2279_SENSOR_ID,
...
extern int s5kjnspd2279_MAIN_38EE_otp_read(void);
extern otp_error_code_t S5KJNSPD2279_OTP_ERROR_CODE;
#endif
extern MUINT32 sn_inf_main_s5kjnspd2279[13];  /*0 flag   1-12 data*/
extern MUINT32 material_inf_main_s5kjnspd2279[4];
extern MUINT32 af_calib_inf_main_s5kjnspd2279[6];
extern unsigned char crosstalk_data_s5kjnspd2279[ FOUR_CELL_SIZE];  /*0 flag   1-836 data*/
extern MUINT32 module_inf_main_s5kjnspd2279[2];
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		s5kjnspd2279_vivo_read_eeprom(0x0001, module_id);
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000);
			...
			if (( *sensor_id == imgsensor_info.sensor_id) || ( *sensor_id == 0xee38)) { //compatible jnv\jn1
				...
				vivo_otp_read_when_power_on = s5kjnspd2279_MAIN_38EE_otp_read();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN_S5KJN1_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, S5KJNSPD2279_OTP_ERROR_CODE);
				/*vivo lxd add end*/
#endif
				return ERROR_NONE;
			}
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJNSPD2279_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjnspd2279, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjnspd2279, sizeof(MUINT32)*4);
				memcpy( feature_data+12, af_calib_inf_main_s5kjnspd2279, sizeof(MUINT32)*6);
				memcpy( feature_data+15, module_inf_main_s5kjnspd2279, sizeof(MUINT32)*2);
			#if 0
				for (i = 0 ; i<12 ; i++ ){
					PK_DBG("sn_inf_main_s5kjnspd2279[%d]= 0x%x\n", i, sn_inf_main_s5kjnspd2279[i]);
				}
			#endif
			}
				break;
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/* get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			for(i = 0; i< FOUR_CELL_SIZE; i++)
				data[i+2] = crosstalk_data_s5kjnspd2279[i];
...
UINT32 S5KJNSPD2279_MIPI_RAW_SensorInit(
