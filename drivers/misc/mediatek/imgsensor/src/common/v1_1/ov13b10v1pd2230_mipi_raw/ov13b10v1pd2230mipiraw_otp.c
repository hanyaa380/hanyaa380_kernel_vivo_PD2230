...
#define PFX "OV13B10V1PD2230_EEPROM_OTP"
...
static unsigned const int AF_inf_golden_158 = 330;
static unsigned const int AF_mac_golden_158 = 630;
#endif
static int checksum = 0;
otp_error_code_t OV13B10V1PD2230_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_main_ov13b10v1pd2230[13];  /*0 flag   1-12 data*/
extern MUINT32  material_inf_main_ov13b10v1pd2230[4];
extern MUINT32  module_inf_main_ov13b10v1pd2230[2];
...
int ov13b10v1pd2230_vivo_otp_read(void)
{
	int i = 0;
	...
	OV13B10V1PD2230_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		...
		{
			OV13B10V1PD2230_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		...
		if( vivo_otp_data[ModuleInfo_checksum_addr] != checksum ) {
			OV13B10V1PD2230_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("ModuleInfo_checksum error!!!   otp_error_code:%d\n",OV13B10V1PD2230_OTP_ERROR_CODE);
			return 0;
		}

		/****module info start****/
		module_inf_main_ov13b10v1pd2230[0] = vivo_otp_data[0x01];
		/****module info end****/

		/****material info start****/
			material_inf_main_ov13b10v1pd2230[0] = (MUINT32)vivo_otp_data[0x10];
			material_inf_main_ov13b10v1pd2230[1] = (MUINT32)vivo_otp_data[0x11];
			material_inf_main_ov13b10v1pd2230[2] = (MUINT32)vivo_otp_data[0x12];
			material_inf_main_ov13b10v1pd2230[3] = (MUINT32)vivo_otp_data[0x13];
			LOG_INF("material_inf_main_ov13b10v1pd2230[0] = 0x%x, material_inf_main_ov13b10v1pd2230[1] = 0x%x, material_inf_main_ov13b10v1pd2230[2] = 0x%x,material_inf_main_ov13b10v1pd2230[3] = 0x%x\n",
				material_inf_main_ov13b10v1pd2230[0]  , material_inf_main_ov13b10v1pd2230[1],  material_inf_main_ov13b10v1pd2230[2], material_inf_main_ov13b10v1pd2230[3]);
		/****material info end****/
		//////fuseIDInfo_checksum
		checksum = 0;
		...
		sn_inf_main_ov13b10v1pd2230[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_ov13b10v1pd2230[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_main_ov13b10v1pd2230[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_ov13b10v1pd2230[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
		...
		if(t > 0) {
			OV13B10V1PD2230_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, OV13B10V1PD2230_OTP_ERROR_CODE);
			return 0;
		}
		...
		if(vivo_otp_data[0x01] == 0x05) {
			...
			if (diff_inf_137 > 150 || diff_mac_137 > 150 || diff_inf_macro > 450 || diff_inf_macro < 150) {  /*AF code out of range*/
				OV13B10V1PD2230_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
				LOG_INF("QTech 2311137 AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV13B10V1PD2230_OTP_ERROR_CODE, vivo_otp_data[0x0A], AF_inf, AF_mac);
				return 0;
			}
		} else {
		...
		OV13B10V1PD2230_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
