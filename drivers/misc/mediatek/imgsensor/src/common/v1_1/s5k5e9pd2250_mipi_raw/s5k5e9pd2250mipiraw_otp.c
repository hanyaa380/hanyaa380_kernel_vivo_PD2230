...
#include "s5k5e9pd2250mipiraw_Sensor.h"

#define PFX "S5K5E9PD2250_EEPROM_OTP"
...
otp_error_code_t S5K5E9PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int S5K5E9PD2250_flag;
extern MUINT32  sn_inf_sub_s5k5e9pd2250[13];  /*0 flag   1-12 data*/
extern MUINT32  material_inf_sub_s5k5e9pd2250[4];  /*0 flag   1-12 data*/
...
int S5K5E9PD2250_otp_read(void)
{
	int i = 0;
	...
	S5K5E9PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
...
/*
	if (vivo_otp_data[0x09] != 0x00 ){ //0x09 vcm id check
		S5K5E9PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		S5K5E9PD2250_flag = 0;
		LOG_INF("VCM ID Error!!! S5K5E9PD2250_flag =%d\n", S5K5E9PD2250_flag);
	}else{
		S5K5E9PD2250_flag = 1;
		LOG_INF("S5K5E9PD2250_flag = %d,VCM ID = 0x%x", S5K5E9PD2250_flag,vivo_otp_data[0x09]);
	}
*/
	if (check_if_group_valid == 0x01) {//all the data is valid
		...
		if (vivo_otp_data[ModuleInfo_checksum_addr] != checksum) {
			S5K5E9PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
            LOG_INF("ModuleInfo_checksum error!!! otp_error_code:%d\n", S5K5E9PD2250_OTP_ERROR_CODE);
			return 0;
        }

		/****material info start****/
		material_inf_sub_s5k5e9pd2250[0] = (MUINT32)vivo_otp_data[0x0F];
		material_inf_sub_s5k5e9pd2250[1] = (MUINT32)vivo_otp_data[0x10];
		material_inf_sub_s5k5e9pd2250[2] = (MUINT32)vivo_otp_data[0x11];
		material_inf_sub_s5k5e9pd2250[3] = (MUINT32)vivo_otp_data[0x12];
		LOG_INF("material_inf_sub_s5k5e9pd2250[0] = 0x%x, material_inf_sub_s5k5e9pd2250[1] = 0x%x, material_inf_sub_s5k5e9pd2250[2] = 0x%x, material_inf_sub_s5k5e9pd2250[3] = 0x%x\n",
			material_inf_sub_s5k5e9pd2250[0], material_inf_sub_s5k5e9pd2250[1],  material_inf_sub_s5k5e9pd2250[2], material_inf_sub_s5k5e9pd2250[3]);
		/****material info end****/
		//fuseIDInfo_checksum
		checksum = 0;
		...
		sn_inf_sub_s5k5e9pd2250[0] = 0x01;
		for(i = 0; i < 12; i++) {
			sn_inf_sub_s5k5e9pd2250[i+1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_sub_s5k5e9pd2250[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i + 1, sn_inf_sub_s5k5e9pd2250[i+1], i + SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//AWB_checksum
		checksum = 0;
		...
		if(t > 0) {
			S5K5E9PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, S5K5E9PD2250_OTP_ERROR_CODE);
			return 0;
		}
		return 1;
	} else {
		S5K5E9PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
