...
#define PFX "MAIN2[0846]_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846PD2215F_SENSOR_ID,
	...
	.custom2 = {
		...
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 3264,   /*2592 record different mode's width of grabwindow*/
		.grabwindow_height = 1836,	 /*1944record different mode's height of grabwindow*/
		/*	 following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario	*/
		.mipi_data_lp2hs_settle_dc = 14,	
		.mipi_pixel_rate = 287700000,
		.max_framerate = 300,
...
extern int vivo_main2_otp_read_hi846pd2215f(void);
int vivo_main2_otp_read_when_power_on_hi846pd2215f;
extern otp_error_code_t MAIN2_OTP_ERROR_CODE_HI846PD2215F;
MUINT32  sn_inf_main2_hi846pd2215f[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_hi846pd2215f[4];  
...
{
    char puSendCmd[I2C_BURST_LEN] = {0};
    kal_uint32 i, tosend;
    kal_uint16 addr = 0;
    kal_uint16 data = 0;
    ...
    while (IDX < size) {
        ...
        if (num_consecutive>1) {
            ...
            transfer_length = num_consecutive*2 + 2;
...
static void write_shutter(kal_uint32 shutter)
{
	...
	if (imgsensor.autoflicker_en) {
      ...
      write_cmos_sensor_16_16(0x0006, imgsensor.frame_length);
		...
		/* Extend frame length */
    write_cmos_sensor_16_16(0x0006, imgsensor.frame_length);
		PK_DBGBUG("(else)imgsensor.frame_length = %d\n",
...
{
#if 0
	PK_DBGBUG("le:0x%x, se:0x%x, gain:0x%x\n",le,se,gain);
#endif
}
#endif
			...
			do {
				*sensor_id = return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {				
					...
					sensor_init();
					/*vivo caofeixiang add for CameraEM otp errorcode*/
					PK_DBGBUG("cfx_add:start read eeprom ---vivo_main2_otp_read_when_power_on_pd2215f = %d\n", vivo_main2_otp_read_when_power_on_hi846pd2215f);
					vivo_main2_otp_read_when_power_on_hi846pd2215f = vivo_main2_otp_read_hi846pd2215f();
					PK_DBGBUG("cfx_add:end read eeprom ---vivo_main2_otp_read_when_power_on = %d,MAIN2_OTP_ERROR_CODE_HI846PD2215F =%d\n", vivo_main2_otp_read_when_power_on_hi846pd2215f,MAIN2_OTP_ERROR_CODE_HI846PD2215F);
					/*vivo caofeixiang add end*/
					return ERROR_NONE;
				}	
				PK_DBGBUG("get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
	...
	PK_DBGBUG("[open]: PLATFORM:VIVO,MIPI 24LANE\n");
	PK_DBGBUG("preview 1280*960@30fps,360Mbps/lane; capture 1560*1920@30fps,880Mbps/lane\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			...
			PK_DBGBUG("open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
{
	PK_DBGBUG("enable: %d", enable);

	if (enable) { 
		  PK_DBGBUG("enter color bar");            
...
// vivo add rawinfo
        ...
        case SENSOR_FEATURE_GET_CUSTOM_INFO:
		    PK_DBGBUG("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  MAIN2_OTP_ERROR_CODE_HI846PD2215F:%d \n", *feature_data,MAIN2_OTP_ERROR_CODE_HI846PD2215F);
			switch (*feature_data) {
				...
				if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				    *(MUINT32 *)(uintptr_t)((feature_data+1)) = MAIN2_OTP_ERROR_CODE_HI846PD2215F;//otp_state
					memcpy( feature_data+2, sn_inf_main2_hi846pd2215f, sizeof(MUINT32)*13); 
				    memcpy( feature_data+10, material_inf_main_hi846pd2215f, sizeof(MUINT32)*4); 
					#if 0
							for (i = 0 ; i<12 ; i++ ){
							printk("sn_inf_main2_hi846pd2215f[%d]= 0x%x\n", i, sn_inf_main2_hi846pd2215f[i]);
							}
...
UINT32 HI846PD2215F_MIPI_RAW_SensorInit(
