...
#define PFX "S5K4H7V3PD2166G_EEPROM_OTP"
...
#define VIVO_OTP_DATA_SIZE 0x0F31 /*sizeof(moduleinfo)+sizeof(awb)+sizeof(af)+sizeof(lsc)*/
...
#define VIVO_MAX_OFFSET 0x0F30
#define VIVO_VENDOR_SUNNY 0x07
...
static unsigned const int Awb_addr = 0x009c;
static unsigned const int Awb_checksum_addr = 0x00D1;
static unsigned const int Lsc_addr = 0x07D3;
static unsigned const int Lsc_checksum_addr = 0x0F30;


static int checksum = 0;
otp_error_code_t S5K4H7V3PD2166G_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int S5K4H7V3PD2166G_flag;
extern MUINT32  sn_inf_sub_s5k4h7v3pd2166g[13];  /*0 flag   1-12 data*/
extern MUINT32  material_inf_sub_s5k4h7v3pd2166g[4];  /*0 flag   1-12 data*/
bool s5k4h7v3pd2166g_vivo_read_eeprom(kal_uint16 addr,  BYTE *data)
{
...
int S5K4H7V3PD2166G_otp_read(void)
{
	int i = 0;
	...
	S5K4H7V3PD2166G_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	for(i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		if(!s5k4h7v3pd2166g_vivo_read_eeprom(offset, &vivo_otp_data[i])){
	...
	if (vivo_otp_data[0x09] != 0x00 ){
		S5K4H7V3PD2166G_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		S5K4H7V3PD2166G_flag = 0;
		LOG_INF("VCM ID Error!!!    S5K4H7V3PD2166G_flag =%d\n", S5K4H7V3PD2166G_flag);
	}else{
		S5K4H7V3PD2166G_flag = 1;
		LOG_INF("S5K4H7V3PD2166G_flag = %d,VCM ID = 0x%x", S5K4H7V3PD2166G_flag,vivo_otp_data[0x09]);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		/////ModuleInfo 
#if 0
		if((VIVO_VENDOR_SUNNY != vivo_otp_data[0x08]) && (VIVO_VENDOR_TRULY != vivo_otp_data[0x01]))
		{
			S5K4H7V3PD2166G_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n",S5K4H7V3PD2166G_OTP_ERROR_CODE);
			return 0;
		} else if ((vivo_otp_data[0x02] != 0x03) ||(vivo_otp_data[0x08] != 0x07) ||(vivo_otp_data[0x07] != 0x7B) ||(vivo_otp_data[0x06] != 0x48)|| (vivo_otp_data[0x09] != 0x00) || (vivo_otp_data[0x0A] != 0x00) || (vivo_otp_data[0x0B] != 0xFF))
		{
        ...
        {
			S5K4H7V3PD2166G_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
            LOG_INF("ModuleInfo_checksum error!!!   otp_error_code:%d\n",S5K4H7V3PD2166G_OTP_ERROR_CODE);
			return 0;
        }

		/****material info start****/
			material_inf_sub_s5k4h7v3pd2166g[0] = (MUINT32)vivo_otp_data[0x0F];
			material_inf_sub_s5k4h7v3pd2166g[1] = (MUINT32)vivo_otp_data[0x10];
			material_inf_sub_s5k4h7v3pd2166g[2] = (MUINT32)vivo_otp_data[0x11];
			material_inf_sub_s5k4h7v3pd2166g[3] = (MUINT32)vivo_otp_data[0x12];
			LOG_INF("material_inf_sub_s5k4h7v3pd2166g[0] = 0x%x, material_inf_sub_s5k4h7v3pd2166g[1] = 0x%x, material_inf_sub_s5k4h7v3pd2166g[2] = 0x%x,material_inf_sub_s5k4h7v3pd2166g[3] = 0x%x\n",
				material_inf_sub_s5k4h7v3pd2166g[0]  , material_inf_sub_s5k4h7v3pd2166g[1],  material_inf_sub_s5k4h7v3pd2166g[2], material_inf_sub_s5k4h7v3pd2166g[3]);
		/****material info end****/
		...
		sn_inf_sub_s5k4h7v3pd2166g[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_sub_s5k4h7v3pd2166g[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_sub_s5k4h7v3pd2166g[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_sub_s5k4h7v3pd2166g[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
		...
		if(t > 0) 
		{
			S5K4H7V3PD2166G_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, S5K4H7V3PD2166G_OTP_ERROR_CODE);
			return 0;
		}
		...
		S5K4H7V3PD2166G_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
