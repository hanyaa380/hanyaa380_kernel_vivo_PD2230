...
#include "gc02m1pd2250mipiraw_Sensor.h"
...
unsigned char vivo_otp_data_gc02m1pd2250[VIVO_OTP_DATA_SIZE];
...
otp_error_code_t GC02M1PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_main2_gc02m1pd2250[13];
extern MUINT32  material_inf_main2_gc02m1pd2250[4];
...
int MAIN2_2e0_otp_read(void)
{
	...
	GC02M1PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (check_if_group_valid != 0x00) {
		if ((vivo_otp_data_temp[0x00] & 0x0C) == 0x04)
			vivo_otp_data_gc02m1pd2250[0x00] = 0x35;
		else if ((vivo_otp_data_temp[0x00] & 0x0C) == 0x08)
			vivo_otp_data_gc02m1pd2250[0x00] = 0x36;
		else
			LOG_INF("vivo otp no info\n");

		vivo_otp_data_gc02m1pd2250[0x01] = char_to_hex(vendor[vivo_otp_data_temp[0x00] >> 4]);
		vivo_otp_data_gc02m1pd2250[0x02] = vivo_otp_data_temp[0x01];
		vivo_otp_data_gc02m1pd2250[0x03] = vivo_otp_data_temp[0x02];
		vivo_otp_data_gc02m1pd2250[0x04] = (vivo_otp_data_temp[0x03] >> 4) + 0x30;
		vivo_otp_data_gc02m1pd2250[0x05] = char_to_hex(month[vivo_otp_data_temp[0x03] & 0x0F]);
		vivo_otp_data_gc02m1pd2250[0x06] = vivo_otp_data_temp[0x04];
		vivo_otp_data_gc02m1pd2250[0x07] = vivo_otp_data_temp[0x05];
		vivo_otp_data_gc02m1pd2250[0x08] = vivo_otp_data_temp[0x06];
		vivo_otp_data_gc02m1pd2250[0x09] = vivo_otp_data_temp[0x07];
		vivo_otp_data_gc02m1pd2250[0x0A] = vivo_otp_data_temp[0x08];
		vivo_otp_data_gc02m1pd2250[0x0B] = vivo_otp_data_temp[0x09];

		if (check_if_group_valid == 0x01) {
			vivo_otp_data_gc02m1pd2250[0x0C] = vivo_otp_data_temp[0x0A];
			vivo_otp_data_gc02m1pd2250[0x0D] = vivo_otp_data_temp[0x0B];
			vivo_otp_data_gc02m1pd2250[0x0E] = vivo_otp_data_temp[0x0C];
		} else if (check_if_group_valid == 0x02) {
			vivo_otp_data_gc02m1pd2250[0x0C] = vivo_otp_data_temp[0x0D];
			vivo_otp_data_gc02m1pd2250[0x0D] = vivo_otp_data_temp[0x0E];
			vivo_otp_data_gc02m1pd2250[0x0E] = vivo_otp_data_temp[0x0F];
		}

		vivo_otp_data_gc02m1pd2250[0x0F] = vivo_otp_data_temp[0x10];
		vivo_otp_data_gc02m1pd2250[0x10] = 0xFF;
		vivo_otp_data_gc02m1pd2250[0x11] = 0x00;
		vivo_otp_data_gc02m1pd2250[0x12] = 0x0B;
		vivo_otp_data_gc02m1pd2250[0x13] = 0x01;

		/****material info start****/
		material_inf_main2_gc02m1pd2250[0] = (MUINT32)vivo_otp_data_gc02m1pd2250[0x00];
		material_inf_main2_gc02m1pd2250[1] = (MUINT32)vivo_otp_data_gc02m1pd2250[0x01];
		material_inf_main2_gc02m1pd2250[2] = (MUINT32)vivo_otp_data_gc02m1pd2250[0x02];
		material_inf_main2_gc02m1pd2250[3] = (MUINT32)vivo_otp_data_gc02m1pd2250[0x03];
		/*LOG_INF("material_inf_main2_gc02m1pd2250[0] = 0x%x, material_inf_main2_gc02m1pd2250[1] = 0x%x, material_inf_main2_gc02m1pd2250[2] = 0x%x, material_inf_main2_gc02m1pd2250[2] = 0x%x\n",
			material_inf_main2_gc02m1pd2250[0], material_inf_main2_gc02m1pd2250[1], material_inf_main2_gc02m1pd2250[2], material_inf_main2_gc02m1pd2250[3]);*/
		/****material info end****/

		/****sn info start****/
		sn_inf_main2_gc02m1pd2250[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_main2_gc02m1pd2250[i+1] = (MUINT32)vivo_otp_data_gc02m1pd2250[i + SN_addr];
			//LOG_INF("sn_inf_main2_gc02m1pd2250[%d] = 0x%x, vivo_otp_data_gc02m1pd2250[0x%x] = 0x%x\n", i+1  , sn_inf_main2_gc02m1pd2250[i+1],  i +SN_addr+1, vivo_otp_data_gc02m1pd2250[i+SN_addr+1]);
		}
		/****sn info end****/

		/****check if awb out of range[5000K high  cct]****/
		R_unit = ((vivo_otp_data_gc02m1pd2250[Awb_addr] & 0xF0) << 4) | (vivo_otp_data_gc02m1pd2250[Awb_addr+2]);
		B_unit = ((vivo_otp_data_gc02m1pd2250[Awb_addr] & 0x0F) << 8) | (vivo_otp_data_gc02m1pd2250[Awb_addr+2]);
		G_unit = 1024;
		...
		if (t > 0) {
			GC02M1PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, GC02M1PD2250_OTP_ERROR_CODE);
			return 0;
		}
		GC02M1PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
		return 1;
		...
		checksum = 0;
		GC02M1PD2250_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
		LOG_INF("invalid otp data. error!!! otp_error_code:%d\n", GC02M1PD2250_OTP_ERROR_CODE);
		return 0;
	}
}
/*vivo lxd add end*/
