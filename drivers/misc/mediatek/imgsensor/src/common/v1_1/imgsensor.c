...
#define MAX_NUM_OF_SUPPORT_MODULEID 9
extern void *find_OTP_by_idx(unsigned char idx);
#endif
...
#if defined(CONFIG_MTK_CAM_PD2257F_EX)
/************************************************************************
 * imgsensor_analyseOTPtype
 ************************************************************************/
struct MODULE_ID moduleid_List[MAX_NUM_OF_SUPPORT_MODULEID] = {
    {0x01 , "Sunny"},
    {0x02 , "Truly"},
    {0x03 , "Sunwin"},
    {0x04 , "shine"},
...
{//hushufen add for otp remake B221117-3674
	struct IMGSENSOR_SENSOR_TABLEINFO *ptableInfo =
		psensor_inst->psensor_tableInfo;
	...
	if (ptableInfo == NULL) {
		...
		PK_DBG("[%s]EEpInfo/tableInfo is exist,idx:%02x\n", __func__,
		       idx);
	}
	...
	if (test_i2c_is_timeout_otp(ptableInfo->OTP_slave_addr ,ptableInfo->ModuleInfo_addr + ptableInfo->OTP_addr_offset)) {
		PK_DBG("[%s]eeprom i2c read 0x%02x failed\n", __func__,
		       ptableInfo->OTP_slave_addr);
		...
		if (ptableInfo->CloudFlag_addr ==
		    0) { // Usually,the cloudflag_addr is 0x13
			otp_read(ptableInfo->OTP_addr_offset +
					 OTP_CLOUD_FLAG_ADDRSHIFT,
				 1, &otp_cloud_flag,
				 ptableInfo->OTP_slave_addr);
		} else { // Compatible with new modules
			otp_read(ptableInfo->OTP_addr_offset +
					 ptableInfo->CloudFlag_addr,
				 1, &otp_cloud_flag,
				 ptableInfo->OTP_slave_addr);
		}
		...
		if (ptableInfo->Af_checksum_addr > ptableInfo->Af_addr) {
			length = ptableInfo->Af_checksum_addr -
				 ptableInfo->Af_addr + 1;
			...
			if (buff != NULL) {
				...
				if (buff[0] == 1) { // is vaild
					...
					if (sum == buff[length - 1]) {
						if (ptableInfo
							    ->Af_StartCode_addr ==
						    0) { // the read rule for normal module which usually hasn`t
							// af_Mid_dis.
							ptableInfo->Af_info[0] =
								buff[7];
							ptableInfo->Af_info[1] =
								buff[8];
							ptableInfo->Af_info[2] =
								buff[9];
							ptableInfo->Af_info[3] =
								buff[10];
							ptableInfo->Af_info[4] =
								buff[11];
							ptableInfo->Af_info[5] =
								buff[12];
							PK_DBG("[%s]afcode:%02x%02x%02x%02x%02x%02x\n",
							       __func__,
							       ptableInfo->Af_info
								       [0],
							       ptableInfo->Af_info
							...
							if ((ptableInfo
							     ...
							     ptableInfo->Af_addr +
								     length -
								     1) && // the read rule for the modulewhich has af_Mid_dis.
							    (ptableInfo
								     ->Af_InfCode_addr <
							     ptableInfo->Af_addr +
								     length -
								...
								PK_DBG("[%s]SPeeprom%d value illegal\n",
								       __func__,
								       idx);
							}
							PK_DBG("[%s]af code with mid_dis:%02x%02x%02x%02x%02x%02x\n",
							       __func__,
							       ptableInfo->Af_info
								       [0],
							       ptableInfo->Af_info
		...
		// vivo lishuo add for ModuleId、SN and materail code,hardcore FIX ME ,begin
		buff = kzalloc(1, GFP_KERNEL);
		otp_read(ptableInfo->ModuleInfo_addr + OTP_ModuleId_addr, 1, buff, ptableInfo->OTP_slave_addr);
		// write ModuleId info node
		PK_DBG("[%s]ZRY buff[0] = %d\n", __func__, buff[0]);
		for (j = 0;j < MAX_NUM_OF_SUPPORT_MODULEID;j++)
		{
			if (buff[0] == moduleid_List[j].value)
			{
				sprintf((char *)ptableInfo->ModuleId_inf,"%s",moduleid_List[j].name);
				break;
			}
		}
		if (j == MAX_NUM_OF_SUPPORT_MODULEID)
		{
			sprintf((char *)ptableInfo->ModuleId_inf,"%s","NULL");
		}
		kfree(buff);
		PK_DBG("[%s]ZRY ModuleId_inf = %s\n", __func__, ptableInfo->ModuleId_inf);
		if (ptableInfo->SN_checksum_addr > ptableInfo->SN_addr) {
			length = ptableInfo->SN_checksum_addr -
				 ptableInfo->SN_addr + 1;
			buff = kzalloc(length, GFP_KERNEL);
			if (buff != NULL) {
				if (length >= 13) {
					otp_read(ptableInfo->OTP_addr_offset +
					...
					if (buff[0] == 1) { // is vaild
						...
						if (sum == buff[length - 1]) {
							sprintf((char *)ptableInfo
								...
								buff[11],
								buff[12]);
							PK_DBG("[%s]get sn_inf---%s\n",
							       __func__,
							       ptableInfo
								       ->SN_inf);
						} else {
							PK_DBG("[%s]sn info checksum failed\n",
							       __func__);
						}
					}
				} else {
					PK_DBG("[%s]sn addr maybe wrong, length:%d\n",
					       __func__, length);
				}
				kfree(buff);
			} else {
				PK_DBG("[%s]%d alloc failed\n", __LINE__);
			}
		} else {
			PK_DBG("[%s]no SN code\n", __func__);
		}

		if (ptableInfo->ModuleInfo_checksum_addr >
		    ptableInfo->ModuleInfo_addr) {
			length = ptableInfo->ModuleInfo_checksum_addr -
				 ptableInfo->ModuleInfo_addr + 1;
			buff = kzalloc(length, GFP_KERNEL);
			if (buff != NULL) {
				if (ptableInfo->ModuleInfo_checksum_addr >=
				    ptableInfo->Material_inf_addr + 4) {
					otp_read(
						ptableInfo->OTP_addr_offset +
							ptableInfo
								->ModuleInfo_addr,
						length, buff,
						ptableInfo->OTP_slave_addr);
							...
							sprintf((char *)ptableInfo
								...
								buff[ptableInfo
								     ...
								     ptableInfo
									     ->ModuleInfo_addr +
								     3]);
							PK_DBG("[%s]get material_inf---%s\n",
							       __func__,
							       ptableInfo
								       ->Material_inf);
						} else {
							PK_DBG("[%s]material info checksum failed\n",
							       __func__);
						}
					}
				} else {
					PK_DBG("[%s]material addr maybe wrong, length:%d\n",
					       __func__, length);
				}
				kfree(buff);
			} else {
			...
			PK_DBG("[%s]no material code\n", __func__);
		}
		// vivo lishuo add for ModuleId、SN and materail code,hardcore FIX ME ,end
		return 0;
	}
	return 0;
}

//hushufen add for otp remake B221117-3674 begin
unsigned char eeprom_slave_addr[IMGSENSOR_SENSOR_IDX_MAX_NUM];
EXPORT_SYMBOL_GPL(eeprom_slave_addr);
//hushufen add for otp remake B221117-3674 end
#endif
...
// add for avdd grouned
	...
	if (sensorID == 0x7309 && gimgsensor.status.reserved==1) {
        sensorID =0xFFFFFFFF;
		gimgsensor.status.reserved = 0;
		PK_DBG("Fail to get sensor ID %x, psensor_inst->sensor_idx=%d\n", sensorID,psensor_inst->sensor_idx);
		err = ERROR_SENSOR_CONNECT_FAIL;
    } else if ((sensorID == 0x38E1 || sensorID == 0xE138) && gimgsensor.status.reserved==1) {
        sensorID =0xFFFFFFFF;
		gimgsensor.status.reserved = 0;
		PK_DBG("Fail to get sensor ID %x, psensor_inst->sensor_idx=%d\n", sensorID,psensor_inst->sensor_idx);
		err = ERROR_SENSOR_CONNECT_FAIL;
	...
	if ((sensorID==0xf971) && gimgsensor.status.reserved==1) {
		sensorID =0xFFFFFFFF;
		gimgsensor.status.reserved = 0;
		PK_INFO("Fail to get sensor ID %x, psensor_inst->sensor_idx=%d\n", sensorID,psensor_inst->sensor_idx);
		err = ERROR_SENSOR_CONNECT_FAIL;
	}
	else if ((sensorID==0x0843) && gimgsensor.status.reserved==1) {
		sensorID =0xFFFFFFFF;
		gimgsensor.status.reserved = 0;
		PK_INFO("Fail to get sensor ID %x, psensor_inst->sensor_idx=%d\n", sensorID,psensor_inst->sensor_idx);
		err = ERROR_SENSOR_CONNECT_FAIL;
...
#if defined(CONFIG_MTK_CAM_PD2257F_EX)
		if (pFeatureCtrl->FeatureId !=
			SENSOR_FEATURE_GET_CUSTOM_INFO) {//hushufen add for otp remake B221117-3674
...
#define _DATA_SIZE 64
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
		{
/*vivo add by zhangyiteng for 2215 otp read Please note this start*/
#if defined(CONFIG_MTK_CAM_PD2215F_EX) || defined(CONFIG_MTK_CAM_PD2230F)
#define PDAF_DATA_SIZE 8008
#elif defined(CONFIG_MTK_CAM_PD2282)
#define PDAF_DATA_SIZE 8898
#else
#define PDAF_DATA_SIZE 4096
#endif
/*vivo add by zhangyiteng for 2215 otp read Please note this end*/
...
static void imgsensor_set_pinctrl(struct device *dev, struct IMGSENSOR_HW *phw)
{
	struct pinctrl_state *state = NULL;
	struct dev_pin_info	*pins = dev->pins;
	struct pinctrl       *bv_pinctl = NULL;
	struct pinctrl_setting *setting = NULL;

	if (NULL == pins)
		goto SET_FAIL;

	bv_pinctl = dev->pins->p;
	if (NULL == bv_pinctl || NULL == bv_pinctl->state) {
		PK_DBG("no found bv pinctrl\n");
		goto SET_FAIL;
	}
	...
	if (!setting || !(setting->pctldev)) {
		PK_DBG("can not find pinctl node\n");
		goto SET_FAIL;
	}

	phw->hw = pinctrl_dev_get_drvdata(setting->pctldev);
	return;
...
#ifdef CONFIG_CAM_I2C_PULLEN_CONTROL
	imgsensor_set_pinctrl(&pplatform_device->dev,phw);
#endif
