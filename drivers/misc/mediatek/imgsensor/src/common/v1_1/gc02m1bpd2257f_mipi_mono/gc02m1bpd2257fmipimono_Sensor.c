...
extern otp_error_code_t GC02M1BPD2257F_OTP_ERROR_CODE;
unsigned char  sn_inf_main2_gc02m1bpd2257f[13];  /*0 flag   1-12 data*/
unsigned char  module_inf_main2_gc02m1bpd2257f[13];
unsigned int   fpc_inf_main2_gc02m1bpd2257f[4];
/*hope add otp check end*/

static DEFINE_SPINLOCK(imgsensor_drv_lock);

struct IMGSENSOR_SENSOR_TABLEINFO gc02m1bpd2257f_otpinfo={
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = GC02M1BPD2257F_SENSOR_ID,
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		switch (*feature_data) {
			case 0:    //info type: otp state
				memcpy((MUINT8 *)feature_data,(MUINT8 *)&gc02m1bpd2257f_otpinfo,sizeof(struct IMGSENSOR_SENSOR_TABLEINFO));
				break;
			default:
...
static kal_uint32 get_imgsensor_fuse_id(unsigned char *data)
{
	...
	int i = 0;
	for (i = 0 ; i < 4; i++){
		sprintf(temp_fpc + i * 3, "%03d", fpc_inf_main2_gc02m1bpd2257f[i]);
	}
	for (i = 0 ; i < 12; i++){
		sprintf(temp_fpc_ascii + i * 2, "%02x", temp_fpc[i]);
	}
	for( i = 0; i < 12; i++){
		sprintf(temp_sn + i * 2, "%02x", sn_inf_main2_gc02m1bpd2257f[i + 1]);
	}
	strcat(temp_sn, temp_fpc_ascii);
	strcpy(gc02m1bpd2257f_otpinfo.SN_inf, temp_sn);
	strcpy(gc02m1bpd2257f_otpinfo.ModuleId_inf, module_inf_main2_gc02m1bpd2257f);
	return 0;
}

static MUINT32 gc02m1bpd2257fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if(p != NULL){
		*p = &gc02m1bpd2257f_otpinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	control,
	close,
	.SensorGetOtpInfo = gc02m1bpd2257fGetOtpInfo
};


UINT32 GC02M1BPD2257F_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
