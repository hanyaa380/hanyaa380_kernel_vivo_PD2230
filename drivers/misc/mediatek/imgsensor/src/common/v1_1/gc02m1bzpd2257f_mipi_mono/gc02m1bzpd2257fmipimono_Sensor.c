...
#include "gc02m1bzpd2257fmipimono_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC02M1BZV1_camera_sensor"
#define LOG_1 LOG_INF("GC02M1BZV1,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
extern int MAIN2_2e1_otp_read(void);
extern otp_error_code_t GC02M1BZPD2257F_OTP_ERROR_CODE;
extern unsigned char  sn_inf_main2_gc02m1bzpd2257f[13];  /*0 flag   1-12 data*/
extern unsigned char  module_inf_main2_gc02m1bzpd2257f[13];
extern unsigned int   fpc_inf_main2_gc02m1bzpd2257f[4];
/*hope add otp check end*/

static DEFINE_SPINLOCK(imgsensor_drv_lock);

struct IMGSENSOR_SENSOR_TABLEINFO gc02m1bzpd2257f_otpinfo={
	.OTP_type=OTP_TYPE_VITRUAL,
	.Position_type="rear",
	.Module_type="rtb",
	.Fuse_id="0",
	.SN_inf="",
...
static struct imgsensor_info_struct imgsensor_info = {
	/*.sensor_id = GC02M1BZV1_SENSOR_ID, */
	.sensor_id = GC02M1BZPD2257F_SENSOR_ID,
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO,	//sensor output first pixel color RR
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((ANALOG_GAIN_1 <= iReg) && (iReg < ANALOG_GAIN_2)) {
		...
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 1x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 1.185x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 1.4x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 1.659x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 2.0x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 2.37x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 2.8x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 3.318x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 4.0x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 4.74x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 5.6x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 6.636x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 8.0x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 9.48x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb2, (temp & 0xff));
		LOG_INF
		    ("GC02M1BZV1MIPI analogic gain 11.2x, GC02M1BZV1MIPI add pregain = %d\n",
		     temp);
	}
	else {
		write_cmos_sensor(0xfe, 0x00);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x04, 0x83);//0x83
	...
	write_cmos_sensor(0xde, 0x1c);//0x1c
	...
	write_cmos_sensor(0x2a, 0x0c);//0x0c
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = MAIN2_2e1_otp_read();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN2_2E0_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, GC02M1BZPD2257F_OTP_ERROR_CODE);
				/*vivo yy add end*/
				return ERROR_NONE;
			}
			LOG_INF
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		switch (*feature_data) {
			case 0:    //info type: otp state
				memcpy((MUINT8 *)feature_data,(MUINT8 *)&gc02m1bzpd2257f_otpinfo,sizeof(struct IMGSENSOR_SENSOR_TABLEINFO));
				break;
			default:
				LOG_INF("SENSOR_FEATURE_GET_CUSTOM_INFO error");
				break;
		}
			break;
#if 1
...
static kal_uint32 get_imgsensor_fuse_id(void)
{
	char temp_fpc[13];
	char temp_fpc_ascii[25];
	char temp_sn[50];
	int i = 0;
	for (i = 0 ; i < 4; i++){
		sprintf(temp_fpc + i * 3, "%03d", fpc_inf_main2_gc02m1bzpd2257f[i]);
	}
	for (i = 0 ; i < 12; i++){
		sprintf(temp_fpc_ascii + i * 2, "%02x", temp_fpc[i]);
	}
	for( i = 0; i < 12; i++){
		sprintf(temp_sn + i * 2, "%02x", sn_inf_main2_gc02m1bzpd2257f[i + 1]);
	}
	strcat(temp_sn, temp_fpc_ascii);
	strcpy(gc02m1bzpd2257f_otpinfo.SN_inf, temp_sn);
	strcpy(gc02m1bzpd2257f_otpinfo.ModuleId_inf, module_inf_main2_gc02m1bzpd2257f);
	return 0;
}

static MUINT32 gc02m1bzpd2257fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if(p != NULL){
		*p = &gc02m1bzpd2257f_otpinfo;
		get_imgsensor_fuse_id();
	}
	return ERROR_NONE;
}
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	control,
	close,
	.SensorGetOtpInfo = gc02m1bzpd2257fGetOtpInfo
};


UINT32 GC02M1BZPD2257F_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*    GC02M1BZV1_MIPI_RAW_SensorInit    */
