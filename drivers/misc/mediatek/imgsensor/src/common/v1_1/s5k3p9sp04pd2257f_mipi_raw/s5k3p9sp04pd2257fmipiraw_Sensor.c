...
#include "s5k3p9sp04pd2257fmipiraw_Sensor.h"

#define LOG_INF(format, args...) pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);

struct IMGSENSOR_SENSOR_TABLEINFO s5ksp9sp04pd2257f_otpinfo={
	.OTP_slave_addr = 0xA2,
	.OTP_size = 0x3FFF,
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3P9SP04PD2257F_SENSOR_ID,
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		memcpy((MUINT8 *)feature_data, (MUINT8 *)&s5ksp9sp04pd2257f_otpinfo, sizeof(struct IMGSENSOR_SENSOR_TABLEINFO));
		break;
...
static MUINT32 s5ksp9sp04pd2257fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if(p != NULL){
		*p = &s5ksp9sp04pd2257f_otpinfo;
	}
	return ERROR_NONE;
}
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	control,
	close,
	.SensorGetOtpInfo = s5ksp9sp04pd2257fGetOtpInfo
};

UINT32 S5K3P9SP04PD2257F_MIPI_RAW_SensorInit(
