 ...
 * Filename:
 * ---------
 *	 s5kjn1sq03pd2226fmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kjn1sq03pd2226fmipiraw_freq.h"
...
struct IMGSENSOR_SENSOR_TABLEINFO s5kjn1sq03pd2226f_otpinfo = {
	.OTP_slave_addr = 0xA0,
	.OTP_size = 0x3AE9,
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1SQ03PD2226F_SENSOR_ID,
...
#if 0
MUINT32  af_calib_inf_main_s5kjn1sq03pd2226f[6];
MUINT32  sn_inf_main_s5kjn1sq03pd2226f[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kjn1sq03pd2226f[4];
...
bool MAIN_38E1_otp_read(kal_uint16 addr,  BYTE *data)
{
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
    unsigned char VCM_id_main_s5kjn1sq03pd2226f[1] ={0};
    int offset = 0x0009;
...
#if 1
			MAIN_38E1_otp_read(offset,&VCM_id_main_s5kjn1sq03pd2226f[0]);
            		if((unsigned int)VCM_id_main_s5kjn1sq03pd2226f[0] == 0x13){
				if (*sensor_id == imgsensor_info.sensor_id) {
                    			pr_info("get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                    			return ERROR_NONE;
			    	}
			}else{
				pr_err("VCM_id_main_s5kjn1sq03pd2226f[0]=0x%x != 0x13",VCM_id_main_s5kjn1sq03pd2226f[0] );
                		*sensor_id = 0xFFFFFFFF;
               			 return ERROR_SENSOR_CONNECT_FAIL;
			}
		#else
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjn1sq03pd2226f temperature
    ...
    case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJN1SQ03PD2226F_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjn1sq03pd2226f, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjn1sq03pd2226f, sizeof(MUINT32)*4);
				memcpy( feature_data+12, af_calib_inf_main_s5kjn1sq03pd2226f, sizeof(MUINT32)*6);
#if 1
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kjn1sq03pd2226f[%d]= 0x%x\n", i, sn_inf_main_s5kjn1sq03pd2226f[i]);
						}
#endif
			}
				break;
	    ...
	    case 0: //info type: otp state
		    memcpy((MUINT8 *)feature_data,
			   (MUINT8 *)&s5kjn1sq03pd2226f_otpinfo,
...
static MUINT32
s5kjn1sq03pd2226fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if (p != NULL) {
		*p = &s5kjn1sq03pd2226f_otpinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	close,
	.SensorGetOtpInfo =
		s5kjn1sq03pd2226fGetOtpInfo, //vivo lishuo add for otp remake 20210823 [B210820-3030]
	//hop,
	//do_hop,
};

UINT32 S5KJN1SQ03PD2226F_MIPI_RAW_SensorInit(
