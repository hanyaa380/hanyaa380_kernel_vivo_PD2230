...
#include "gc02m1bpd2281fmipimono_Sensor.h"
...
//vivo lishuo add for otp remake 20210823 [B210820-3030] begin
struct IMGSENSOR_SENSOR_TABLEINFO gc02m1bpd2281f_otpinfo={
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = GC02M1BPD2281F_SENSOR_ID,
				  ...
				  UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		switch (*feature_data) {
			case 0:
				memcpy((MUINT8 *)feature_data,(MUINT8 *)&gc02m1bpd2281f_otpinfo,sizeof(struct IMGSENSOR_SENSOR_TABLEINFO));
				break;
			default:
...
static kal_uint32 get_imgsensor_fuse_id(unsigned char *data)
{
	...
	char temp_string[9];
	unsigned char material_inf_gc02mlbpd2281f[4];
	unsigned char sn_inf_main_gc02m1bpd2281f[13];
	if(data == NULL){
		return 0;
	}
	write_cmos_sensor(0xfc, 0x01);
	...
	write_cmos_sensor(0xf3, 0x34);
		...
		sn_inf_main_gc02m1bpd2281f[j]= value;//read_cmos_sensor(0x19);
		LOG_INF("sn_inf read addr[%02x] = 0x%04x\n",i,sn_inf_main_gc02m1bpd2281f[j]);
		timeout=0;
	}
	for(j = 0; i <= 0xF8; i+=8,j++){
	write_cmos_sensor(0xfe, 0x02);
	write_cmos_sensor(0x17, i);
	write_cmos_sensor(0xf3, 0x34);
		while((read_cmos_sensor(0xf3)&(1<<6))!=0){
		...
		value =read_cmos_sensor(0x19);
		material_inf_gc02mlbpd2281f[j]=value;//read_cmos_sensor(0x19);
		LOG_INF("material_inf read addr[%02x] = 0x%04x\n",i,material_inf_gc02mlbpd2281f[j]);
		timeout=0;
	}
	sprintf(temp_string,"%02d%02d%02d%02d",material_inf_gc02mlbpd2281f[0],material_inf_gc02mlbpd2281f[1],material_inf_gc02mlbpd2281f[2],material_inf_gc02mlbpd2281f[3]);
	sprintf(gc02m1bpd2281f_otpinfo.SN_inf,"%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
		sn_inf_main_gc02m1bpd2281f[1],
		sn_inf_main_gc02m1bpd2281f[2],
		sn_inf_main_gc02m1bpd2281f[3],
		sn_inf_main_gc02m1bpd2281f[4],
		sn_inf_main_gc02m1bpd2281f[5],
		sn_inf_main_gc02m1bpd2281f[6],
		sn_inf_main_gc02m1bpd2281f[7],
		sn_inf_main_gc02m1bpd2281f[8],
		sn_inf_main_gc02m1bpd2281f[9],
		sn_inf_main_gc02m1bpd2281f[10],
		sn_inf_main_gc02m1bpd2281f[11],
		sn_inf_main_gc02m1bpd2281f[12],
		temp_string[0],
		temp_string[1],
		temp_string[2],
		temp_string[3],
...
LOG_INF("SN_inf = %s",gc02m1bpd2281f_otpinfo.SN_inf);
	return 0;
}

static MUINT32 gc02m1bpd2281fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if(p != NULL){
		*p = &gc02m1bpd2281f_otpinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	control,
	close,
	.SensorGetOtpInfo = gc02m1bpd2281fGetOtpInfo //vivo lishuo add for otp remake 20210823 [B210820-3030]
};


UINT32 GC02M1BPD2281F_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
