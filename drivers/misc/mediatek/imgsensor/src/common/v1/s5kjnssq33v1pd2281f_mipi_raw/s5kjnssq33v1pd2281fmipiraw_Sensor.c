 ...
 * Filename:
 * ---------
 *	 s5kjnssq33v1pd2281fmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "MAIN[38EF]_s5kjnssq33v1pd2281f"
...
#include "s5kjnssq33v1pd2281fmipiraw_freq.h"
...
struct IMGSENSOR_SENSOR_TABLEINFO s5kjnssq33v1pd2281f_otpinfo = {
	.OTP_slave_addr = 0xA0,
	.OTP_size = 0x4000,
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJNSSQ33V1PD2281F_SENSOR_ID,
...
MUINT32  sn_inf_main_s5kjnssq33v1pd2281f[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kjnssq33v1pd2281f[4];
MUINT32  crosstalk_data_PD2281F[PDAF_DATA_SIZE - 2];  /*0 flag   1-836 data*/
...
#define ModuleId_addr 0x01
bool MAIN_38EF_otp_read(kal_uint16 addr,  BYTE *data)
{
...
{
	kal_uint8 i = 0;
	unsigned char module_id_main_s5kjnssq33v1pd2281f[1] ={0};
	kal_uint8 retry = 2;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		// s5kjnssq33v1pd2281f_vivo_read_eeprom(0x0001, module_id);
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000) + 1;
			pr_info("read out sensor id 0x%x,addr:%d \n", *sensor_id,i);
#if 1

			MAIN_38EF_otp_read(ModuleId_addr,&module_id_main_s5kjnssq33v1pd2281f[0]);

	  		if((unsigned int)module_id_main_s5kjnssq33v1pd2281f[0] == 0x01){
				PK_DBG("otp read  module_id success: 0x%x\n", module_id_main_s5kjnssq33v1pd2281f[0]);
				if (*sensor_id == imgsensor_info.sensor_id) {
				    pr_info("get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					return ERROR_NONE;
				}
			}else{
				pr_err("module_id_main_s5kjnssq33v1pd2281f[0]=0x%x != 0x01",module_id_main_s5kjnssq33v1pd2281f[0] );
                		*sensor_id = 0xFFFFFFFF;
                		return ERROR_SENSOR_CONNECT_FAIL;
			}
#endif
				...
				vivo_otp_read_when_power_on = s5kjnssq33v1pd2281f_MAIN_38EF_otp_read();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN_S5KJNS_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, S5KJNSSQ33V1PD2281F_OTP_ERROR_CODE);
				/*vivo lxd add end*/
#endif
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
	    		sensor_id = read_cmos_sensor_16_16(0x0000)+1;
			if (( sensor_id == imgsensor_info.sensor_id) || ( sensor_id == 0xef38)) { //compatible jnv\jn1
	...
	if ((imgsensor_info.sensor_id != sensor_id) && (sensor_id != 0xef38)) //compatible jnv\jn1
		return ERROR_SENSOR_CONNECT_FAIL;
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjnssq33v1pd2281f temperature
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJNSSQ33V1PD2281F_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjnssq33v1pd2281f, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjnssq33v1pd2281f, sizeof(MUINT32)*4);
				memcpy( feature_data+12, af_calib_inf_main_s5kjnssq33v1pd2281f, sizeof(MUINT32)*6);
#if 1
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kjnssq33v1pd2281f[%d]= 0x%x\n", i, sn_inf_main_s5kjnssq33v1pd2281f[i]);
						}
#endif
			}
				break;
	    ...
	    case 0: //info type: otp state
		    memcpy((MUINT8 *)feature_data,
			   (MUINT8 *)&s5kjnssq33v1pd2281f_otpinfo,
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/* get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			for(i = 0; i< FOUR_CELL_SIZE; i++)
				data[i+2] = crosstalk_data_s5kjnssq33v1pd2281f[i];
...
static MUINT32
s5kjnssq33v1pd2281fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if (p != NULL) {
		*p = &s5kjnssq33v1pd2281f_otpinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	close,
	.SensorGetOtpInfo =
		s5kjnssq33v1pd2281fGetOtpInfo, //vivo lishuo add for otp remake 20210823 [B210820-3030]
	//hop,
  	//do_hop,
};

UINT32 S5KJNSSQ33V1PD2281F_MIPI_RAW_SensorInit(
