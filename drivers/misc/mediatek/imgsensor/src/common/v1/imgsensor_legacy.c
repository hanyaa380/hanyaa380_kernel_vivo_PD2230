...
int iBurstWriteReg_multi(
	...
	u16 timing)
{
	return imgsensor_i2c_write(pgi2c_cfg_legacy, pData, bytes,
				   transfer_length, i2cId, timing);
}

//vivo lishuoyxqd add for otp remake 20220613 begin
bool otp_read(kal_uint16 reg_addr, kal_uint16 length, BYTE *data,
	      kal_uint8 slave_addr)
{
	kal_uint16 i = 0, j = 0, rest_num = 0, start_addr = reg_addr;

	char pu_send_cmd[2] = { (char)(start_addr >> 8),
				(char)(start_addr & 0xFF) };
	if (data == NULL) {
		return false;
	}
	...
	while (i < j) {
		if (iReadRegI2C(pu_send_cmd, 2,
				(u8 *)(data + i * MAX_DMA_TRANS_SIZE),
				MAX_DMA_TRANS_SIZE, slave_addr) < 0) {
			return false;
		}
	...
	if (rest_num > 0) {
		if (iReadRegI2C(pu_send_cmd, 2,
				(u8 *)(data + i * MAX_DMA_TRANS_SIZE), rest_num,
				slave_addr) < 0) {
			return false;
		}
	}
