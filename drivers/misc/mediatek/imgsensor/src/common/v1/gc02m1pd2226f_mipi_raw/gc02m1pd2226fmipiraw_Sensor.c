...
struct IMGSENSOR_SENSOR_TABLEINFO gc02m1pd2226f_otpinfo = {
	.OTP_slave_addr = 0xA4,
	.OTP_size = 0x143F,
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = GC02M1PD2226F_SENSOR_ID,
...
bool MACRO_02E1_otp_read(kal_uint16 addr,  BYTE *data)
{
...
#if 1
			MACRO_02E1_otp_read(TempFlag_addr, &flag_AWB);
			LOG_INF("flag_AWB:0x%x\n", flag_AWB);
			if( 0x0 == flag_AWB  )
			{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)((feature_data+1)) = GC02M1PD2226F_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main2_gc02m1pd2226f, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main2_gc02m1pd2226f, sizeof(MUINT32)*4);
#if 0
						for (int i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main2_gc02m1pd2226f[%d]= 0x%x\n", i, sn_inf_main2_gc02m1pd2226f[i]);
						}
#endif
			}
				break;
...
static MUINT32 gc02m1pd2226fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if (p != NULL) {
		*p = &gc02m1pd2226f_otpinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	close,
	.SensorGetOtpInfo =
		gc02m1pd2226fGetOtpInfo, //vivo lishuo add for otp remake 20210823 [B210820-3030]

};

UINT32 GC02M1PD2226F_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
