...
#include "imgsensor.h"
#include "imgsensor_legacy.h" //vivo lishuoyxqd otp remake add 20220613
...
{ //vivo lishuoyxqd otp remake add 20220613
	...
	if (test_i2c_is_timeout_otp(ptableInfo->OTP_slave_addr ,ptableInfo->ModuleInfo_addr + ptableInfo->OTP_addr_offset)) {
		...
		otp_read(ptableInfo->OTP_addr_offset + OTP_ModuleId_addr, 1, buff, ptableInfo->OTP_slave_addr);
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		eeprom_slave_addr[psensor_inst->sensor_idx] =
			0; //default,invaild addr
		if (psensor->pfunc->SensorGetOtpInfo == NULL) {
			PK_DBG("psensor%d->pfunc.SensorGetOtpInfo is NULL\n",
			       psensor_inst->sensor_idx);
		} else {
			PK_DBG("psensor%d->pfunc.SensorGetOtpInfo is not NULL\n",
			       psensor_inst->sensor_idx);
			imgsensor_mutex_lock(&psensor->inst);
			psensor->pfunc->SensorGetOtpInfo(
				&psensor->inst.psensor_tableInfo);
			eeprom_slave_addr[psensor_inst->sensor_idx] =
				psensor->inst.psensor_tableInfo->OTP_slave_addr;
	...
	case SENSOR_FEATURE_GET_PIXEL_RATE: {
		...
		*(pFeaturePara_64 + 1) = (uintptr_t)&Value;
		...
		ret = imgsensor_sensor_feature_control(
			psensor, pFeatureCtrl->FeatureId,
			(unsigned char *)pFeaturePara,
			(unsigned int *)&FeatureParaLen);
#if defined CONFIG_MTK_CAM_PD2226F_EX || defined CONFIG_MTK_CAM_PD2281F_EX
		if (pFeatureCtrl->FeatureId !=
		    SENSOR_FEATURE_GET_CUSTOM_INFO) { //vivo lishuo add for otp remake 20210823 [B210820-3030]
