 ...
 * Filename:
 * ---------
 *	 s5kjnssq33pd2281fmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "MAIN[38EE]_s5kjnssq33pd2281f"
...
#include "s5kjnssq33pd2281fmipiraw_freq.h"
...
struct IMGSENSOR_SENSOR_TABLEINFO s5kjnssq33pd2281f_otpinfo = {
	.OTP_slave_addr = 0xA0,
	.OTP_size = 0x4000,
	...
	.HW_GGC_addr = 0x398D,
	.HW_GGC_addr_checksum_addr = 0x3FFF
};

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJNSSQ33PD2281F_SENSOR_ID,
	...
	.pre = {		//1
		...
		.mipi_pixel_rate = 738400000,//mipi
	...
	.custom1 = { //3264*2448 30fps 1651Mbps/lane
		.pclk =563000000,
		.linelength = 5946,
		.framelength = 3152,
		.startx =0,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[15] = {
	{4080, 3072,   0,   6, 4080, 3060, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /*Preview*/			// 4080*3060*30FPS
	{4080, 3072,   0,   6, 4080, 3060, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /* capture */		// 4080*3060*30FPS
	{4080, 3072,   0, 388, 4080, 2296, 4080, 2296, 0, 0, 4080, 2296, 0, 0, 4080, 2296}, /* normal video */	// 4080*2296*30FPS
	...
	{4080, 3072, 408, 312, 3264, 2448, 3264, 2448, 0, 0, 3264, 2448, 0, 0, 3264, 2448}, /* custom1 */		// 3264*2448*30FPS
};
#if 1
...
{/* Preview mode setting 4080*3060*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 ...
	 0x01, 0x30, 0x027C, 0x0BE0, 0x00, 0x00, 0x0000, 0x0000},//VC DT bufW bufH
	...
	/* HS_VIDEO mode setting 1920*1080 120fps*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x0780, 0x0438, 0x00, 0x00, 0x0000, 0x0000,
	 0x01, 0x30, 0x0258, 0x0218, 0x00, 0x00, 0x0000, 0x0000},
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =  //4080X3060
{
	...
	.i4BlockNumY = 382,
	.iMirrorFlip = 0,
	.i4Crop = { {0,6}, {0,6}, {0, 388}, {120, 456}, {120, 456}, {408, 312}},
...
extern int s5kjnssq33pd2281f_MAIN_38EE_otp_read(void);
extern otp_error_code_t S5KJNSSQ33PD2281F_OTP_ERROR_CODE;
extern MUINT32 sn_inf_main_s5kjnssq33pd2281f[13];  /*0 flag   1-12 data*/
extern MUINT32 material_inf_main_s5kjnssq33pd2281f[4];
extern MUINT32 af_calib_inf_main_s5kjnssq33pd2281f[6];
extern unsigned char crosstalk_data_s5kjnssq33pd2281f[ FOUR_CELL_SIZE];  /*0 flag   1-836 data*/
extern MUINT32 module_inf_main_s5kjnssq33pd2281f[2];
/*hope add otp check end*/
#endif
#define VIVO_EEPROM_WRITE_ID 0xA0
#define VIVO_I2C_SPEED 400
#define VIVO_MAX_OFFSET 0x4000

#define HOPE_ADD_FOR_CAM_TEMPERATURE_NODE
//#define READ_FRAMELENGTH_SHUTTER_GAIN

#ifdef READ_FRAMELENGTH_SHUTTER_GAIN
static int s5kjns_read_framelength;
static int s5kjns_read_shutter;
static int s5kjns_read_gain;
#endif
...
#ifdef READ_FRAMELENGTH_SHUTTER_GAIN
	s5kjns_read_framelength = read_cmos_sensor_16_16(0x0340);
	s5kjns_read_shutter = read_cmos_sensor_16_16(0x0202);
	PK_DBG("s5kjns_read_shutter = %d, s5kjns_read_framelength = %d \n", s5kjns_read_shutter,s5kjns_read_framelength);
#endif
}	/*	write_shutter  */
...
#ifdef READ_FRAMELENGTH_SHUTTER_GAIN
	s5kjns_read_gain = read_cmos_sensor_16_16(0x0204);
	PK_DBG("s5kjns_read_gain = %d \n", s5kjns_read_gain);
#endif
...
static kal_uint16 addr_data_pair_slim_video[] = {  //1920x1080 120fps
0x6028, 0x2400,
0x602A, 0x1A28,
0x6F12, 0x4C00,
0x602A, 0x065A,
...
static kal_uint16 addr_data_pair_custom1[] = {  //3264x2448
    ...
    0x030C, 0x0000,
    0x030E, 0x0004,
    0x0310, 0x0063,///0080   007F--1651mbps 0067--1339mbps  0063---1287.0mbps
    0x0312, 0x0000,
    0x080E, 0x0000,
    0x0340, 0x0C50,///0FE0  0FF8--1651mbps 0D90--1339mbps   0C50---1287.0mbps
    0x0342, 0x173A,///      11E8--1651mbps 1510--1339mbps   173A---1287.0mbps
    0x0702, 0x0000,
    0x0202, 0x0100,
    0x0200, 0x0100,
    0x0D00, 0x0101,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000);
			...
			s5kjnssq33pd2281f_vivo_read_eeprom(OTP_ModuleId_addr, &module_id[0]);
			if(module_id[0] == 0x05) {
				PK_DBG("otp read  module_id success: 0x%x\n", module_id[0]);
				if ((*sensor_id == imgsensor_info.sensor_id) || (*sensor_id == 0xee38)) {
					PK_DBG("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
		}else{
			pr_err("module_id[0]=0x%x != 0x11",module_id[0] );
            *sensor_id = 0xFFFFFFFF;
        	return ERROR_SENSOR_CONNECT_FAIL;
		}
	...
	if (*sensor_id != imgsensor_info.sensor_id) { //compatible jnv\jn1
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
	    		sensor_id = read_cmos_sensor_16_16(0x0000);
			if (( sensor_id == imgsensor_info.sensor_id) || (sensor_id == 0xee38)) { //compatible jnv\jn1
				PK_DBG("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjnssq33pd2281f temperature
...
//	write_cmos_sensor_16_16(0x6028, 0x4000);
	...
	PK_DBG("s5kjnssq33 TMC 0X0020 Value = 0x%4x, TMC Shift Value = 0x%4X\n", TMC_000A_Value, TMC_Shift_Value);
	...
	if (TMC_Shift_Value == 1){
		...
		if ((TMC_Value > 80) || (TMC_Value < 20)){
			PK_DBG("s5kjnssq33 TMC 0X0020 temperature Value = %d, cause of TMC > 80 or TMC < 20 \n", TMC_Value);
			TMC_Value = 0;
		}
		PK_DBG("s5kjnssq33 TMC 0x0020 register value 0x%x temperature value = %d", TMC_000A_Value, TMC_Value);
	}
	return TMC_Value;
}
#endif
...
// vivo add rawinfo
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
			*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 1; /* 1920*1080 */
			break;
		default:
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJNSSQ33PD2281F_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjnssq33pd2281f, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjnssq33pd2281f, sizeof(MUINT32)*4);
				memcpy( feature_data+12, af_calib_inf_main_s5kjnssq33pd2281f, sizeof(MUINT32)*6);
				memcpy( feature_data+15, module_inf_main_s5kjnssq33pd2281f, sizeof(MUINT32)*2);
			#if 0
				for (i = 0 ; i<12 ; i++ ){
					PK_DBG("sn_inf_main_s5kjnssq33pd2281f[%d]= 0x%x\n", i, sn_inf_main_s5kjnssq33pd2281f[i]);
				}
			#endif
			}
				break;
	    ...
	    case 0: //info type: otp state
		    memcpy((MUINT8 *)feature_data,
			   (MUINT8 *)&s5kjnssq33pd2281f_otpinfo,
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/* get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			for(i = 0; i< FOUR_CELL_SIZE; i++)
				data[i+2] = crosstalk_data_s5kjnssq33pd2281f[i];
...
static MUINT32
s5kjnssq33pd2281fGetOtpInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
	if (p != NULL) {
		*p = &s5kjnssq33pd2281f_otpinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	close,
	.SensorGetOtpInfo =
		s5kjnssq33pd2281fGetOtpInfo, //vivo lishuo add for otp remake 20210823 [B210820-3030]
	//hop,
  	//do_hop,
};

UINT32 S5KJNSSQ33PD2281F_MIPI_RAW_SensorInit(
