...
enum ACDK_SENSOR_FEATURE_ENUM {
	...
	SENSOR_FEATURE_GET_HS_TRAIL,  /*Copy from zjj add for video caton (B210703-2473) 2021/7/5  ALPS05888257*/
...
struct ACDK_SENSOR_INFO_STRUCT {
	...
	/*vivo zhenrong add beg for sensor chroma*/
	MUINT8 SensorChroma;
	/*vivo zhenrong add end for sensor chroma*/
};
...
#define OTP_ModuleId_addr 0x0001
typedef enum {
	OTP_TYPE_NORMAL = 0,
	OTP_TYPE_EMPTY,
...
//reference to LscMgrImp::doShadingAlign()
typedef enum {
	ROTATION_NORMAL = 0,
	ROTATION_HV_MIRROR,
	ROTATION_H_MIRROR,
...
struct IMGSENSOR_SENSOR_TABLEINFO {
	...
	const MUINT16 CloudFlag_addr; //if CloudFlag_addr!= 0x13, you need to fill this parameter.
	...
	const MUINT16
		Af_disScaleRatio; //0(default) or 1 means x1, 10 means x10.only when the unit "cm" needs x10
	const MUINT16
		Af_StartCode_addr; //afcode for vivo need;Don`t need to fill the addr which doesn`t exist.
	const MUINT16 Af_InfCode_addr;
	const MUINT16 Af_MidCode_addr; //reserve 20211219
	const MUINT16 Af_MacroCode_addr;
	const MUINT16
		Af_InfDis_addr; //info for mtk api need;Don`t need to fill the addr which doesn`t exist.
	const MUINT16 Af_MidDis_addr; //reserve 20211219
	const MUINT16 Af_MacroDis_addr;
	...
	const MUINT16 length_of_proc1data;
	const MUINT16 PDAF_proc1_checksum_addr;
	const MUINT16 PDAF_proc2_addr;
	const MUINT16 length_of_proc2data;
	const MUINT16 PDAF_proc2_checksum_addr;
	const MUINT16 PDAF_proc1_HVbin_addr; //for QPD sensor
	const MUINT16 PDAF_proc1_HVbin_checksum_addr;
	const MUINT16 PDAF_proc2_HVbin_addr;
	const MUINT16 PDAF_proc2_HVbin_checksum_addr;
	const MUINT16 PDAF_proc1_Vbin_addr;
	const MUINT16 PDAF_proc1_Vbin_checksum_addr;
	const MUINT16 PDAF_proc2_Vbin_addr;
	const MUINT16 PDAF_proc2_Vbin_checksum_addr;
	const MUINT16 PDAF_proc1_Fullremosaic_addr;
	const MUINT16 PDAF_proc1_Fullremosaic_checksum_addr;
	const MUINT16 PDAF_proc2_Fullremosaic_addr;
	const MUINT16 PDAF_proc2_Fullremosaic_checksum_addr;
	...
	const MUINT16 OTP_size; //absolute size to read
	const MUINT8 OTP_slave_addr; //8bit slave addr
	MUINT8 OTP_type; //set OTP_TYPE_VITRUAL while virtural module,or set OTP_TYPE_EMPTY in sensortableinfo.
	MUINT8 Fuse_id[64]; //only for clouds module
	const MUINT8 Position_type[16]; //refer to camera_config.xml
	const MUINT8 Module_type[16]; //refer to camera_config.xml
	MUINT8 Material_inf[32];
	MUINT8 SN_inf[64];
	MUINT8 ModuleId_inf[32];
	const MUINT8 PLATFORM_id; //is legacy
	const MUINT8 LENS_id; //is legacy
	const MUINT8 VCM_id; //is legacy
	const MUINT8 DRIVERIC_id; //is legacy
	const MUINT8 CalibrationVersion[4]; //is legacy

	const MUINT8 MtkLscType; //Usually set 2(mtk default value).
	const MUINT8 PixId; //refer to OTP guide,
	const MUINT8
		TableRotation; //"mirror/flip mode in OTP guide" XOR  "mirror/flip mode in sensor driver". Warning:It`s not the value XOR, pls reference examples below:
		//e.g. "H" in otp guide, "H" in sensor driver====>set "ROTATION_NORMAL" refer to enum lsc_rotation_mode_t
		//e.g. "Normal" in otp guide, "H" in sensor driver====>set "ROTATION_H_MIRROR"
		//e.g. "V" in otp guide, "H" in sensor driver====>set "ROTATION_HV_MIRROR"

	MUINT8 AWBcheckRangeRatio; //e.g. 15 means 15%.   >5M pixels sensor:12; <=5M pixels sensor:17; wide/ultra sensor:12.
	MUINT8 AWBforceGainSetNum; //0:self-adaption(default); 1:force use only 5100K data; 2:force use both 5100k and 3100K

	MUINT8 PDAF_type; //Usually set 0 except for QPD sensor which needs to set PDAF_SUPPORT_CAMSV_QPD(refer to IMGSENSOR_PDAF_SUPPORT_TYPE_ENUM in this file.).
...
enum IMGSENSOR_CHROMA_MODE{
    SENSOR_CHROMA_4_IN_1 = 0x0,
    SENSOR_CHROMA_9_IN_1
};
