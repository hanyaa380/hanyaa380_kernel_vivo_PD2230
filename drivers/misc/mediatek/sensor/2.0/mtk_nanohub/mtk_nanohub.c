...
static void mtk_nanohub_init_sensor_info(void)
{
	...
	p = &sensor_state[COMM_SENSOR_TYPE_VLOGGER];
	p->sensorType = COMM_SENSOR_TYPE_VLOGGER;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "vlogger", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_PROX_GESTURE];
	p->sensorType = COMM_SENSOR_TYPE_PROX_GESTURE;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "prox_gesture", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));

	p = &sensor_state[COMM_SENSOR_TYPE_GAME_GESTURE];
	p->sensorType = COMM_SENSOR_TYPE_GAME_GESTURE;
	p->gain = 1;
	strlcpy(p->name, "game_gesture", sizeof(p->name));
	strlcpy(p->vendor, "vivo", sizeof(p->vendor));
}
...
static int mtk_nanohub_send_timestamp_wake_locked(void)
{
	...
	pr_err("sync ap boottime=%lld  size:%d/%d\n", now_time,
			sizeof(req.set_config_req), sizeof(struct SCP_SENSOR_HUB_SET_CONFIG_REQ));
...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		if (data->sensor_type == ID_PROXIMITY ||
			...
			data->sensor_type == ID_PROX_GESTURE ||
