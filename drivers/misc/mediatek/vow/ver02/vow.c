...
#ifdef CONFIG_MTK_VOW_YUEMENG_SUPPORT
static int vow_seamless_flag = -1;
static unsigned int seamless_buf_offset;
#endif
/*****************************************************************************
...
static void vow_ipi_rx_handle_data_msg(void *msg_data)
{
	...
	if ((ipi_ptr->ipi_type_flag & DEBUG_DUMP_IDX_MASK) &&
		...
		#ifdef CONFIG_MTK_VOW_YUEMENG_SUPPORT
			vow_seamless_flag = 1;
			seamless_buf_offset = vowserv.scp_shared_voice_buf_offset;
		    VOWDRV_DEBUG("vow,v_len=%x,vow_seamless_flag:%d seamless_buf_offset:%d\n", vowserv.scp_shared_voice_length, vow_seamless_flag, seamless_buf_offset);
		#else
...
static void vow_register_vendor_feature(int uuid)
{
	...
	case VENDOR_ID_AMAZON:
		...
		break;
	case VENDOR_ID_YUEMENG:
		vow_register_feature(VOW_VENDOR_Y_FEATURE_ID);
		break;
...
static void vow_deregister_vendor_feature(int uuid)
{
	...
	case VENDOR_ID_AMAZON:
		...
		break;
	case VENDOR_ID_YUEMENG:
		vow_deregister_feature(VOW_VENDOR_Y_FEATURE_ID);
		break;
	case VENDOR_ID_OTHERS:
					      ...
					      unsigned int buf_length)
{
	...
	if (buf_length != 0) {
    #ifdef CONFIG_MTK_VOW_YUEMENG_SUPPORT
		if ((vowserv.kernel_voicedata_idx + buf_length)
	#else
			 ...
			 || (vowserv.recording_flag == false)) {
				...
				if (vow_seamless_flag == 1) {
					...
					stop_condition =
						...
						seamless_buf_offset,
						0x12480);
					printk("%s vow seamless read done", __func__);
				} else {
			#endif
					/* To Read Voice Data from Kernel to User */
					stop_condition =
