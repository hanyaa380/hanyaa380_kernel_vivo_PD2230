 ...
 */
#define MICBIAS_DISABLE_TIMER   (3 * HZ) //modify vivo audio yanghen for accdet 
...
static void send_key_event(u32 keycode, u32 flag)
{
	...
	case AS_KEY:
		...
		input_sync(accdet_input_dev);
		pr_info("accdet KEY_IMUSIC_COLLECT %d\n", flag);
		break;
	}
}
		...
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			if (eint_accdet_sync_flag){
				...
				if (accdet_no_work && cur_key != NO_KEY) {
					...
					headset_plug_out();  // modify by yanghen  for mic disconnected send plug out 
					pr_info("accdet headset has been plug-out\n");
 				}
#else
				headset_plug_out();  // modify by yanghen  for mic disconnected send plug out
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	__set_bit(KEY_IMUSIC_COLLECT, accdet_input_dev->keybit);  // modify by vivo audio yanghen for key=760
