...
unsigned int _mt_cpufreq_get_cpu_level(void)
{
	...
	if (val == 2) {
		...
		if (!force_downgrade_mt6877t_node) {
			force_downgrade_mt6877t = false;
		}
		else {
			force_downgrade_mt6877t = true;
			...
			if (!gpio_is_valid(gpio_num)) {
				...
				pr_info("%s: cpu if downgrade_mt6877t gpio num: %d and gpio val: %d\n",
		...
		pr_info("%s: force_downgrade_mt6877t(%d)",
				__func__, force_downgrade_mt6877t);

		if (force_downgrade_mt6877t)
			lv = CPU_LEVEL_1;
	} else if (val == 3)
		lv = CPU_LEVEL_0;
	else
