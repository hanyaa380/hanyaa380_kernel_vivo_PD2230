...
static void tcp_handshake_check(struct sk_buff *skb)
{
	static u64 handshake_count = 0;
	static u64 handshake_fail_count = 0;
	int oldlen = 0;
	...
	if (sk && (skb->data)) {
		icsk = inet_csk(sk);
		/* During TCP handshake state*/
		if (sk->sk_state == TCP_SYN_SENT) {
			handshake_count++;
			/* TCP retransmission occurred*/
			printk("retransmits:%d, count:%lu ,%lu ,%pI4 ,uid:%d\n",
				icsk->icsk_retransmits,
				handshake_fail_count,handshake_count,&ip_hdr(skb)->daddr,
				sk->sk_uid.val);
			if (icsk->icsk_retransmits > 0) {
				handshake_fail_count++;
				if (icsk->icsk_retransmits == 2) {
					if (ip_hdr(skb)->version == 4) { // ipv4
						oldlen = snprintf(data_buffer, sizeof(data_buffer), "%pI4,%d,%u,%lu,%lu",
						  &ip_hdr(skb)->daddr,
						  icsk-> icsk_retransmits,
						  (unsigned int)sk->sk_uid.val,
						  handshake_fail_count,
						  handshake_count);
					} else if (ip_hdr(skb)->version == 6) { // ipv6
						printk("retransmits:%d,count:%lu ,%lu ,%pI6c ,uid:%d\n",
							   icsk->icsk_retransmits,
							   handshake_fail_count,handshake_count,&ipv6_hdr(skb)->daddr,
							   sk->sk_uid.val);
						oldlen = snprintf(data_buffer, sizeof(data_buffer), "%pI6,%d,%u,%lu,%lu",
						  &ipv6_hdr(skb)->daddr,
						  icsk-> icsk_retransmits,
						  (unsigned int)sk->sk_uid.val,
						  handshake_fail_count,
			...
			/* Notify user space  ,reset handshake_count and handshake_fail_count */
...
static netdev_tx_t ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	// vivo wumengxiang add for HANDSHAKE_INFO begin
	tcp_handshake_check(skb);
	// vivo wumengxiang add for HANDSHAKE_INFO end
