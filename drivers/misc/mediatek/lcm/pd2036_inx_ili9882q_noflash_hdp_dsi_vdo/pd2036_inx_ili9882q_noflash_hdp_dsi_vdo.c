...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.horizontal_backporch = 84;
...
{
	LCM_INFO("%s inx ili9882q -> enter power on begin, panel_reset_state=%d\n", __func__, panel_reset_state);
	...
	************************************/
		...
		LCM_INFO("%s pd1987_qunchuang_ili9881h_noflash_hdp_dsi_vdo -> enter power on end\n", __func__);
	}
}

static void lcm_suspend_power(void)
{
	if (!strcmp(project_name, "PD2139JF_EX")) {
		...
		lcm_enp_setting(0);
		MDELAY(3);
		LCM_INFO("[LCM]lcm_suspend_power ili9881h- vddi keep high level\n");
...
{
	LCM_INFO("[LCM]inx ili9882q- init begin\n");
	lcm_reset_setting(1);
	MDELAY(10);
	lcm_reset_setting(0);
	MDELAY(3);
	...
	******************************************************************/
	...
	LCM_INFO("[LCM]inx ili9882q- init end\n");
}
...
static void lcm_suspend(void)
{
	...
	push_table(lcm_suspend_setting,
		...
		1);
	panel_off_state = 1;
	LCM_INFO("[LCM]inx ili9882q- lcm_suspend display off\n");
}
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	/*level =level*16;*/
	level = level * 2;
	if (lcm_id_version == 0x02 && level >= 3801)
	{
		level = 3801;
	}
...
struct LCM_DRIVER pd2036_inx_ili9882q_noflash_hdp_dsi_vdo_lcm_drv = {
	.name = "pd2036_inx_ili9882q_noflash_hdp_dsi_vdo",
