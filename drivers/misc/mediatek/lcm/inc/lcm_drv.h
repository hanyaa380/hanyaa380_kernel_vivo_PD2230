...
struct LCM_DRIVER {
	...
	void (*lcm_cabc_open)(void *handle, unsigned char leveldimming, unsigned char levelsetting); //170704+ for cabc setting
	void (*lcm_cabc_close)(void *handle, unsigned char level);
	void (*lcm_reset)(void);
	void (*tp_reset)(unsigned int status);
	void (*avdd_avee_output)(unsigned int status);
};
