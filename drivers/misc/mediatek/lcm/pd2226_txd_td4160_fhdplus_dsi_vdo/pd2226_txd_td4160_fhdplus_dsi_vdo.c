...
/******************************************lcmcabc start**************************************************/
extern atomic_t lcm_sre_state;
static DEFINE_MUTEX(bl_level_mutex);
extern int pd2226_brightness_hal;
int last_backlight = 0;
static ktime_t set_backlight_time = 0;
static u64 do_away_udelay(void)
{
	s64 dimming_us;
	s64 delta_us = ktime_us_delta(ktime_get(), set_backlight_time);
	dimming_us = 100*167*24;
	//LCM_INFO("LastBacklight delta_us=%d, dimming_us=%d", delta_us, dimming_us);
	return (((dimming_us - delta_us) > 0) ? (dimming_us - delta_us) : (0));
}
static void set_backlight_func(struct work_struct *data)
{
	mutex_lock(&bl_level_mutex);
	if(last_backlight != 0) {
		bl_level_12bit[0].para_list[0] = (unsigned char)((last_backlight>>8)&0xFF);
		bl_level_12bit[0].para_list[1] = (unsigned char)(last_backlight&0xFFFF);
		push_table(NULL, bl_level_12bit, sizeof(bl_level_12bit) / sizeof(struct LCM_setting_table), 1);
		set_backlight_time = ktime_get();
		LCM_INFO("LastBacklight = %d, last_backlight=%d, high_bit=0x%x, low_bit=0x%x\n", bl_brightness_hal, last_backlight, bl_level_12bit[0].para_list[0], bl_level_12bit[0].para_list[1]);
		last_backlight = 0;
	}
	mutex_unlock(&bl_level_mutex);
}
static DECLARE_WORK(set_backlight_work, (void *) set_backlight_func);
static void backlight_timeout(struct timer_list *unused)
{
	schedule_work(&set_backlight_work);
}
static DEFINE_TIMER(backlight_timeout_timer, backlight_timeout);
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	static u64 expires_time = 0;
	mutex_lock(&bl_level_mutex);
	if (level >= 3389)	//20.049mA
		level = 3389;

	if ((level != 0) && (dimming_enable == 0) && (bl_brightness_hal != 0) && (bl_lvl != 0)) {
	...
	if (level == 0) {
		del_timer_sync(&backlight_timeout_timer);
	...
	bl_lvl = level;
	if((atomic_read(&lcm_sre_state) == 1) && (level != 0))
		level = 4095;

	expires_time = do_away_udelay();
	if((expires_time == 0) || (level == 0)) {
	...
	else if((pd2226_brightness_hal == level) && (level != 0)) {
		last_backlight = level;
		mod_timer(&backlight_timeout_timer, jiffies + usecs_to_jiffies(expires_time+1600));
	}
	else {
		;
	}
	LCM_INFO("LastBacklight = %d, level=%d, expires_time = %d\n", bl_brightness_hal, level, expires_time);
	mutex_unlock(&bl_level_mutex);
}
...
{
	if(lcm_software_id == 0x24)
		return 0x91;
	else
		return 0xFF;
}

struct LCM_DRIVER pd2226_txd_td4160_fhdplus_dsi_vdo_lcm_drv = {
	.name = "pd2226_txd_td4160_fhdplus_dsi_vdo",
