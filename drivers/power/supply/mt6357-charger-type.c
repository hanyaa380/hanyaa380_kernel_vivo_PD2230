...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
static struct mtk_charger_type *this_info = NULL;
#endif
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	/* RG_bc11_VREF_VTH = [1:0]=01 vref=1.2v */
	bc11_set_register_value (info->regmap,
			PMIC_RG_BC11_VREF_VTH_ADDR,
			PMIC_RG_BC11_VREF_VTH_MASK,
			PMIC_RG_BC11_VREF_VTH_SHIFT,
...
static int get_charger_type(struct mtk_charger_type *info)
{
	...
	if (hw_bc11_DCD(info)) {
		if (hw_bc11_stepA1(info)) {
			info->psy_desc.type = POWER_SUPPLY_TYPE_APPLE_BRICK_ID;
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
unsigned int __attribute__ ((weak)) upmu_get_rgs_chrdet()
{
	unsigned int chrdet = 0;
	struct mtk_charger_type *info = this_info;

	if (!info) {
		pr_err("mtk_charger_type not init\n");
	} else {
...
void __attribute__ ((weak)) pmic_register_interrupt_callback_ex(void (EINT_FUNC_PTR) (void))
{
	struct mtk_charger_type *info = this_info;
	...
	if (!info) {
		...
		info->callback = EINT_FUNC_PTR;
	}
}
EXPORT_SYMBOL(pmic_register_interrupt_callback_ex);
