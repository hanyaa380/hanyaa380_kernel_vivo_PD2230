...
static void pm8008_release_reset_gpio(void)
{
	if (gpio_is_valid(rst_data.reset_gpio)) {
		//pinctrl_gpio_free(rst_data.reset_gpio);
		gpio_free(rst_data.reset_gpio);
		pr_err("pm8008 relase reset gpio");
	}
}
...
static int pm8008_regulator_probe(struct platform_device *pdev)
{
	int rc = 0;
	struct regmap *regmap;
	pr_err("pm8008_regulator_probe enter\n");
	...
	if (gpio_is_valid(rst_data.reset_gpio)) {
		if (gpio_get_value(rst_data.reset_gpio) == 0) {
			...
			pr_err("pm8008_regulator_probe  reset gpio set to high\n");
		}
	}
...
static int pm8008_chip_probe(struct platform_device *pdev)
{
	...
	pr_err("pm8008_chip_probe enter\n");
	...
	if (gpio_is_valid(rst_data.reset_gpio)) {
		...
		if (rc) {
			...
			gpio_direction_output(rst_data.reset_gpio, 1);
			usleep_range(500, 600);
			pr_err("pm8008_chip_probe  reset gpio set to high\n");
		}
	} else {
	...
	if (chip->ocp_irq < 0) {
		pr_err("Failed to get pm8008-ocp-irq\n");
	} else {
