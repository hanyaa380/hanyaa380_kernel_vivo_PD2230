...
#if IS_ENABLED(CONFIG_UFSRINGBUF_POC)
...
static const struct file_operations fops_proc_print = {
	.open = ufsringbuf_proc_print_open,
	.read = seq_read,
 ...
 * For POC.
 */
static ssize_t+ufsringbuf_sysfs_store_debug_set_pwd(struct ufsringbuf_dev *ringbuf,
...
static ssize_t+ufsringbuf_sysfs_show_issue_abort_cmd(struct ufsringbuf_dev *ringbuf, char *buf)
{
	struct ufsf_feature *ufsf = ringbuf->ufsf;
	u8 resp = 0xF;
	int err;
	...
	pm_runtime_get_sync(ufsf->hba->dev);
	err = ufshcd_issue_tm_cmd(ufsf->hba, 0, 0, UFS_ABORT_TASK, &resp);
	pm_runtime_put_sync(ufsf->hba->dev);
	if (err) {
