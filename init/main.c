...
unsigned int is_atfirst;
EXPORT_SYMBOL(is_atfirst);
unsigned int bsp_test_mode;
EXPORT_SYMBOL(bsp_test_mode);
unsigned int bsptest_battery;
EXPORT_SYMBOL(bsptest_battery);
unsigned int is_atelsp1;
EXPORT_SYMBOL(is_atelsp1);
...
static void __init is_at_boot(char *command_line)
{
	...
	if (strnstr(command_line, "sendAT_no_wifi", strlen(command_line))) {
		is_atelsp1 = 1;
		pr_warn("AT ELSP1 Mode & Factory mode\n");
	} else {
		is_atelsp1 = 0;
	}

	if (strnstr(command_line, "bsptest_battery=1", strlen(command_line)) == NULL) {
		bsptest_battery = 0;
	} else {
		bsptest_battery = 1;
	}

	if (strnstr(command_line, "sendAT_first", strlen(command_line))) {
		is_atfirst = 1;
		pr_warn("first into AT Mode Boot & Factory mode\n");
	} else {
		is_atfirst = 0;
	}
