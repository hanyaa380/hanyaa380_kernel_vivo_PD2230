...
struct mt63xx_accdet_data {
	...
	struct work_struct r_eint_work;
	struct workqueue_struct *r_eint_workqueue;
#ifdef ACCDET_RIGHT_EINT
	u32 r_gpiopin;
	u32 r_gpioirq;
	u32 accdet_no_work;
#endif
...
static u32 key_check(u32 v)
{
	if (HAS_CAP(accdet->data->caps, ACCDET_FOUR_KEY)) {
		...
		if ((v < accdet_dts.four_key.up) &&
			(v >= accdet_dts.four_key.collect))
			return UP_KEY;
		if ((v < accdet_dts.four_key.collect) &&
...
static void send_status_event(u32 cable_type, u32 status)
{
	...
	pr_info("accdet:%s: cable_type:%d, status:%d\n",__func__, cable_type, status);
	switch (cable_type) {
	case HEADSET_NO_MIC:
		if (status)
	...
	case LINE_OUT_DEVICE:
		...
		break;
	default:
		pr_info("accdet: %s Invalid cableType\n", __func__);
	}
}
...
{
	pr_info("accdet:%s: cable_type:%d, accdet_status:%d, cur_eint_state:%d\n",
				__func__, accdet->cable_type, accdet->accdet_status, accdet->cur_eint_state);
...
static void right_eint_work_callback(struct work_struct *work)
{
	int hphl_val = 0, hphr_val = 0, i;

	hphl_val = gpio_get_value(accdet->gpiopin);
	hphr_val = gpio_get_value(accdet->r_gpiopin);
	printk("[accdet] %s hphl_val=%d, hphr_val=%d. \n", __func__, hphl_val, hphr_val);
	if (!hphr_val) {
		mutex_lock(&accdet->res_lock);
		accdet->eint_sync_flag = true;
		mutex_unlock(&accdet->res_lock);
		gpio_set_debounce(accdet->r_gpiopin,	accdet_dts.plugout_deb * 1000);
		...
		if (!hphl_val && !hphr_val) {
			if (accdet->cable_type == NO_DEVICE) {
				...
				for (i = 0; i < 35; i++) {
					...
					if (accdet->cable_type != NO_DEVICE)
						break;
					if (gpio_get_value(accdet->r_gpiopin)) {
						gpio_set_debounce(accdet->r_gpiopin, accdet_dts.plugout_deb);
						irq_set_irq_type(accdet->r_gpioirq, IRQ_TYPE_LEVEL_LOW);
						enable_irq(accdet->r_gpioirq);
		...
		irq_set_irq_type(accdet->r_gpioirq, IRQ_TYPE_LEVEL_HIGH);
	} else {
		mutex_lock(&accdet->res_lock);
		accdet->eint_sync_flag = false;
		accdet->thing_in_flag = false;
		mutex_unlock(&accdet->res_lock);
		gpio_set_debounce(accdet->r_gpiopin, accdet->gpio_hp_deb);
		del_timer_sync(&micbias_timer);
		/* disable accdet_sw_en=0
		*/
		accdet_clear_bits(ACCDET_CMP_PWM_IDLE_ADDR,
		...
		disable_accdet();
		headset_plug_out();
		accdet->accdet_no_work = 0;
		irq_set_irq_type(accdet->r_gpioirq, IRQ_TYPE_LEVEL_LOW);
		accdet->cur_eint_state = EINT_PLUG_OUT;
	}
	if (HAS_CAP(accdet->data->caps, ACCDET_AP_GPIO_EINT))
		enable_irq(accdet->r_gpioirq);
	pr_info("accdet:%s: enable R irq\n",__func__);
}
...
#if 0
	if (!gpio_is_valid(accdet->r_gpiopin)) {
		if (accdet->cur_eint_state == EINT_PLUG_IN) {
			mutex_lock(&accdet->res_lock);
			accdet->eint_sync_flag = true;
			mutex_unlock(&accdet->res_lock);
			...
			if (!hphl_val && !hphr_val) {
				...
				if (accdet->cable_type == NO_DEVICE) {
					...
					for (i = 0; i < 35; i++) {
						...
						if (accdet->cable_type != NO_DEVICE)
							break;
						if (gpio_get_value(accdet->gpiopin)) {
...
static inline void check_cable_type(void)
{
	...
	pr_info("%s: enter, cur_AB:%d, accdet_status:%d\n",__func__,cur_AB,accdet->accdet_status);
	...
	case MIC_BIAS:
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			if (accdet->eint_sync_flag) {
				accdet->accdet_status = PLUG_OUT;
				//accdet->cable_type = NO_DEVICE;
				if (accdet->accdet_no_work || accdet->cur_key != NO_KEY) {
...
static void accdet_queue_work(void)
{
	...
	pr_info("%s: cali_voltage is:%d\n",__func__,accdet->cali_voltage);
...
void accdet_irq_handle(void)
{
	...
	pr_info("%s: eintID:%d, irq_status:%d, acc_sts:%d, eint_sts:%d\n",
			__func__, eintID, irq_status, acc_sts, eint_sts);
...
static irqreturn_t ex_r_eint_handler(int irq, void *data)
{
	...
	disable_irq_nosync(accdet->r_gpioirq);
	pr_info("[Accdet]accdet_r_eint_func after cur_eint_state=%d\n", accdet->cur_eint_state);
	ret = queue_work(accdet->r_eint_workqueue, &accdet->r_eint_work);
	return IRQ_HANDLED;
}
...
static inline int ext_eint_setup(struct platform_device *platform_device)
{
	...
	if (IS_ERR(accdet->pins_eint)) {
		...
		pr_err("%s: no state_eint_as_int pinctl.\n", __func__);
		return ret;
	}
	...
	pr_info("%s: gpioirq=%d\n", __func__, accdet->gpioirq);
	ret = of_property_read_u32_array(node, "interrupts", ints,
			ARRAY_SIZE(ints));
	if (ret) {
		pr_notice("accdet get interrupts array fail: %d\n", ret);
		return ret;
	}
	accdet->accdet_eint_type = ints[1];
#ifdef ACCDET_RIGHT_EINT
	accdet->r_gpiopin = of_get_named_gpio(node, "deb-rgpios", 0);
	if (!gpio_is_valid(accdet->r_gpiopin)) {
		pr_notice("accdet %s fail to get r gpio pin", __func__);
		return -1;
	}
	...
	if (IS_ERR(accdet->pins_eint)) {
		ret = PTR_ERR(accdet->pins_eint);
		pr_err("%s: no state_eint_r_as_int pinctl.\n", __func__);
		return ret;
	}
	pinctrl_select_state(accdet->pinctrl, accdet->pins_eint);
	gpio_set_debounce(accdet->r_gpiopin, accdet->gpio_hp_deb);

	accdet->r_gpioirq = irq_of_parse_and_map(node, 1);
	pr_info("%s: r_gpioirq=%d\n", __func__, accdet->r_gpioirq);
	ret = of_property_read_u32_array(node, "interrupts", ints,
			ARRAY_SIZE(ints));
	if (ret) {
		pr_notice("accdet get interrupts array fail: %d\n", ret);
...
static void accdet_init_once(void)
{
	...
	pr_info("accdet eint %s done, irq=%d \n", __func__, accdet->gpioirq);

	ret = request_irq(accdet->r_gpioirq, ex_r_eint_handler, IRQF_TRIGGER_NONE,
		"ccdet-eint-r", NULL);
	if (ret) {
		pr_notice("accdet %s request_irq fail, ret:%d.\n", __func__,
			ret);
	...
	pr_info("accdet right eint %s done, irq=%d \n", __func__, accdet->r_gpioirq);
...
static int accdet_probe(struct platform_device *pdev)
{
	...
	snd_jack_set_key(accdet->jack.jack, SND_JACK_BTN_3, KEY_IMUSIC_COLLECT);
	...
	accdet->r_eint_workqueue = create_singlethread_workqueue("accdet_r_eint");
	INIT_WORK(&accdet->r_eint_work, right_eint_work_callback);
	if (!accdet->r_eint_workqueue) {
		ret = -1;
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_AP_GPIO_EINT)) {
		...
		if (ret)
			destroy_workqueue(accdet->r_eint_workqueue);
	}
