...
#include <linux/proc_fs.h>  // add by yanghen for codec debug
#include <sound/soc.h>
...
static int mt6358_amic_enable(struct mt6358_priv *priv)
{
	...
	if (mux_pga_l == PGA_MUX_AIN0 || mux_pga_l == PGA_MUX_AIN2 ||
		...
		regmap_update_bits(priv->regmap, MT6358_AUDENC_ANA_CON9,
				   0xff, 0x41);  // modify by audio vivo yanghen micbias0 = 2.1v
	} 
	...
	if (mux_pga_l == PGA_MUX_AIN1 || mux_pga_r == PGA_MUX_AIN1) {
		...
		if (mic_type == MIC_TYPE_MUX_DCC_ECM_SINGLE)
			...
			regmap_write(priv->regmap,
				     MT6358_AUDENC_ANA_CON10, 0x0061); // modify by audio vivo yanghen micbias1 = 2.6v 
	}
	...
	dev_info(priv->dev, "%s(), 2:MT6358_AUDENC_CON12(rc2, rc1) = 0x%x(0x%x, 0x%x)\n",
...
{
	int ret = 0;
	//yanghen add for miso begin
	int miso_drv = 0, res = 0;
	res = of_property_read_u32(priv->dev->of_node,
				"mt6358-miso-driving", &miso_drv);
	if (!res)
	...
	if (miso_drv == 4) {
		...
		dev_info(priv->dev, "%s(), set miso_drv: %dmA\n",__func__, miso_drv);
		regmap_write(priv->regmap, MT6358_DRV_CON3, 0xeeee);
	} else {
		dev_info(priv->dev, "%s(), invalue miso_drv，set default 4mA\n",__func__);
		regmap_write(priv->regmap, MT6358_DRV_CON3, 0x8888);
	}
	//yanghen add for miso end
	/* set gpio */
	playback_gpio_reset(priv);
	capture_gpio_reset(priv);
...
{
   /*add by yanghen for codec debug start*/
	if(!(inode->i_private))
	   pr_info("%s inode->i_private is null\n",__func__);
  	if (inode->i_private) {
	...
	/*add by yanghen for codec debug end*/
	return 0;
}
...
#endif
/*add by yanghen for codec debug start*/
static void __init mt6358_proc_dump_register(struct mt6358_priv *priv)
{
	kuid_t uid = KUIDT_INIT(0);
	kgid_t gid = KGIDT_INIT(1000);
	struct proc_dir_entry *entry;
	dev_err(priv->dev, "%s(),enter\n",__func__);
	entry = proc_create_data("mtksocanaaudio",0664,NULL,&mt6358_debugfs_ops,priv);
	if(!entry)
		dev_err(priv->dev,"%s() Can not create mtksocanaaudio\n", __func__);
	else
		proc_set_user(entry, uid, gid);
	dev_err(priv->dev, "%s() exit\n", __func__);
}
/*add by yanghen for codec debug end*/
...
static int mt6358_platform_driver_probe(struct platform_device *pdev)
{
    ...
    /*add by yanghen for codec create procfs file*/
	mt6358_proc_dump_register(priv);
