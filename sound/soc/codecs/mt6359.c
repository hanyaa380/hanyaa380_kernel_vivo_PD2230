...
#else
static void gpio_driving_set(struct mt6359_priv *priv,unsigned int val)
{
	/* 8:4mA(default), a:8mA, c:12mA, e:16mA */
	regmap_update_bits(priv->regmap, MT6359_DRV_CON2, 0xffff, val);
	regmap_update_bits(priv->regmap, MT6359_DRV_CON3, 0xffff, val);
	regmap_update_bits(priv->regmap, MT6359_DRV_CON4, 0x00ff, val);
}
#endif
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_err(priv->dev, "%s(), name %s, reg(0x%x) = 0x%x, set index = %x\n",  // modify by yanghen for vivo audio
...
enum {
	...
	RCV_MUX_TEST_MODE,
	//vivo chengrongfei add for handset & speaker playback simultaneously start
	RCVSPK_MUX_PLAYBACK,
	RCV_MUX_MASK = 0x4,
	//vivo chengrongfei add for handset & speaker playback simultaneously end
};

static const char * const rcv_in_mux_map[] = {
	//vivo chengrongfei add for handset & speaker playback simultaneously start
	"Open", "Mute", "Voice Playback", "Test Mode", "HSSPK Playback"
	//vivo chengrongfei add for handset & speaker playback simultaneously end
};
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		//vivo chengrongfei add for handset & speaker playback simultaneously start
		if (dapm_kcontrol_get_value(w->kcontrols[0]) == RCVSPK_MUX_PLAYBACK) {
			/* Disable handset short-circuit protection */
			regmap_write(priv->regmap, MT6359_AUDDEC_ANA_CON7, 0x0010);
		...
		if (dapm_kcontrol_get_value(w->kcontrols[0]) == RCVSPK_MUX_PLAYBACK) {
			...
			regmap_write(priv->regmap, MT6359_AUDDEC_ANA_CON6, 0x009f);
			/* Enable low-noise mode of DAC */
			  ...
			  int event)
{
	...
	unsigned int mic_type_index; /* Add by vivo audio yanghen for compatible fast charge */
	...
	if (fast_charge_flag == 1)
		...
		mic_type_index = MUX_MIC_TYPE_1;
	/* Add by vivo audio yanghen for compatible fast charge end*/
...
static int mt6359_codec_init_reg(struct mt6359_priv *priv)
{
	...
	/* gpio miso driving set */
 	...
 	dev_info(priv->dev, "%s(), set miso_drv: %dmA, reg_val is 0x%x\n",
 		__func__, miso_drv, reg_val);
	/*add by yanghen for vivo audio end*/
	/* set gpio */
	gpio_smt_set(priv);
	/*modify by yanghen for vivo audio*/
