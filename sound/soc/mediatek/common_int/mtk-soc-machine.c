...
static void __init mtk_snd_register_init(struct device *dev)
{
	...
	dev_dbg(dev, "%s() enter\n", __func__);
	entry = proc_create(DEBUG_FS_NAME, 0664, NULL, &mtaudio_debug_ops);
	if (!entry)
		dev_err(dev, "%s() Can not create mtksocaudio\n", __func__);
	else
		proc_set_user(entry, uid, gid);

	entry = proc_create(DEBUG_ANA_FS_NAME, 0444, NULL, &mtaudio_ana_debug_ops);
	if (!entry)
		dev_err(dev, "%s() Can not create mtksocanaaudio\n", __func__);
	else
		proc_set_user(entry, uid, gid);
	dev_dbg(dev, "%s() exit\n", __func__);
}
...
static struct snd_soc_dai_link mt_soc_exvivo_dai[] = {
#if 1
	{
...
static int mt_soc_snd_probe(struct platform_device *pdev)
{
...
#endif
	pr_err("%s(), vivo_codec_name = %s\n",__func__,vivo_codec_name);
	if (vivo_codec_name) {	
		mt_soc_exvivo_dai[0].codec_name = vivo_codec_name;
		mt_soc_exvivo_dai[0].codec_dai_name = "VIVO-Codec-dai";
		memcpy(mt_soc_dai_component + daiLinkNum,
		mt_soc_exvivo_dai, sizeof(mt_soc_exvivo_dai));
		daiLinkNum += ARRAY_SIZE(mt_soc_exvivo_dai);
	}
	...
	mtk_snd_register_init(&pdev->dev);
