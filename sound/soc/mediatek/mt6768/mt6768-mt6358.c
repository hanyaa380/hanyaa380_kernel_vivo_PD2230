...
/*yanghen add for vivo audio start */
#include "../../vivo/vivo-codec-common.h"
#include "../../vivo/vivo_soc_codec.h"
/*yanghen add for vivo audio end*/
/*
...
//yanghen add for vivo codec begin
	{
...
//ljp add begin for compatibling with primary supply pa and secondary supply pa
...
static int class_attr_create(struct device_node *np)
{
	...
	ret = of_property_read_string(np, "vivo,product-model", &product_name);
	if (ret < 0) {
		product_name = NULL;
		pr_info("%s: product_name is null \n", __func__);
	}

	if (product_name && (!strcmp(product_name, "PD2226F_EX"))) {
		codec_name = vivo_get_codec_name();
		if (!strncmp(codec_name, "aw87xx", 6))
			product_name = "PD2226F_EX_AW87xx";
		else if (!strncmp(codec_name, "fs15xx", 6))
			product_name = "PD2226F_EX_FS15xx";
		else
			pr_warning("%s: invalid code name:%s\n", __func__, codec_name);
	}
