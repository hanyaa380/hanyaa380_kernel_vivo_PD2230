...
/*add by yanghen for proc  dump reg  start*/
#include <linux/proc_fs.h>
#include <linux/uidgid.h>
/*add by yanghen for proc  dump reg  end*/
				      ...
				      snd_pcm_uframes_t frames)
{
	...
	if (rtd->cpu_dai->id == MT6833_MEMIF_DL1 || rtd->cpu_dai->id == MT6833_MEMIF_VUL12) {
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (rtd->cpu_dai->id == MT6833_MEMIF_DL1) {
		...
		if (rtd->cpu_dai->id == MT6833_MEMIF_VUL12) {
			vivo_audio_ktv_set_channels(substream->runtime->channels);
...
static int mt6833_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	/*add by yanghen for proc  dump reg*/
