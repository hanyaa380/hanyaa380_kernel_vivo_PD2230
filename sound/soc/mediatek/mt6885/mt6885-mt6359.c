...
extern unsigned int get_codec_hw_version(void);
...
#ifndef CONFIG_VIVO_ANA_PA /* vivo audio linzhinan add for ana pa */
...
{
	int n = 64;
	unsigned int codec_hw_ver;
	codec_hw_ver=get_codec_hw_version();
	//n = scnprintf(ubuf, n - 1, "%s", product_name ? product_name : "");
	if (product_name)
	{
		if (codec_hw_ver >= 0)
			n = scnprintf(ubuf, n - 1, "%s_%lu", product_name, codec_hw_ver);
		else
			n = scnprintf(ubuf, n - 1, "%s", product_name);
	}
	else
	{
		n = scnprintf(ubuf, n - 1, "");
	}
	ubuf[n] = 0;
	return n;
}
